...
char menuconfigFileName[2048] = {0};
...
#define MATCH_ELSE	    3
#define MATCH_IF_CUSTOM_CONFIG	    4
...
int removeDef_checkMenu(char *string, int len)
{
	char tmp[512] = {0};
	char *ptr;
	char *ptrTmp = tmp;
...
int getOptionFromMenuConfigFile(char* opt,char* menuFile)
{
	FILE *fp =NULL;
	char line[512] = {0};
	int rev = 0;
	...
	while(fgets (line, sizeof(line), fp) )
	{
		  if(strstr(line, opt) && strstr(line,"config "))
		  {
		  	fclose(fp);
//			printf("SUCESS\n");
			//if(rev == 0)
...
//modify macros to 8, tell it in SOP	
	...
	ret  = sscanf(tmpBuf, "%s %s %s %s %s %s %s %s",  \
		symb,macro1[0],tmp1,macro1[1],tmp1,macro1[2],tmp1,macro1[3]);
	...
	printf("andFlag: %d\n", andFlag);
#endif
	if(strlen(menuconfigFileName) > 0){
		for(i = 0; i < ret/2; i++)
		{
			removeDef_checkMenu(macro1[i],sizeof(macro1[i]));
			//printf("macro1[%d]: %s\n", i, macro1[i]);
			ret_status = getOptionFromMenuConfigFile(macro1[i], menuconfigFileName);
			if(ret_status)
			{
				//printf("macro1[%d]: %s return MATCH_IF_CUSTOM_CONFIG\n", i, macro1[i]);
				return MATCH_IF_CUSTOM_CONFIG;
			}
...
//		}
		...
		if ( MATCH_IF_CUSTOM_CONFIG == ret )
		{
			...
			{
				layerStack[layer++] = MATCH_IF_CUSTOM_CONFIG;
...
//				printf("444\n");
			...
			else if(layerStack[layer-1] == MATCH_IF_CUSTOM_CONFIG)
			{
				memset(tmpLine, 0, sizeof(tmpLine));
				sprintf(tmpLine,"#else\n");
				fputs(tmpLine, fpTmp);
				endifStack[layer-1] = 1;
			}
		//	endifStack[layer-1] = 1;
		...
		{
			//printf("code\n");
			if( (layer == 0) ||  (layerStack[layer-1] == REMOVE_DEF) || (layerStack[layer-1] == MATCH_IF_CUSTOM_CONFIG) )
			{
				fputs(tmpBuf, fpTmp);
...
//	system("mkdir tools/cplopts_checktool/tmp");
	if(argc == 4){
		strcpy(menuconfigFileName, argv[3]);
	}else{
		memset(menuconfigFileName,0,sizeof(menuconfigFileName));
	}
...
//	return 0;
	...
	while ( fscanf(fp, "%s", fileName) != EOF )
	{
		...
		sprintf(commandbuf, "echo %s > tools/cplopts_checktool/tmp/parsingfile", fileName);
        system(commandbuf);
...
//			printf("error occur  parseOptionsFromConfigFile in file %s\n", fileName);
	...
	//		exit(1);
		...
		system("rm -f tools/cplopts_checktool/tmp/parsingfile");
//		if(errFlag)
//			printf(".");	
	}
	printf("\ndone!\n");
