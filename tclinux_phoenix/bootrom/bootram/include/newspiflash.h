...
#define AUTO_FAST_READ_QUADOUT		(4)
#define AUTO_FAST_READ_QUADIO		(5)
...
// Parameters:
//		data:	type is char
#define SEND_WRITE_REGISTER_INTERNAL_TWO_BYTE_CMD(op_cmd, data1, data2)			\
        {														\
          SEND_CSL_CMD											\
          /* write op_cmd */										\
          SEND_OPFIFO_WRITE_CMD(OP_OUTS, OP_REPEAT_ONE)			\
          SEND_DFIFO_WRITE_CMD(op_cmd)							\
          /* write register */										\
          SEND_OPFIFO_WRITE_CMD(OP_OUTS, OP_REPEAT_TWO)			\
          SEND_DFIFO_WRITE_CMD(data1)							\
          SEND_DFIFO_WRITE_CMD(data2)							\
          SEND_CSH_CMD											\
        }
...
// Parameters:
//		data:	type is char
#define SEND_WRITE_REGISTER_TWO_BYTE_CMD(op_cmd, data1, data2)					\
        {														\
          SEND_MANUAL_WREN_BEGIN_CMD							\
          SEND_WRITE_REGISTER_INTERNAL_TWO_BYTE_CMD(op_cmd, data1, data2)		\
          SEND_MANUAL_WREN_END_CMD								\
        }
...
/* change auto mode to Fast Read Quad Output */
#define SEND_AUTO_FASTREAD_QUADOUT_CMD							\
        {														\
      	 /* auto mode is Fast Read Quad Output */					\
          WriteReg(SF_READ_MODE, AUTO_FAST_READ_QUADOUT);		\
        }

/*  change auto mode to Fast Read Quad IO */
#define SEND_AUTO_FASTREAD_QUADIO_CMD							\
        {														\
      	 /* auto mode is Fast Read Quad IO */						\
          WriteReg(SF_READ_MODE, AUTO_FAST_READ_QUADIO);		\
        }
