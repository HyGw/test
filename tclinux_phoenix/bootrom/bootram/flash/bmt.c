...
#else
#if	defined(TCSUPPORT_CPU_MT7510)||defined(TCSUPPORT_CPU_MT7520)
	return mt6573_nand_exec_read_page(&mt6573_nand_hw, page, PAGE_SIZE_BMT, dat, oob);
#else
...
#else
#if	defined(TCSUPPORT_CPU_MT7510)||defined(TCSUPPORT_CPU_MT7520)
	return mt6573_nand_block_bad_hw(&devinfo, offset, bmt_block);
#else
...
#else
#if	defined(TCSUPPORT_CPU_MT7510)||defined(TCSUPPORT_CPU_MT7520)
	return mt6573_nand_erase_hw(offset / PAGE_SIZE_BMT);
#else
...
#else
#if	defined(TCSUPPORT_CPU_MT7510)||defined(TCSUPPORT_CPU_MT7520)
	return mt6573_nand_block_markbad_hw(&devinfo, offset,bmt_block);
#else
...
#else
#if	defined(TCSUPPORT_CPU_MT7510)||defined(TCSUPPORT_CPU_MT7520)
	return mt6573_nand_exec_write_page(&mt6573_nand_hw, page, PAGE_SIZE_BMT, dat, oob);
#else	
...
/*
   version1 : do not support BMT
   version2 : support BMT
   flash total block num : 1024
   when change from version1 to version2 
   copy flashdata of bob & eeprom to the right logic address  
*/
#ifdef TCSUPPORT_NAND_COPY_BOB
int CopyFlashData(void)
{
	static unsigned char bob_data[1280] = {0};			 		//bob_data size is 1280 bytes
	static unsigned char eeprom_data[1024] = {0}; 		 		//eeprom_data size is 1024 bytes (1k)
	unsigned long bob_data_write_offset = 123207680; 	//940th block 0x7580000
	unsigned long eeprom_data_write_offset = 123469824; //942th block 0x75c0000
	unsigned long bob_data_read_offset = 133824512; 	//1021th block 0x7fa0000
	unsigned long eeprom_data_read_offset = 133890048; 	//1021th block 0x7fb0000

	unsigned long retlen = 0;
	int ret = 0;

	/*if bad block exist, modify the write_offset*/
	if(!(init_bbt.badblock_count)){
		bob_data_write_offset -= 128*1024*(init_bbt.badblock_count);
		eeprom_data_write_offset -= 128*1024*(init_bbt.badblock_count);
	}
	/*copy flash data of bob*/
	ret = nandflash_read(bob_data_read_offset, 1280, &retlen, bob_data);
	if (ret != 0)
		return -1;
	
	#ifndef LZMA_IMG
	ret = nandflash_write(bob_data_write_offset, 1280, &retlen, bob_data);
	#endif
	
 	if (ret != 0)
		return -1;
	
	/*copy flash data of eeprom*/
	ret = nandflash_read(eeprom_data_read_offset, 1024, &retlen, eeprom_data);
	if (ret != 0)
		return -1;

	#ifndef LZMA_IMG
	ret = nandflash_write(eeprom_data_write_offset, 1024, &retlen, eeprom_data);
	#endif
...
/*
	if the 940th block is empty and the 1021th block is not empty, then copy the flash data
	return : 0  empty
		     1  not empty
		     -1 fault
		
*/
int check_BMT_reservearea_empty_or_not(unsigned long index)
{
	
	unsigned char reservearea_data[16] = {0}; 		//check reservearea data buffer

	unsigned long retlen = 0;
	int ret =0;
	int i = 0;
	
	/*if bad block exist, modify the read_offset*/
	if(!(init_bbt.badblock_count)){
		index -= 128*1024*(init_bbt.badblock_count);
	}
	
	ret = nandflash_read(index, 16 , &retlen, reservearea_data);	
	
	if (ret != 0)
		return -1;

	for(i=0; i<15; i++){
		if(reservearea_data[i] != 0xFF){
			break;
		}
	}
	if(i == 15)
