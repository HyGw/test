...
if Request_Form("Upnp_Flag") = "1" then
	...
	if tcwebApi_get("WebCustom_Entry","isDMSSupported","h" ) = "Yes" then
		TCWebApi_set("DMS_Entry","Enable","DMSEnableStatus")
		TCWebApi_commit("DMS")
	end if
	tcWebApi_save()
end if
%>
<HTML><HEAD><TITLE>UPNP配置</TITLE>
...
border=0>
        ...
        <TR>
          ...
          <TD width=434 bgColor=#427594 height=33 class=CucBgColorTD_1>
            ...
            value="应用->UPNP配置" name=Selected_Menu> </FONT></B><SPAN 
            ...
            <TABLE cellSpacing=10 cellPadding=0 width="100%" border=0>
              ...
              <TR>
                ...
                <%if TCWebApi_get("WebCustom_Entry","isDMSSupported","h" ) = "Yes" then%>
                    <TR>
                      <TD>启用DMS</TD>
                      <TD><INPUT id=DMSEnable type=checkbox name="DMSEnable" <%if tcWebApi_get("DMS_Entry","Enable","h") = "Yes" then asp_Write("checked") end if%>>
                      <input type="hidden" name="DMSEnableStatus" value="<% tcWebApi_get("DMS_Entry","Enable","s") %>">
                      </TD></TR>
                <%end if%>
                      </TBODY></TABLE></P></TD>
                <TD width=10>&nbsp;</TD></TR>
...
function btnSaveUpnp()
{
	...
	<%if TCWebApi_get("WebCustom_Entry","isDMSSupported","h" ) = "Yes" then%>
	document.ConfigForm.DMSEnableStatus.value = document.ConfigForm.DMSEnable.checked ? "Yes" : "No";
	<%end if%>
