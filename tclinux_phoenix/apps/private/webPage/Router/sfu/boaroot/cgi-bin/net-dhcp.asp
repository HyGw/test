...
function DhcpCheckForm(type)
{
	with (document.forms[2])
	{
		if (1 != getCheckVal('dhcp6SrvType') || "Normal" != UseWan.value)
		{
			...
			if ( '998' == dnsSourceType )
			{
				if ("" == DnsSrvOne.value || true != CheckPrefix(DnsSrvOne_Source.value,1))
				{
					alert('首选DNS服务器 : "' + DnsSrvOne_Source.value + '" 不是合法的IPv6地址 !');
					return false;
				}
				
				if ("" != DnsSrvTwo.value && true != CheckPrefix(DnsSrvTwo_Source.value,1))
				{
...
<!-- InstanceEndEditable --></P></TD>
          <TD width=170 
bgColor=#313031 class=CucBgColorTD_2>　</TD></TR></TBODY></TABLE></TD></TR></TBODY></TABLE><BR></BODY></HTML>
