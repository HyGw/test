...
function uiDoUpdate()
{
	...
	if (form.tools_FW_UploadFile.value=="") {
		...
		else if (form.upload_type[2].checked)
		...
		if("<%tcWebApi_get("System_Entry", "upgrade_vdsl_co_romfile", "h")%>" == "Yes") {
			if (((form.upload_type[0].checked) && (string3 >= 0) && (string4 < 0)) || 
			...
			if (((form.upload_type[0].checked) && (string3 >= 0)) || 
				((form.upload_type[1].checked) && (string5 >= 0))) {
				form.uiStatus.value = "Upgrade in progress, Please wait...";
				form.postflag.value = "1";
				form.submit();
			}
...
<input name="upload_type" type="radio">VDSL CO romfile
<%end if%>
<input name="upload_type" type="radio" value="4">tclinux.bin
<%else%>
<input name="upload_type" type="radio" value="1" style="display:none">
<input name="upload_type" type="radio" value="4" CHECKED style="display:none">tclinux.bin
<%end if%>
<%else%>
<input name="upload_type" type="radio" value="1" CHECKED>romfile
<%if tcWebApi_get("System_Entry", "upgrade_vdsl_co_romfile", "h") = "Yes" then%>
...
<INPUT TYPE="FILE" NAME="tools_FW_UploadFile" SIZE="30" MAXLENGTH="128"></td></tr>
<%if tcwebApi_get("WebCustom_Entry","isThreeLevelChangePassWord","h") = "Yes" then%>
<%if TCWebAPI_get("WebCurSet_Entry", "CurrentAccess", "h") = "0" then %>
<tr>
<td class="light-orange">&nbsp;</td><td class="light-orange">&nbsp;</td><td class="tabdata"><div align=right>
...
<%if tcWebApi_get("System_Entry", "upgrade_vdsl_co_romfile", "h") = "Yes" then%>
<div><input type="button" value="VDSL CO ROMFILE BACKUP" onClick='backup_vdsl_settings()'></div>
<%end if%>
</td></tr>
<%end if%>
<%else%>
...
<INPUT TYPE="TEXT" NAME="uiStatus" SIZE="55" MAXLENGTH="55" disabled VALUE="<% if tcWebApi_get("System_Entry","upgrade_fw_status","h") = "NONE" then asp_Write("") elseif tcWebApi_get("System_Entry","upgrade_fw_status","h") = "FAIL" then asp_Write("The file is illegal, please to upload again!!") end if%>" class="InputTextWarning">
</td></tr><tr>
...
<%else%>
<%if tcwebApi_get("WebCustom_Entry", "isProgressBarSupported","h")="Yes" then%>
<head>
