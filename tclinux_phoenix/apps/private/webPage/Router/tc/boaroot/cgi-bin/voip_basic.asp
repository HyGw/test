...
function check_rtpPort(sipPort, rtpPort){
	if(Number(sipPort) == Number(rtpPort)){
		alert('RTP Port ' + rtpPort + ' is the same as SIP port');
		return false;
	}
	return true;
}
...
function dosave(){
	...
	if((check_rtpPort(form.VoIPLocalSIPPort.value, form.VoIPLocalRTPPort.value) == false)){
			return;
	}
...
<%if TCWebAPI_get("WebCurSet_Entry", "SIPIndexBasic", "h") = "0" then%>
		<td class="tabdata"><font color="#000000"><%if TCWebAPI_get("VoIPSysParam_Entry0", "SC_ACCT_INFO_REG_STATUS", "h") = "Registered" then asp_Write("Connected") else asp_Write("Not Connected") end if %></font></td></tr>
<%end if%>
<%if TCWebAPI_get("WebCurSet_Entry", "SIPIndexBasic", "h") = "1" then%>
		<td class="tabdata"><font color="#000000"><%if TCWebAPI_get("VoIPSysParam_Entry1", "SC_ACCT_INFO_REG_STATUS", "h") = "Registered" then asp_Write("Connected") else asp_Write("Not Connected") end if %></font></td></tr>
<%end if%>
