...
function chkPortRange()
{
	...
	for(i = 0; i < 8; i++)
	{
		...
		{
			active++;
			if(isNaN(document.forms[0].elements[i*7+4].value)|| 
			isNaN(document.forms[0].elements[i*7+5].value)||
			isNaN(document.forms[0].elements[i*7+7].value)|| 
			isNaN(document.forms[0].elements[i*7+8].value))
			{
				alert("The port value must be number!");return true;
			}		
		}
		else
			continue;
...
function doSave() 
{
	if (chkPortRange())
	 ...
	 if (chkRuleLimit())
	 {
		alert('Free port triggering rule is not enough!Please check Port Triggering Summary');
	 	return;
	 }	
