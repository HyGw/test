...
function onClickRunWizard() {
	...
	window.open("/cgi-bin/wizardstart.asp","OpenClose","toolbar=no,menubar=no,height=430,width=520,location=0,left=100,top=100");
	<%else%>
...
<table width="760" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr>
    ...
    <td width="500" class="databold" height="150"><span class="Model">
        ...
        This <%if tcWebApi_get("WebCustom_Entry","haveXPON","h") = "Yes" then%>xPON<%Else%><%if tcWebApi_get("WebCustom_Entry","havePtm","h") = "Yes" then%>VDSL/<%end if%>ADSL<%End If%> Router 
		is ideal for home networking and small business networking.
        The 'Quick Start' wizard will guide you to configure the 
		<%if tcWebApi_get("WebCustom_Entry","haveXPON","h") = "Yes" then%>xPON<%Else%><%if tcWebApi_get("WebCustom_Entry","havePtm","h") = "Yes" then%>VDSL/<%end if%>ADSL<%End If%> Router 
