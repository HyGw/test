...
<% if tcwebApi_get("WebCustom_Entry","isCTCPU7521Supported","h") = "Yes" then %>
					|| parseInt(weightArr[idx]) <= 0
<%end if%>
					)
			{
				alert('权重值非法');
                ...
                <TD>
                  ...
                  <TABLE width=420 border=1>
                    ...
                    <TR>
                      ...
                      <TD align=middle><INPUT id=Q14Enable type=checkbox name="Q14Enable" <%if tcWebApi_get("QoS_Common","QueueSW6","h") = "Yes" then asp_Write("checked") end if%>></TD></TR></TBODY></TABLE></DIV><% if tcWebApi_get("WebCustom_Entry", "isCTPONCYJXSupported","h") = "Yes" then if tcWebApi_get("DataSpeedLimit_Entry","SpeedLimitModeUP","h") <> "0" then %></div><% end if end if %></DIV>
                          <P> 
