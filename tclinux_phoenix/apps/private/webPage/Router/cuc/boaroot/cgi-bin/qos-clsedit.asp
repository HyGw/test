...
<td height="33" width="434" bgcolor="#427594" class=CucBgColorTD_1>
...
<td height="33" width="211" bgcolor="#EF8218" class=CucBgColorTD_1>
...
<tr>
<td height="43" colspan="2"  bgcolor="#EF8218" valign="bottom" id="MenuArea_L1" class=CucBgColorTD_1>&nbsp;</td>
</tr>
<tr>
<td colspan="2" height="24" bgcolor="#427594" id="MenuArea_L2" class=CucBgColorTD_3></td>
</tr>
</table>
<script language="javascript">
...
<tr>
<td width="157" class="Item_L1" height="29"><p>QoS</p></td>
...
for(k=0; k < AppCnt; k++)
{
	...
	{
		if(CurQoS.App[k].AppName == "VOIP")
			AppNameStr = "VOICE";
		else
			AppNameStr = CurQoS.App[k].AppName;
			
		loc += '<TD align="center">'+ AppNameStr +'</TD>';
	}
...
function stEther(Domain,Name)
{
this.Domain = Domain;
this.Name = Name;
}
var EtherList = new Array();
EtherList[0] = new stEther("1","ARP");
EtherList[1] = new stEther("2","IPv6");
EtherList[2] = new stEther("3","IPv4");
...
function writeTypeTable(domain)
{
	...
	for(k = 0; k < ClsTypeCnt; k++)
	{
		...
		else if (CurTypeArray[k].Type == "EtherType")
		{
			for(i=0;i<EtherList.length;i++)
			{
				if (EtherList[i].Name == CurTypeArray[k].Min)
				{
					loc += '<TD align="center">' + EtherList[i].Name + '</TD>';
					loc += '<TD align="center">' + EtherList[i].Name + '</TD>';
					break;		
				}
			}
...
function EtherTypeChange()
{
	if(getElById('TypeEtherInterFaceMin').value == "ARP")
	{
		setDisplay('ProtocolType', 0);
		setSelect('ProtocolList','all');
	}
	else
		setDisplay('ProtocolType', 1);
}

function WriteEtherInterFace()
{
	var loc,k;
	loc = '<table width="100%" border="0">';
	loc +='<tr><td width="90">以太网类型：</td>';
	loc +='<td colspan="6">';
	loc +='<select size="1" id="TypeEtherInterFaceMin" name="TypeEtherInterFaceMin" onChange="EtherTypeChange()">';
	if(EtherList!=null)
	{
		if(EtherList.length > 0)
		{
			for(i=0;i<EtherList.length;i++)
			{
				if (i==0)
					loc +='<option value="'+EtherList[i].Name+'" selected>'+ EtherList[i].Name +'</option>';
				else
					loc +='<option value="'+EtherList[i].Name+'" >'+ EtherList[i].Name +'</option>';
			}
		}
	}
	loc +='</select></td>';
...
function btnEditClsType(index)
{
	...
	else if(getValue('Type')=="EtherType")
	{
		...
		WriteEtherInterFace();		
		setDisplay('ether', 1);
		setSelect('TypeEtherInterFaceMin', CurTypeArray[CurEditTypeIndex].Min);
		EtherTypeChange();
	}
	else
	{
...
function QoSTypeSubmit()
{
	...
	with (getElById('ConfigForm'))
	{
			...
			{
				vForm.Min.value = vForm.TypeEtherInterFaceMin.value;
				vForm.Max.value = vForm.TypeEtherInterFaceMin.value;				
			}
...
<option value="VOIP" <%if tcWebApi_get("QoS_Entry","AppName","h") = "VOIP" then asp_Write("selected") end if%>>VOICE</option>
...
<table width="104%" border="0"  id="ProtocolType">  <tr>
<td width="94">协议类型：</td>
...
<td width="170" bgcolor="#313031" class=CucBgColorTD_2>　</td>
</tr>
</table>
</td>
</tr>
