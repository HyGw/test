...
function LoadFrame()
{
	with ( document.forms[0] )
	{
		...
		var h_key = [hkey1, hkey2, hkey3, hkey4];
		for ( i = 0; i < 4; i++)
		{
			//wlKeys[i].value = keys[i][1];
			wlKeys[i].value = h_key[i].value;
		}
		...
		setText('wlWpaPsk',hwpakey.value);
...
function addParameter(Form)
{
	...
	if (<% tcWebApi_get("WebCurSet_Entry","wlan_id","s") %> == 0)
	{
		if(document.forms[0].isCUCSupport.value == 1){
			...
			var the_substring = ssid.substring(0, 3);
			if (the_substring != 'CU-')
			{
				alert('SSID1 "' + getValue('wlSsid') + '" 应该是以"CU-"开头的字符串');
				setCheck('wlEnbl',1);
				wlClickEnble(getElById('wlEnbl'));
				return false;
			}
      ...
      <TABLE height="100%" cellSpacing=0 cellPadding=0 border=0>
		...
		if (curUserName != sptUserName)
			getElement("11NgiControl").style.display = "none";
              ...
              <TR>
                <TD width=10>&nbsp;</TD>
                <TD width=10><input type="hidden" name="Save_Flag" value="0">
            <INPUT type='hidden' name=hkey1 value="<%if tcWebApi_get("WLan_Entry","Key1Str","h") <> "N/A" then tcWebApi_get("WLan_Entry","Key1Str","s") end if%>">
          	<INPUT type='hidden' name=hkey2 value="<%if tcWebApi_get("WLan_Entry","Key2Str","h") <> "N/A" then tcWebApi_get("WLan_Entry","Key2Str","s") end if%>">
          	<INPUT type='hidden' name=hkey3 value="<%if tcWebApi_get("WLan_Entry","Key3Str","h") <> "N/A" then tcWebApi_get("WLan_Entry","Key3Str","s") end if%>">
          	<INPUT type='hidden' name=hkey4 value="<%if tcWebApi_get("WLan_Entry","Key4Str","h") <> "N/A" then tcWebApi_get("WLan_Entry","Key4Str","s") end if%>">
            <INPUT type='hidden' name=hwpakey value="<%if tcWebApi_get("WLan_Entry","WPAPSK","h") <> "N/A" then tcWebApi_get("WLan_Entry","WPAPSK","s") end if%>">    
            </TD>
