...
if Request_Form("Wan_Flag") = "1" then
	...
	if Request_Form("PDUsed") = "Yes" then
		...
		TCWebApi_set("WanInfo_WanIF","DHCPv6PD","enablepd")
	else
		TCWebApi_set("WanInfo_WanIF","PDEnable","PDDisabled")
		TCWebApi_set("WanInfo_WanIF","DHCPv6PD","PDDisabled")
	end if
...
document.write(top.gateWayName);
          ...
          <TD width=211 bgColor=#DA6825 height=33 class=CucBgColorTD_1>
...
<% end if %>
	...
	if (AddFlag == true)
	{
		if ((serviceList.value == "OTHER" || serviceList.value == "IPTV") && wanMode.value == "Bridge")
		{
...
function ServiceListLoad(isBridge)
{
	...
	<% else %>
	var routeArray = new Array('TR069', 'INTERNET', 'TR069_INTERNET', 'VOICE', 'TR069_VOICE', 'VOICE_INTERNET', 'TR069_VOICE_INTERNET', 'IPTV','OTHER');
	<% end if %>

	var bridgeArray = new Array('INTERNET', 'IPTV', 'OTHER');
	var i = 0;
	...
	else
	{
				if (getSelectVal('linkMode') == 'linkPPP')
				{
					...
					if (getCheckVal('cb_enabledslite') == 1){
						...
						{
							alert("MTU非法，请输入值为1320~1492之间的整数！(或0默认)");
							return false;
						}					
					}else{
				...
				else
				{
					...
					else
					{
						if (getCheckVal('cb_enabledslite') == 1){
							...
							{
								alert("MTU非法，请输入值为1320~1500之间的整数！(或0默认)");
								return false;
							}				
						}else{
...
function MultiVIDDispChange()
{
	...
	with (getElById('ConfigForm'))
	{
		...
		if ( svrList.indexOf('INTERNET') >= 0 || svrList.indexOf('IPTV') >= 0 || svrList.indexOf('OTHER') >= 0 )
...
function WanIdChange()
{
	...
	document.ConfigForm.submit();
                  ...
                  <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
                    ...
                    <TR>
                      <TD width=150>模式：</TD>
                      <TD width=200><LABEL><select id=select2 onChange=WanModeSelect() name="wanMode">
