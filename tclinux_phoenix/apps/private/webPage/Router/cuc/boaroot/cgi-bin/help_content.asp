...
<td class="tabdataleft">
<ul>
<li id=statusACS_1>显示交互建立情况、业务配置下发状态，交互建立包括主动上报Inform情况以及接受RMS连接请求情况。</li>
</ul>
</td>
</tr>
...
<td>&nbsp;</td>
<td class="tabdataleft"><% if TCWebApi_get("WebCustom_Entry","isWLanSupported","h" ) = "Yes" then %>5<% else %>4<% end if%>.远程管理<a name=远程管理 id="远程管理"></a></td>
</tr>
...
<td>&nbsp;</td>
<td class="tabdataleft"><ul>
<li>用于设置RMS服务器的URL或者IP地址，RMS认证网关的用户名和密码，以及网关认证RMS的用户名和密码。同时还可以配置网关是否发送周期性上报报文，以及上报报文的周期。另外还可以进行证书认证的启用或禁用。</li>
<li>OLT认证注册</li>
</ul></td>
</tr>
...
<li>维护结束上报:当联通维护人员在维护中使用了联通维护帐号时，通过该界面操作通知RMS更新联通维护帐号的密码。</li>
</ul></td>
</tr>
