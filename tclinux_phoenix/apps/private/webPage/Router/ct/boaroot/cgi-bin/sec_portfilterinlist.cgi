...
function doDel(i)
{
	...
	var tempstr;
	if(this.rmlin.length>0)
	{
		for(var n=0;n<(this.rmlin.length);n++)
		{
			if(this.rmlin[n].value == i){
				if(this.rmlin[n].checked)
				{
					tempstr = i + \",\";
	...
	{
		if(this.rmlin.checked) f.delnum.value = i + \",\";
		else f.delnum.value = \"\";
	}
...
bgcolor=\"#FFFFFF\"><tr><TD class=table_title align=middle width=\"10%\"><STRONG>名称</STRONG></TD><td width=\"10%\" align=center
...
do
	...
	then
		DesPort=$LINE
		echo "<tr align=\"middle\"><td width=\"10%\">$IPName</td><td width=\"10%\">$Protocol</td><td width=\"14%\">$SrcIPAddr</td><td
