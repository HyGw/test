...
#include "voipmgr.h"
#include "voipmgr_global.h"
#include "voipmgr_helper.h"
...
#define CHK_DSP_STATE	if (0 == gDspState) { \
							...
							printf("DSP has been revoked, please exit voipmgr, "); \
...
void cmdDspInvoke(const char *inStr) {
	...
	if (gDspState > 1) {
		printf("DSP has been revoked, please exit voipmgr, remove and re-insert DSP modules, and start again!\n\n");
		return;
	}
		
	if (ECNT_E_SUCCESS != voipmgr_lib_dsp_query(&dspContext, &active, &feature)) {
	...
	if (ECNT_E_SUCCESS != voipmgr_lib_dsp_invoke(&dspContext)) {
	...
	if (ECNT_E_SUCCESS != voipmgr_lib_poll_event(BLOCKING, &mEvent)) {
...
void cmdDspRevoke(const char *inStr) {
	...
	if (1 == gDspState) {
		...
		if (!strcasecmp("yes", cmdLine)) {
			printf("Revoking DSP ...\n\n");
			if (ECNT_E_SUCCESS != voipmgr_lib_dsp_revoke(&dspContext)) {
				printf("Error: DSP shutdown failed!\n");;
				return;
			}

			gDspState++;
			printf("DSP shutdown completed! You can quit voipmgr safely now.\n");
		}
		return;
	}
...
void cmdDspConfigTone(const char *inStr) {
	...
	if (isValidToneid(pVal)) {
		...
		if(ECNT_E_SUCCESS !=  voipmgr_lib_dsp_config_tone(&dspContext, toneId, &gTones[toneId])){
			printf("Error: voipmgr_lib_dsp_config_tone failed!\n");
			return;
		}
		return;
	}
...
void cmdDspConfigCpt(const char *inStr) {
	...
	if (isValidCptType(pVal)) {
		...
		if(ECNT_E_SUCCESS !=  voipmgr_lib_dsp_config_cpt(&dspContext, &gCpts[cptIndex])){
			printf("Error: voipmgr_lib_dsp_config_cpt failed!\n");
			return;
		}
		return;
	}
...
void cmdChanConfig(const char *inStr) {
	...
	if (isValidChanId(pVal)) {
		ch = atoi(pVal);
 		dspContext.chanId = ch;
		
		if (ECNT_E_SUCCESS != voipmgr_lib_chan_query(&dspContext, &config)) {
		...
		if(ECNT_E_SUCCESS !=  voipmgr_lib_chan_config(&dspContext, &config)){
			printf("Error: voipmgr_lib_chan_config failed!\n");
			return;
		}
		if (ECNT_E_SUCCESS != voipmgr_lib_chan_query(&dspContext, &config)) {
			printf("Execution failed! Cannot retrieve channel configuration.\n\n");
	...
	if (isValidChanId(pVal)) {
		...
		if (isValidNum(pVal, ~(MAX32 >> 1), MAX32 >> 1)) {
			...
			if (strlen(pVal) > 0) {
				...
				printf("dur:%d\r\n", dur);
				if(ECNT_E_SUCCESS != voipmgr_lib_chan_play_tone(&dspContext, &toneList,rpt,dur)){
					printf("Error: voipmgr_lib_chan_play_tone failed! \n");
					return;
				}
...
void cmdChanStopTone(const char *inStr)
{
	...
	if (isValidChanId(pVal)) {
        ...
        if(ECNT_E_SUCCESS != voipmgr_lib_chan_stop_tone(&dspContext)){
            printf("Error: voipmgr_lib_chan_stop_tone failed! \n");
            return;
        }
...
void cmdChanPlayDtmfTone(const char *inStr)
{
	...
	if (isValidChanId(pVal)) {
		...
		if (isValidNum(pVal, ~(MAX32 >> 1), MAX32 >> 1)) {
			...
			if(ECNT_E_SUCCESS != voipmgr_lib_chan_play_dtmf(&dspContext, digit, dur)){
				printf("Error: voipmgr_lib_chan_play_dtmf failed! \n");
				return;
			}
...
void cmdChanPlayCid(const char *inStr) {
	...
	if (isValidChanId(pVal)) {
		...
		if(ECNT_E_SUCCESS != voipmgr_lib_chan_play_cid(&dspContext, &cid)){
			printf("Error: voipmgr_lib_chan_play_cid failed! \n");
			return;
		}	

		printf("Generate %s caller ID [%s] name [%s]on channel (%d),the fskower is %d mwi=%s\n" \
			, etosCidFormat(cid.format), cid.number,cid.name, dspContext.chanId, cid.fskPower,gValidMWIStrVal[cid.mwi]);
...
void cmdChanPlayCid2(const char *inStr) {
	...
	if (isValidChanId(pVal)) {
		...
		if(ECNT_E_SUCCESS != voipmgr_lib_chan_play_type2_cid(&dspContext, &cid)){
			printf("Error: voipmgr_lib_chan_play_type2_cid failed! \n");
			return;
		}
		...
		printf("Generate %s caller ID [%s] name [%s] on channel (%d),cidp=:%d mwi=%s T9=%d T10=%d T11=%d T12=%d T13=%d T14=%d" \
			...
			, etosCidFormat(cid.format), cid.number,cid.name, dspContext.chanId, cid.fskPower,gValidMWIStrVal[cid.mwi],cid.cid2timing.T9,cid.cid2timing.T10,cid.cid2timing.T11,
...
void cmdChanPlayDtmfCid(const char *inStr) {
	...
	if (isValidChanId(pVal)) {
		...
		if (ECNT_E_SUCCESS != voipmgr_lib_chan_play_dtmf_cid(&dspContext, &cid)){
			printf("Error: voipmgr_lib_chan_play_dtmf_cid failed! \n");
			return;
		}

		printf("Generate dtmf CID [%s] on channel (%d),maketime:%d breaktime:%d dtmfp:%d dtmfT3:%d,please check the telephone!\n" \
		, cid.number, dspContext.chanId, cid.makeTime,cid.breakTime,cid.dtmfPower,cid.dtmfT3);
		return;
	}
...
void cmdChanPcmDump(const char *inStr) {
	...
	if (isValidChanId(pVal)) {
		...
		if (isValidIp(pVal)) {
			...
			if(ECNT_E_SUCCESS != voipmgr_lib_chan_dump_pcm(&dspContext, &mAddr)){
				printf("Error: voipmgr_lib_chan_dump_pcm failed! \n");
				return;
			}
...
void cmdStrmConfig(const char *inStr) {
	...
	strmConfig_t config;
	activeState_e active;
	dspContext_t *dspContext_p = &dspContext;
	CHK_DSP_STATE

 	getAttrVal(inStr, "st", "=", pVal);
	if (isValidStrmId(pVal, &(dspContext_p->chanId), &(dspContext_p->strmId))) {

		if (ECNT_E_SUCCESS != voipmgr_lib_strm_query(dspContext_p, &active, &config)) {
		...
		getAttrVal(inStr, "red", "=", pVal);
		if (strlen(pVal) > 0 && isValidOnOff(pVal)) {
			config.strmAttr.rtpRedundant= convertControl(pVal);
		}

		getAttrVal(inStr, "red_mode", "=", pVal);
		if (strlen(pVal) > 0 && isValidNum(pVal, 0, 2)) {
		...
		if (ECNT_E_SUCCESS != voipmgr_lib_strm_config(dspContext_p, &config)) {
		...
		printf("Channel %d -> Stream %d Configuration:\n", dspContext_p->chanId, dspContext_p->strmId);
		...
		printf("ulBitrate: %s \n", etosBitrate(config.strmAttr.ulBitrate));   
#if defined(CZ_DSP_SUPPORT) || defined(CJ_DSP_SUPPORT) 
...
void cmdStrmStart(const char *inStr) {
	...
	if (isValidStrmId(pVal, &(dspContext_p->chanId), &(dspContext_p->strmId))) {
		...
		if (ECNT_E_SUCCESS != voipmgr_lib_strm_start(dspContext_p)) {
...
void cmdStrmStop(const char *inStr)
{
	...
	if (isValidStrmId(pVal, &(dspContext_p->chanId), &(dspContext_p->strmId))) {
		...
		if (ECNT_E_SUCCESS != voipmgr_lib_strm_stop(dspContext_p)) {
	...
	if (isValidStrmId(pVal, &(dspContext_p->chanId), &(dspContext_p->strmId))) {
		...
		if (isValidNum(pVal, ~(MAX32 >> 1), MAX32 >> 1)) {
			...
			if (strlen(pVal) > 0) {
				...
				toneList.numOfTone = sz;
				toneList.toneIdSeq = mTone;
				if (ECNT_E_SUCCESS != voipmgr_lib_strm_play_tone(dspContext_p, &toneList, rpt)) {
...
void cmdStrmQueryMediaInfo(const char *inStr){
	...
	if (isValidStrmId(pVal, &(dspContext_p->chanId), &(dspContext_p->strmId))) {
		...
		if (ECNT_E_SUCCESS != voipmgr_lib_strm_query_media_info(dspContext_p, &mediaInfo)) {
...
void cmdStrmResetMediaInfo(const char *inStr){
	...
	if (isValidStrmId(pVal, &(dspContext_p->chanId), &(dspContext_p->strmId))) {
		...
		if (ECNT_E_SUCCESS != voipmgr_lib_strm_reset_media_info(dspContext_p)) {
...
void cmdInfcQuery(const char *inStr)
{
	...
	if (isValidInfcId(pVal)) {
		...
		if(ECNT_E_SUCCESS != voipmgr_lib_infc_query(infc, &infcConf)){
			printf("Error: voipmgr_lib_infc_query failed! \n");
			return;
		}
	
		if(ECNT_E_SUCCESS != voipmgr_lib_infc_slic_type_query(infc, &slicParams)){
			printf("Error: voipmgr_lib_infc_slic_type_query failed! \n");
			return;
		}
	...
	if (isValidInfcId(pVal)) {
		...
		if (INFC_FXS == infcConf.type) {
			...
			if (LINE_ERROR == lineState) {
				printf("Incorrect line state! line=[down | fwd | rev | ring | revoht]");
			}
			else {
				infcConf.lineState = lineState;

				if(ECNT_E_SUCCESS != voipmgr_lib_infc_config_line(infc, infcConf.lineState)){
					printf("Error: voipmgr_lib_infc_config_line failed! \n");
					return;
				}
	...
	if (isValidInfcId(pVal)) {
		...
		if (INFC_FXO == infcConf.type) {
			...
			if (-1 == hookState) {
				...
				if(ECNT_E_SUCCESS != voipmgr_lib_infc_config_hook(infc, infcConf.hookState)){
					printf("Error: voipmgr_lib_infc_config_hook failed! \n");
					return;
				}
...
void cmdInfcConfigHwProf(const char *inStr) {
	...
	hwProf = atoi(pVal);	
	voipmgr_lib_infc_config_hw_prof(infc,hwProf);	
...
void cmdInfcConfigHookTs(const char *inStr) {
	...
	if (isValidInfcId(pVal)) {
		...
		if(ECNT_E_SUCCESS != voipmgr_lib_infc_config_hook_ts(infc, &infcConf.hookTs)){
			printf("Error: voipmgr_lib_infc_config_hook_ts failed! \n");
			return;
		}
...
void cmdInfcConfigRing(const char *inStr) {
	...
	if (isValidInfcId(pVal)) {
		...
		if (INFC_FXS == infcConf.type) {
			...
			if(ECNT_E_SUCCESS != voipmgr_lib_infc_config_ring(infc, &(infcConf.ring))){
				printf("Error: voipmgr_lib_infc_config_ring failed! \n");
				return;
			}
...
void cmdInfcRing(const char *inStr) {
	...
	if (isValidInfcId(pVal)) {
		...
		if (INFC_FXS == infcConf.type) {
			...
			if(ECNT_E_SUCCESS != voipmgr_lib_infc_ring(infc, infcConf.ring.dur, &(infcConf.ring.cid))){
				printf("Error: voipmgr_lib_infc_ring failed! \n");
				return;
			}
		}
		else {
...
void cmdPcmLoopback(const char *inStr){
	...
	if (!strcasecmp("on", pVal)){
		voipmgr_lib_pcm_loopback(&dspContext, CONTROL_ENABLE);
		return;
	}
	else if (!strcasecmp("off", pVal)){
		voipmgr_lib_pcm_loopback(&dspContext, CONTROL_DISABLE);
		return;
	}
...
void cmdRtpLoopback(const char *inStr){
	...
	if (isValidChanId(pVal)) {
		...
		if (!strcasecmp("on", pVal)){
			voipmgr_lib_rtp_loopback(&dspContext, CONTROL_ENABLE);
			return;
		}
		else if (!strcasecmp("off", pVal)){
			voipmgr_lib_rtp_loopback(&dspContext, CONTROL_DISABLE);
			return;
		}
		return;
	}
...
void cmdDspLoopback(const char *inStr){
	...
	if (isValidChanId(pVal)) {
		...
		if (!strcasecmp("on", pVal)){
			voipmgr_lib_dsp_loopback(&dspContext, CONTROL_ENABLE);
			return;
		}
		else if (!strcasecmp("off", pVal)){
			voipmgr_lib_dsp_loopback(&dspContext, CONTROL_DISABLE);
			return;
		}
		return;
	}
...
void cmdDspReplaceData(const char *inStr){
	...
	voipmgr_lib_dsp_replace_data(&dspContext, &pReplaceConf);
	return;
...
void cmdInfcLineTest(const char *inStr){
	...
	if (isValidChanId(pVal)) {
		...
		if(ECNT_E_SUCCESS != voipmgr_lib_infc_line_test(&lineTest)){
			printf("Error: adamInfcLineTest failed! \n");
			return;
		}

		if(ECNT_E_SUCCESS != voipmgr_lib_infc_slic_type_query(lineTest.infc, &slicParams)){
			printf("Error: voipmgr_lib_infc_slic_type_query failed! \n");
			return;
		}
...
void cmdInfcRingParams(const char *inStr){
	...
	if (isValidInfcId(pVal)) {
		...
		if (INFC_FXS == infcConf.type) {
			...
			if(ECNT_E_SUCCESS != voipmgr_lib_infc_ring_params(infc, &infcConf.ringParams)){
				printf("Error: voipmgr_lib_infc_ring_params failed! \n");
				return;
			}
...
void cmdInfcDcFeedParams(const char *inStr){
	...
	if (isValidInfcId(pVal)) {
		...
		if (INFC_FXS == infcConf.type) {
			...
			if(ECNT_E_SUCCESS != voipmgr_lib_infc_dc_feed_params(infc, &infcConf.dcFeedParams)){
				printf("Error: voipmgr_lib_infc_dc_feed_params failed! \n");
				return;
			}
...
void cmdInfcCULineTest(const char *inStr){
	...
	if (isValidChanId(pVal)){
		...
		if(ECNT_E_SUCCESS != voipmgr_lib_infc_cu_line_test(&lineTest, tmpSlicType, &unicomLineTest)){
			printf("Error: voipmgr_lib_infc_cu_line_test failed! \n");
			return;
		}
...
void cmdQuit(const char *inStr) {
	char cmdLine[MAX_CMD_LEN] = {0};
	printf("Quit voipmgr and shutdown DSP, sure? (type 'yes' to quit) ");
	...
	if (!strcasecmp("yes", cmdLine)) {
		if (1 == gDspState) {
			voipmgr_lib_strm_stop_all(&dspContext); /*release streams before exiting*/
			if (ECNT_E_SUCCESS != voipmgr_lib_dsp_revoke(&dspContext)) {
				printf("Error: DSP shutdown failed!\n");;
				return;
			}
...
int invokeOnce = 0;
int voipmgrApplication = 0;//1: with test machine; 2: tester; 3 wait tester digit input
unsigned long VOIPMGR_SINGAL;
...
} voipmgrApplication_state; 
void quitEvcom(void)
{
	if(voipmgrApplication == TEST_TESTER)
	{
		char tmp[32] = {0};
		printf("Strm Stop....\n\n");
...
void recvDigit(int chan, int digit)
{
	char lastRecv[32]={0};
	char tmp='\0';
	if(voipmgrApplication !=TEST_TESTER_WAIT_DTMF){
		return;
	}
...
void checkDigit(int chan)
{
	...
	if(strcmp(recvBuf[chan],RING_DTMF)==0){
		...
		//voipmgrApplication=TEST_TESTER;
		//sem_post(&RING_SINGAL);
	    memset(recvBuf[chan],0,32);
	}
	return;
...
void signal_handler(int signum)
{
	...
	if (ECNT_E_SUCCESS != voipmgr_lib_strm_stop_all (&dspContext)) {
...
main(int argc, char* argv[]) {
	...
	dspContext.dspId = DSP_MTK;


	printf("Initializing ADAM ... \n");
	if (ECNT_E_SUCCESS != voipmgr_lib_init()) {
		printf("ADAM initialization failed!! Exit the applicatoin!\n\n");
		exit(-1);
	}

	if (ECNT_E_SUCCESS == voipmgr_lib_query(&adConf)) {
	...
	if((argc == 2) && ((strcmp(argv[1],"on")==0)||(strcmp(argv[1],"tester")==0)))
	{
		...
		{
			printf("usage: voipmgr [on | tester]");
			return -1;
		}

		ret = sem_init(&VOIPMGR_SINGAL,0,0);
		if(ret != 0)
		{
			printf("Semaphore SEM_INFORM initialization fail!\n");
			return -1;
	...
	else
	{
		...
		printf("Closing VoIPMGR ... : %s \n\n", etosExec(voipmgr_lib_exit()));
	
	}
#if 0//def TCSUPPORT_CY_VOIP_SIP
	if((argc == 2) && ((strcmp(argv[1],"on")==0)||(strcmp(argv[1],"tester")==0)))
	{
		tcdbg_printf("exit voipmgr \n");
		invokeOnce = 0;
		sem_destroy(&VOIPMGR_SINGAL);
	}
#endif
...
static void _threadEventPolling(void *argv)
{
	...
	switch(mEvent->evtCode) {
		...
		case EVENT_CODE_CALLERID_SEND:
			...
			break;
#ifdef CJ_DSP_SUPPORT
		case EVENT_CODE_FIRST_RTP_DELIVERD:  
