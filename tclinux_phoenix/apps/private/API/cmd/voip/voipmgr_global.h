...
#ifndef _VOIPMGR_GLOBAL_H_
#define _VOIPMGR_GLOBAL_H_
#include "voipmgr.h"
...
command_t gCmdSet[NUM_OF_COMMAND] = {
    ...
    { .code = CMD_CHAN_PLAY_DTMF_TONE,
      ...
      .usage = "<chanplaydtmftone | cpt> ch=[channel_id] digit=[dtmf_digit] dur=[duration]",
      .example = "EVCom >> chanplaytone ch=0 digit=1 dur=5000",
   ...
   { .code = CMD_INFC_HW_PROF,
      ...
      .example = "EVCom >> infcconfighook if=2 HwProf=2",
    ...
    { .code = CMD_INFC_CONFIG_RING,
      ...
      .usage =  "<infcconfigring | icr> if=[interface_id] $attr0=[value] $attr1=[value] ...\n" \
      			...
      			"attr: cidpwr=[pwr lv], i.e. cidpwr=-9\n" \
      ...
      .example = "EVCom >> infcconfigring if=0 dur=500 cad0=500,1000 cad1=0,0 cad2=0,0 cidf=us \n"
                 "cidb=0 cidt=100 cid=1234  cidpwr=-9",
...
/* Support line test.*/
    { .code = CMD_INFC_LINE_TEST,
      ...
      .usage = "InfcLineTest | ir> if=[interface_id] testid=[test_items_id,zarlink:1~4,7,10~16 siliconlab:1~7].\n",
...
/*Support ring parameters set. */
    { .code = CMD_INFC_RING_PARAMS,
      ...
      .usage = "<InfcRingParams | irp> if=[channel_id] prof=<sine | trap> freq=[frequence(mHz)] amp=[amplitude(mV)] dcb=[dcBias(mV) \ 
...
/*Support dc feed parameters set. */
    { .code = CMD_INFC_DCFEED_PARAMS,
      ...
      .usage = "<InfcDcFeedParams | idp> if=[channel_id] ila=[current(uA)] ilaSlab=[current(mA)]\n",	/* added by mtk08176_20150627  */
...
/* China Unicom inside and outside line test */
    { .code = CMD_INFC_CULINETEST_PARAMS,
      ...
      .usage = "<InfcCULineTest | iult> if=[channel_id]\n",
      .example = "EVCom >>InfcDcFeedParams if=0\n",
