...
/*
** $Id: //BBN_Linux/Branch/Branch_for_Rel_CMCC_7526_20161014/tclinux_phoenix/apps/private/TR69_64/cwmp/Includes/cwmpRpc.h#1 $
*/
/************************************************************************
 *
...
/*The following defines are for theIADFlag,  mtk06404, 20140418*/
#define CWMP_IAD_ACTIVE			(1 << 0)
#define CWMP_IAD_INPROGRESS		(1 << 1) 
#define CWMP_IAD_TERMINATED		(1 << 2) //used to check the IAD test is terminated unusally or not.
...
#endif
/* replace wchttp download with wget method. */
#define CWMP_DIAGNOSTICS_WGET_DOWNLOAD_ACTIVE (1 << 3)
#define CWMP_DIAGNOSTICS_CURL_UPLOAD_ACTIVE (1 << 4)
#define TMP_DNS_TR69_FILETRANS "/etc/tr69DWUPUrl.conf"
...
typedef enum{
	...
	IllegalFirmware = 9018,
	InvalidRequestUsername = 9800,
	NoGetValueFunction = 9801, //added by hqfeng,2007.01.05
	WriteSimCardFail = 9802,
	SimCardAuthenticationFail = 9803,
	NoGetAttributeFunction = 9804 //added by hqfeng,2007.01.05
} FaultCode;
...
typedef struct DeviceIdStruct{
	...
	char OUI[MANUFACUREROUILEN];
	char productClass[PRDDUCTCLASSLEN];
	char serialNumber[SERIALNUMLEN];
...
typedef struct flagStruct{
	...
	uint8  theIADFlag;  //Add for IAD diagnose, mtk06404, 20140418
...
#define	ALARM_NONE_FLAG		0
#define	ALARM_FLAG		(1<<0)
#define	CLEARALARM_FLAG		(1<<1)
typedef struct DeviceAlarmNodeInfo {
	uint8 usedFlag;
	uint8 configIndex;
	uint8 typeFlag;
	uint32 count;
...
}DeviceAlarmNodeInfo, *DeviceAlarmNodeInfoPtr;

#define	MONITOR_NONE_FLAG	0
#define	MONITOR_FLAG		(1<<0)
typedef struct DeviceMonitorNodeInfo {
	uint8 usedFlag;
	uint8 configIndex;
	uint8 typeFlag;
	char paramValue[10][16];
	ParameterType type[10];
	struct timeval startTime;
}DeviceMonitorNodeInfo, *DeviceMonitorNodeInfoPtr;
...
typedef struct MonitorCollectorNodeInfo {
	struct timeval startTime;
}MonitorCollectorNodeInfo, *MonitorCollectorNodeInfoPtr;

typedef struct ParaListWanInfo {
	int isDelete;
	int ip_ppp;
	int iParaListItem;
	int pvcIndex;
	int entryIndex;
}ParaListWanInfo, *pParaListWanInfo;
