...
#if/*TCSUPPORT_COMPILE*/ defined(TC_SUPPORT_3G) && defined(TCSUPPORT_CD_WIND)
if [ "$TC_SUPPORT_3G" != "" ] && [ "$TCSUPPORT_CD_WIND" != "" ]; then
fail_count=0
...
#if/*TCSUPPORT_COMPILE*/ defined(TC_SUPPORT_3G) && defined(TCSUPPORT_CD_WIND)
if [ "$TC_SUPPORT_3G" != "" ] && [ "$TCSUPPORT_CD_WIND" != "" ]; then
	PPP_PARAM="unit $k user $USERNAME password $PASSWORD nodetach holdoff 4 maxfail $fail_count usepeerdns lcp-echo-interval 1 lcp-echo-failure 60"
fi
#endif/*TCSUPPORT_COMPILE*/
...
if [ "$TCSUPPORT_UNIQUEMAC" != "" ] ;then
	...
	if [ "$MSS" = "0" ] || [ "$MSS" = "" ] ; then
		if [ "$TCSUPPORT_FON_V2" != "" ] ; then
			iptables -I FORWARD -o ppp$k -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
		else
		iptables -A FORWARD -o ppp$k -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
		fi
	else
		if [ "$TCSUPPORT_FON_V2" != "" ] ; then
			iptables -I FORWARD -o ppp$k -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS
	else
