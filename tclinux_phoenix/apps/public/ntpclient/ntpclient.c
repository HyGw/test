...
#define ENABLE_DEBUG
#define BIND2DEVICE 1
#define ENABLE_MULTISERVER 1
...
#if BIND2DEVICE
char *wanif = NULL;
#define BIND2DEVICE_OPTION "w:"
#else
#define BIND2DEVICE_OPTION
#endif
...
#if ENABLE_MULTISERVER
	return rfc1305print(data,&udp_arrival_ntp);
#else
...
int ntpdnsrequest(const char *url){
	FILE *fp = NULL;
	char buffer[256] = {0};
	
	fp = fopen(TMP_DNS_NTP_PATH, "w");
	if(fp == NULL){
		perror("open ntpdns.conf err");
		return -1;
	}
...
#if BIND2DEVICE	
	if(wanif) ntpdnsrequest(hostname);
#endif	
	ntpserver=gethostbyname(hostname);
#if BIND2DEVICE	
	if(wanif) unlink(TMP_DNS_NTP_PATH);
#endif
...
int primary_loop(int usd, int num_probes, int cycle_time)
{
	...
	for (;;) {
		...
		#if ENABLE_MULTISERVER
			if(!udp_handle(usd,incoming,pack_len,&sa_xmit,sa_xmit_len)){
				if (debug) printf("udp_handle done...\n");
				return 0;
			}
		#else
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		c = getopt( argc, argv, "c:" DEBUG_OPTION BIND2DEVICE_OPTION"h:i:p:lrs");
		if (c == EOF) break;
		switch (c) {
			case 'c':
	...
	foreach(ntps, hostname, next) {
	...
	#if BIND2DEVICE	
		if(wanif) setsockopt(usd, SOL_SOCKET, SO_BINDTODEVICE, wanif, strlen(wanif) + 1) ;
	#endif	
		...
		if(setup_transmit(usd, ntps, NTP_PORT)){
		#if ENABLE_MULTISERVER
			if (debug) {
				printf("trans err...\n");
			}
...
#if ENABLE_MULTISERVER
	if (debug) {
		printf("commit timezone res=%d ..\n", res);
	}
	if(res == 0) tcapi_commit("Timezone_Entry_Success");
	else tcapi_commit("Timezone_Entry_Fail");
#endif
	return res;
}
