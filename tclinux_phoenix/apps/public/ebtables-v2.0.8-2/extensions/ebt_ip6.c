...
#define IP_SPORT  '5'
#define IP_DPORT  '6'
#if defined(TCSUPPORT_CZ_GENERAL) || defined(TCSUPPORT_CT_PORTSLIMIT)
#define IP_ICMPV6TYPE   '7'
#endif
...
#if defined(TCSUPPORT_CZ_GENERAL) || defined(TCSUPPORT_CT_PORTSLIMIT)
	{ "ip6-icmpv6type"       , required_argument, 0, IP_ICMPV6TYPE   },
#endif
	{ 0 }
};
...
#if defined(TCSUPPORT_CZ_GENERAL) || defined(TCSUPPORT_CT_PORTSLIMIT)
"--ip6-icmpv6type   [!] icmpv6type   : icmpv6 type\n"
#endif
);
}
...
#if defined(TCSUPPORT_CZ_GENERAL) || defined(TCSUPPORT_CT_PORTSLIMIT)
	case  IP_ICMPV6TYPE:
		ebt_check_option2(flags, OPT_ICMPV6TYPE);
		if (ebt_check_inverse2(optarg)) {
			ipinfo->invflags |= EBT_IP6_ICMPV6TYPE;
		}
		i = strtoul(optarg, &end, 10);
		ipinfo->icmpv6type = (unsigned char) i;
		ipinfo->bitmask |= EBT_IP6_ICMPV6TYPE;
		break;
#endif
	default:
		return 0;
...
#if defined(TCSUPPORT_CZ_GENERAL) || defined(TCSUPPORT_CT_PORTSLIMIT)
	if (ipinfo->bitmask & EBT_IP6_ICMPV6TYPE) {
		printf("--ip6-icmpv6type ");
		if (ipinfo->invflags & EBT_IP6_ICMPV6TYPE)
			printf("! ");
		printf("%d ",ipinfo->icmpv6type);
	}
#endif
}
...
#if defined(TCSUPPORT_CZ_GENERAL) || defined(TCSUPPORT_CT_PORTSLIMIT)
	if (ipinfo1->bitmask & EBT_IP6_ICMPV6TYPE) {
		if ( ipinfo1->icmpv6type != ipinfo2->icmpv6type )
			return 0;
	}
#endif
	return 1;
