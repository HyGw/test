...
#include "../../../private/tcapi_lib/libtcapi.h"
#endif
	...
	)
{
	...
	if (cmd_put) {
		opcode = TFTP_WRQ;
			...
			if (transferFlag == 1)
			{
				memset(&omciMsg, 0, sizeof(notify_msg_t));
				omciMsg.notifyType = OMCI_NOTIFY_TYPE_AVC;
				omciMsg.avcId = OMCI_AVC_ID_FILE_TRANSFER_STATUS;
				omciMsg.otherInfo[0] = 5;
				sendOmciNotifyMsg(&omciMsg);
			}
#endif				
	while (1) {
...
#define PUT
...
#ifdef TCSUPPORT_OMCI	
	tcapi_get("OMCI_Entry", "TransferFlag", buffer);
	transferFlag = (unsigned char)atoi(buffer);
	if (transferFlag == 1)
	{
		if (gethostbyname(argv[optind]) == NULL)
		{
			memset(&msg, 0, sizeof(notify_msg_t));
			msg.notifyType = OMCI_NOTIFY_TYPE_AVC;
			msg.avcId = OMCI_AVC_ID_FILE_TRANSFER_STATUS;
			msg.otherInfo[0] = 3;
			sendOmciNotifyMsg(&msg);
			printf("tftp_main:SEND a AVC to OMCI 3---URI undefined or unreachable");	
		}
	}
#endif	
	...
	if (transferFlag == 1)
	{
		...
		printf("tftp_main: result = %x", result);
	}
#endif
