...
"$Id: //BBN_Linux/Branch/Branch_for_Rel_CMCC_7526_20161014/tclinux_phoenix/apps/public/rp-pppoe-3.10/src/if.c#1 $";
...
#if defined(TCSUPPORT_CT) && !defined(TCSUPPORT_MULTICAST_SPEED)
	if ( 0 != setsockopt(fd, SOL_SOCKET, SO_TYPE_WAN_BI, &val, sizeof(val)) )
		tcdbg_printf("\nppp-relay set SO_TYPE_WAN_BI failed \n");
#endif
...
static int check_bind(char *wan_if, __u32 mark)
{
	...
	memset(wan_if_nas, 0, sizeof(wan_if_nas));
	for (i = 0; i < MAX_PVC_NUM; i++) {
		for (j = 0; j < MAX_SMUX_NUM; j++) {
			...
			if (!strcmp(lan_if_val, "Yes")) {
				tcapi_get(wan_node, "NASName", wan_if_nas);
				bind_flag = 1;
				break;
			}
		}
...
int check_ppp_packets(int sock, int type)
{
#define SO_MARK			36
	int len = sizeof(int);
	unsigned int skb_mark = 0;
	int bind_reval = 0;

	if ( ETH_PPPOE_DISCOVERY != type ) // only check discover packets.
		return 0;

	if ( 0 != getsockopt(sock, SOL_SOCKET, SO_MARK, &skb_mark, &len) )
		return -1;

	bind_reval = check_bind(getFWdevname(), skb_mark);
	if ( bind_reval > 0  //  lan if is not  binded
		&& 0 == isInternetInterface() )
	{
		//tcdbg_printf("\ndbg: other iface ,lan not bind\n"); 
		return -1;
	}
