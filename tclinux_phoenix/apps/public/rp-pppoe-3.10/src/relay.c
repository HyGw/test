...
* LIC: GPL
*
* $Id: //BBN_Linux/Branch/Branch_for_Rel_CMCC_7526_20161014/tclinux_phoenix/apps/public/rp-pppoe-3.10/src/relay.c#1 $
*
***********************************************************************/
...
extern int setFWdevname(char *name);
extern int check_ppp_packets(int sock, int type);
#endif
...
relayGotDiscoveryPacket(PPPoEInterface const *iface)
{
	...
	size = ntohs(packet.length) + HDR_SIZE;
	 ...
	 switch ( packet.code )
	 {
		case CODE_PADI:
		case CODE_PADR:
			if ( 0 != check_ppp_packets(iface->discoverySock, packet.ethHdr.h_proto) )
			{
				//tcdbg_printf("\ndbg: check_ppp_packets fail\n");
				return ;
			}
			break;
		default:
