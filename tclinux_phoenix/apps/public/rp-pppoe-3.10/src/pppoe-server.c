...
*
* $Id: //BBN_Linux/Branch/Branch_for_Rel_CMCC_7526_20161014/tclinux_phoenix/apps/public/rp-pppoe-3.10/src/pppoe-server.c#1 $
*
* LIC: GPL
*
***********************************************************************/
...
#if defined(TCSUPPORT_CT_PPPOEPROXY)	
    int mark;
    if(ioctl(sock, SIOCGSKBMARK, &mark) < 0){
		tcdbg_printf("ioctl:%s\n", strerror(errno));
    }
    port_id = (mark & 0xf0000000) >> 28;
...
#if defined(TCSUPPORT_CT_PPPOEPROXY)
    /* add a new argument to record user side lan port */
    argv[c++] = "lanport";
    sprintf(port, "%d", port_id);
    argv[c++] = port;
#endif
