...
#if defined(TCSUPPORT_CT_DNSBIND)
	  setsockopt(fd, SOL_IP, IP_SKB_MARK_FLAG, &opt, sizeof(opt)) == -1 || 
#endif

#if defined(TCSUPPORT_CT_VLAN_BIND)
	  setsockopt(fd, SOL_IP, IP_SKB_VLAN_ID_FLAG, &opt, sizeof(opt)) == -1 || 
#endif
      !fix_fd(fd) ||
      !fix_fd(tcpfd) ||
...
int isds_server(struct server *svr)
{
	int ret = 0;
	struct server *start = NULL;
	struct server *another_same = NULL;
	...
	for (start = daemon->servers; start; start = start->next)
	{
		if ( start->flags & SERV_DUAL_STACK
			&& svr != start )
		{
			if ( !(start->flags & SERV_DUAL_STACK_DOUBLE) && !(start->flags & SERV_DUAL_STACK_SINGLE) )
			{
				if ( 0 == strcmp(svr->interface, start->interface) )
				{
					if ( svr->addr.sa.sa_family == start->addr.sa.sa_family )
						another_same = start;
					else
					{
						start->flags |= SERV_DUAL_STACK_DOUBLE;
						ret = 1;
					}
				}
			}
	...
	{
		if ( 0 == ret )
			another_same->flags |= SERV_DUAL_STACK_SINGLE;
		else
			another_same->flags |= SERV_DUAL_STACK_DOUBLE;
	}

	return ret;
}

void check_dualstack_servers(void)
{
	struct server *start = NULL;
	...
	for (start = daemon->servers; start; start = start->next)
	{
		if ( start->flags & SERV_DUAL_STACK )
		{
			if ( !(start->flags & SERV_DUAL_STACK_DOUBLE) && !(start->flags & SERV_DUAL_STACK_SINGLE) )
			{
				if ( 1 == isds_server(start) )
					start->flags |= SERV_DUAL_STACK_DOUBLE;
				else
					start->flags |= SERV_DUAL_STACK_SINGLE;
			}
		}
	}
}
