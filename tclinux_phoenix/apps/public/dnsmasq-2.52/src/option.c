...
static const struct myoption opts[] = 
  ...
  { 
    ...
    { "test", 0, 0, LOPT_TEST },
#if defined(TCSUPPORT_CT_ADV_DNSPROXY)
    { "query-delay", 1, 0, LOPT_QUERY_TIMEOUT },
#endif
    { NULL, 0, 0, 0 }
  };
...
#if defined(TCSUPPORT_CT_ADV_DNSPROXY)
  { LOPT_QUERY_TIMEOUT, ARG_DUP, "timeout", gettext_noop("Specify DNS query delay time-out."), NULL },
#endif
  { 0, 0, NULL, NULL, NULL }
}; 
	...
	if (!arg || !*arg)
	...
	else
	  {
		...
		!atoi_check16(portno, &source_port))
	      problem = _("bad port");
#if defined(TCSUPPORT_CT_ADV_DNSPROXY)
		if ((pdsmode = split_chr(source, '$'))
			&& 'd' != pdsmode[0] )
			problem = _("bad dual stack mode");
#endif
...
#if defined(TCSUPPORT_CT_ADV_DNSPROXY)
	case LOPT_QUERY_TIMEOUT:
	{
		if ( atoi_check(arg, &i) )
			 daemon->delay_timeout = i;
		break;
	}
#endif
...
void read_opts(int argc, char **argv, char *compile_opts)
{
  ...
  daemon->log_fac = -1;
#if defined(TCSUPPORT_CT_ADV_DNSPROXY)
  daemon->delay_timeout = DNS_DELAY_TIMEOUT;
#endif
