...
#define SERV_DELAY_SENT     	 (1<<10)
#define SERV_FREC_DELAY_SENT     (1<<11)
#define SERV_DUAL_STACK     	 (1<<12)
#define SERV_DUAL_STACK_SINGLE   (1<<13) // dual stack interface but only v4 or v6 dns exist
#define SERV_DUAL_STACK_DOUBLE   (1<<14)
#endif
...
#if defined(TCSUPPORT_CT_ADV_DNSPROXY)
struct frec_svr {
	struct server *dstsvr;
	struct frec_svr *next;
};
#endif


#if defined(TCSUPPORT_CT_ADV_DNSPROXY)
#define	DNS_DELAY_TIMEOUT 5
#define FREC_DELAY_SENT			(1<<0)
#define FREC_LOOKUP_OK			(1<<1)
#endif
...
struct frec {
  ...
  int fd, forwardall;
	  ...
	  unsigned short   lVlanId;
  unsigned int mark;
#endif
  unsigned int crc;
  time_t time;
#if defined(TCSUPPORT_CT_ADV_DNSPROXY)
  struct frec_svr *frec_server;
  char *packet;
  size_t plen;
  int flags;
#endif
  ...
  int voip_frec;
	...
	struct server *urlDnsServer1;
	struct server *urlDnsServer2;
	struct server *last_server;	
	struct server *pre_server;		
#endif
...
#define MAX_GROUP_IPLEN 64
#define AWN_IP_NUM 8
#define AWN_DOMAIN_NUM 8
struct attach_server_group {
	union mysockaddr addr;
	struct attach_domain_group *dnslist;
	struct attach_server_group *next;
};
struct attach_domain_group {
	char *domain;
	struct attach_domain_group *next;
};
/*
...
#define MAX_DNS_LIMIT_CLIENTS 30

enum dns_speed_limit_mode{
	DNS_SPEED_LIMIT_MODE_OFF,
	DNS_SPEED_LIMIT_MODE_PERDOMAIN,
	DNS_SPEED_LIMIT_MODE_ALL
};

enum dns_speed_limit_action{
	DNS_SPEED_LIMIT_ACTION_NONE,
	DNS_SPEED_LIMIT_ACTION_ALERT,
	DNS_SPEED_LIMIT_ACTION_DROP
};

enum dns_speed_limit_host_ipver{
	DNS_SPEED_LIMIT_HOST_UNKNOW,
	DNS_SPEED_LIMIT_HOST_IPV4,
	DNS_SPEED_LIMIT_HOST_IPV6,
	DNS_SPEED_LIMIT_HOST_BOTH
};

struct dns_speed_limit_client_group {
	int grp_index;
	unsigned short ipver;
	unsigned char hwaddr[ETH_ALEN];
	union mysockaddr addrv4;
	union mysockaddr addrv6;

	struct dns_speed_limit_client_group *next;
};

struct dns_speed_limit_group {
	char *domain;
	struct dns_speed_limit_client_group *client_group;
	int limit_count;
	int curr_count;
	time_t time;

	struct dns_speed_limit_group *next;
};
...
struct url {
 ...
 #endif
#if defined(TCSUPPORT_CT_JOYME)
  union mysockaddr addr[AWN_IP_NUM];
  unsigned char addr_used[AWN_IP_NUM];
#endif
};
...
enum url_type{
	...
	URL_IPPINGDIAGNOSTIC,
#endif
#if defined(TCSUPPORT_CT_MONITORCOLLECTOR)
	URL_MONITORCOLLECTOR,
#endif
...
#define VPN_DELENTRY		"VPN_DelDomainEntry"
#define VPN_INSTANCE_NUM	16
#define VPN_ATTACH_NUM		512

/* VPN tunnel domain list */
struct vpn_dns_group {
	char *domain;
	int vpn_tunnel_idx;

	struct vpn_dns_group *next;
};
...
#endif
#if defined(TCSUPPORT_CT)
struct URLFilter_Info
{
	char strUrlName[64];
	struct URLFilter_Info* next;
};
#if defined(TCSUPPORT_CT_VLAN_BIND)
...
#define MAX_GRP_IPLEN 40
struct dns_ip_group {
	int mode;
	int flags;
	struct all_addr SIP;
	struct all_addr DIP;
	struct dns_ip_group *next;
};
...
#if (defined(TCSUPPORT_CT_VLAN_BIND) && defined(TCSUPPORT_CT))
#if KERNEL_3_18_21
#define	INVALID_VLAN_ID VBIND_INVALID_VLANID
#else
#define	IP_SKB_VLAN_ID				51
#define	IPV6_SKB_VLAN_ID			75
