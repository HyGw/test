...
//gcyin 
	...
	else if(!strcmp(servicetype,GateServiceType[2]))
	{
		...
		else if(!strcmp(cmd,"GetStatusInfo"))
		{
			succeed = 1;
			
			sprintf((char *)linerep,"<NewConnectionStatus>Connected</NewConnectionStatus>\n<NewLastConnectionError>ERROR_NONE</NewLastConnectionError>\n<NewUptime>%s</NewUptime>\n", "0");	
		}
		...
		else if(!strcmp(cmd,"AddPortMapping"))
		{
				...
				&&(GetFirstXmlItemValue((char *)value_desc,"NewPortMappingDescription",body)!=-1))
			{
				...
				if(FindPortMapList(value_proto,atoi(value_extport)))
				{
					...
					PortMapDelete(value_proto, atoi(value_extport), wanip);	
					if(DeletePortMapList(value_proto,atoi(value_extport)) == 1)
						PortMapNumOfEntries--;
				}
				
				
				if(PortMapNumOfEntries > 64)
				{
					/*Random delete one entry*/
					int     ExternalPort;
        			char    Protocol[32];
					strcpy(Protocol, PortMapListPtr->Protocol);
					ExternalPort = PortMapListPtr->ExternalPort;
					getIpAddrFromIfname(WAN_IFNAME,wanip);
					PortMapDelete(Protocol, ExternalPort, wanip);	
					if(DeletePortMapList(Protocol, ExternalPort) == 1)
						PortMapNumOfEntries--;
