...
int main( int argc, char *argv[] )
{
	...
	while( 1 )
	{
		...
		option = getopt_long( argc, argv, "s:n:o:S::NqwrvhVaH:U:", axel_options, NULL );
		if( option == -1 )
			break;
	...
	i = axel->ready ? 0 : 2;
	ret = (axel->size == axel->bytes_done) ? axel->bytes_done : 0;
...
void print_help()
{
	...
	printf(	_("Usage: axel [options] url1 [url2] [url...]\n"
		...
		"--write-null\t\t-w\tdon't write data to file\n"
