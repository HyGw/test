...
static void ieee802_1x_encapsulate_radius(rtapd *rtapd, struct sta_info *sta, u8 *eap, size_t len)
{
	...
	if (sta->identity && !Radius_msg_add_attr(msg, RADIUS_ATTR_USER_NAME, sta->identity, sta->identity_len))
	{
	   ...
	   // apd->conf->own_ip_addr is filled according to configuration file
			...
			if(getsockname(rtapd->radius->mbss_auth_serv_sock[sta->ApIdx], (struct sockaddr *) &host, &host_len) != 0)
			{
				perror("get host ip:");		
			}
	#else
		
			if(getsockname(rtapd->radius->auth_serv_sock, (struct sockaddr *) &host, &host_len) != 0)
			{
				perror("get host ip:"); 	
			}			
	#endif		
   
	if (!Radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IP_ADDRESS, (u8 *) &(host.sin_addr.s_addr), 4))
	{
