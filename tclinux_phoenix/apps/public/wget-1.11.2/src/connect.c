...
#define	IP_SKB_PBIT 52
  ...
  if (opt.ipv6_only) {
	...
	int revbufsize = 262144; //256k
	setsockopt (sock, SOL_SOCKET, SO_RCVBUF,
                  (void *)&revbufsize, (socklen_t)sizeof (revbufsize));
#endif
#if defined(TCSUPPORT_CT_TR143_WGET_DOWNLOAD)
  if ( 1 == opt.tr143_flag )
  {
	  if ( opt.tr143_waniface
	  	&& -1 == setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE,
	  						opt.tr143_waniface, 32) )
		tcdbg_printf("\nwget setsockopt is faild(SO_BINDTODEVICE)\n");
	  pbit = opt.tr143_ethernet_priority;
	  if ( pbit >= 0 && pbit <= 7 )
	  {
		  if( setsockopt(sock, SOL_IP, IP_SKB_PBIT
		  				,(void *)&pbit, sizeof(pbit)) == -1)
			  tcdbg_printf("\nwget setsockopt is faild(IP_SKB_PBIT)\n");
	  }
  }
	...
	if ( 1 == opt.tr069_flag )
	{
		//tcdbg_printf("\nconnet_to_ip: opt.tr069_flag == 1!!\n");
		if ( opt.tr069_waniface
			&& -1 == setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE,
							  opt.tr069_waniface, 32) )
			tcdbg_printf("\nwget for tr069 setsockopt is faild(SO_BINDTODEVICE)\n");
	}
#endif
