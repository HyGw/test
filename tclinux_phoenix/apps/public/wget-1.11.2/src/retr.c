              ...
              wgint *qtyread, wgint *qtywritten, double *elapsed, int flags)
{
...
#if defined(TCSUPPORT_CT_PHONEAPP)
	unsigned int count_time = 1;
#endif

#if defined(TCSUPPORT_CT_TR143_WGET_DOWNLOAD)
#if defined(TCSUPPORT_CT_PON_SC)
	char tmpvalue[64] = {0};
	double dlrate_kb, secs, pre_secs = 0;
	uint32_t old_count = 0, new_count = 0;
	int samplevalue_count = 0;
  ...
  while (!exact || (sum_read < toread))
...
#if defined(TCSUPPORT_CT_TR143_WGET_DOWNLOAD)
		gTR143DiagInfo.EOMInOctet += ret;
#if defined(TCSUPPORT_CT_PON_SC)
		new_count += ret;
		secs = ptimer_read (timer);
		
		if(secs-pre_secs > 1 && samplevalue_count<20){
			/* calu the download rate */
			dlrate_kb = (double)((new_count - old_count) / (secs - pre_secs));
			dlrate_kb = dlrate_kb / 1024;

			if (old_count != 0) {
				if(strlen(sampleValueBuf)==0){
					sprintf (tmpvalue, "%.2f", dlrate_kb);	
					strcpy(sampleValueBuf,tmpvalue);
					samplevalue_count = 1;
				}
				else{
					sprintf (tmpvalue, "|%.2f", dlrate_kb);
					strcat(sampleValueBuf,tmpvalue);
					samplevalue_count++;
				}
			}
	...
	if(opt.ratetesttime > 0)
	{
		if(ptimer_read (timer) >= count_time)
		{
			count_time ++;
			if(count_time > opt.ratetesttime)
				goto out;
		}
	}
#endif
...
{
  double dlrate;
#if 0//defined(TCSUPPORT_CT_TR143_WGET_DOWNLOAD)
#if defined(TCSUPPORT_CT_PON_SC)
  char tmpvalue[64] = {0};
  double dlrate_kb;
  static double pre_secs = 0;
  static int samplevalue_count = 0;
#endif
#endif
