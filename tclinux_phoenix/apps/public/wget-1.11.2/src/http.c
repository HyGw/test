...
gethttp (struct url *u, struct http_stat *hs, int *dt, struct url *proxy)
{
  ...
  char cmdbuf[128] = {0}, nodeval[64] = {0}, tmpvalue[64] = {0};
  char *ptmp = NULL;
#endif
  ...
  if (sock < 0)
    {
		...
		if ( 1 == opt.tr143_flag )
		{
			gettimeofday( &gTR143DiagInfo.sysTimeTCPOpenRequest, NULL );
		}
#endif
      ...
      if (sock == E_HOST)
        {
		  ...
		  if ( 1 == opt.tr143_flag )
		  {
			  gTR143DiagInfo.diagtate = eState_Error_InitConnectionFailed;
		  }
#endif
          request_free (req);
          return HOSTERR;
	  ...
	  if ( 1 == opt.tr143_flag )
	  {
		  gettimeofday( &gTR143DiagInfo.sysTimeTCPOpenResponse, NULL );
	  }
#endif
#ifdef HAVE_SSL
	...
	if ( 1 == opt.tr143_flag )
	{
		gettimeofday( &gTR143DiagInfo.sysTimeROM, NULL );
		/* BOMInOctet must be recorded before send http get, 
		it's too late for small http data if record it at http 200 ok. */
		if ( opt.tr143_waniface )
		{
			snprintf(tmpvalue, sizeof(tmpvalue) - 1, "%s"
					, opt.tr143_waniface);

			/* transfer ppp interface to nas interface */
		
			if (strstr(tmpvalue, "ppp") != NULL) {
				int pvc_index = 0, entry_index = 0, wan_index = 0;
				char *p_index = tmpvalue;
				wan_index = atoi(p_index+3);
				pvc_index = wan_index / 8;
				entry_index = wan_index - 8*pvc_index;
				sprintf(tmpvalue, "nas%d_%d", pvc_index, entry_index);
			}
			...
			snprintf(cmdbuf, sizeof(cmdbuf) - 1,
					"Info_%s",
					tmpvalue);
			tcapi_get( cmdbuf, "rxbytes", nodeval );
			//gTR143DiagInfo.BOMInOctet = atoi(nodeval);
			gTR143DiagInfo.BOMInOctet = 0;
		}
	}
#endif
  ...
  *dt &= ~RETROKF;
  ...
  if (!opt.server_response)
	...
	if ( 1 == opt.tr143_flag )
	{
		if ( eWGetHttpOK == statcode )
		{
			gettimeofday( &gTR143DiagInfo.sysTimeBOM, NULL );
		}
		else if ( eWGetHttpBadRequest == statcode )
		{
			gTR143DiagInfo.diagtate = eState_Error_InitConnectionFailed;
		}
		else if ( eWGetHttpObjectNotFound == statcode )
		{
			gTR143DiagInfo.diagtate = eState_Error_NoResponse;
		}
		else
		{
			if ( eWGetHttpUnAuthorized != statcode
				&& eWGetHttpNoContent != statcode )
			{
				gTR143DiagInfo.diagtate = eState_Error_NoResponse;
			}
		}
...
exit:
  if (ret == RETROK) 
  {
    *local_file = xstrdup (hstat.local_file);
	...
	if ( 1 == opt.tr143_flag )
	{
		gettimeofday( &gTR143DiagInfo.sysTimeEOM, NULL );
		gTR143DiagInfo.diagtate = eState_Complete;
		...
		if (strstr(tmpvalue, "ppp") != NULL) {
			...
			entry_index = wan_index - 8*pvc_index;
			sprintf(tmpvalue, "nas%d_%d", pvc_index, entry_index);
			tcdbg_printf("\r\nwan_index = %d, pvc_index = %d, entry_index = %d", wan_index, pvc_index, entry_index);
		}
		...
		snprintf(cmdbuf, sizeof(cmdbuf) - 1,
				...
				tmpvalue);
		tcapi_get( cmdbuf, "rxbytes", nodeval );
		//gTR143DiagInfo.EOMInOctet = atoi(nodeval);
		gTR143DiagInfo.testBytesReceived = hstat.len;
	}
#endif
  }
#if defined(TCSUPPORT_CT_TR143_WGET_DOWNLOAD)
  ...
  {
	  if ( (1 == opt.tr143_flag) && 0 == gTR143DiagInfo.diagtate )
	  {
	  	gTR143DiagInfo.diagtate = eState_Error_TransferFailed;
	  }
  }
#endif
