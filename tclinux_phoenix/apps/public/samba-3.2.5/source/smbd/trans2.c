	...
	*got_exact_match = False;
	...
	while (!found) {
		...
		if (mangle_must_mangle(dname,conn->params)) {
			...
			//fname = mangled_name;
			fname = dname;// add by jiehao
		}

		if(!(got_match = *got_exact_match = exact_match(conn, fname, mask))) {
			got_match = mask_match(fname, mask, conn->case_sensitive);
		}
		/*if(!(got_match = *got_exact_match = exact_match(conn, dname, mask))) {
			got_match = mask_match(dname, mask, conn->case_sensitive);
		}//add by jiehao*/
	...
	switch (info_level) {
		...
		case SMB_FIND_FILE_BOTH_DIRECTORY_INFO:
			...
			len = srvstr_push(base_data, flags2, p,
					  ...
					  STR_TERMINATE_ASCII);
			/*len = srvstr_push(base_data, flags2, p,
					  dname, PTR_DIFF(end_data, p),
					  STR_TERMINATE_ASCII);//add by jiehao*/
			SIVAL(q,0,len);
			p += len;
