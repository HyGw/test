...
#if defined (HAVE_EVENTFD)
#if 0 //ndef TEST_EVENTFD_FALLBACK
...
g_wakeup_acknowledge (GWakeup *wakeup)
{
  ...
  while ((ret = read (wakeup->fds[0], buffer, sizeof (buffer)) ) == sizeof(buffer)){
  	//g_print("acknowledge:%d, %d,%d!\n",wakeup->fds[0],ret, ++i);
  };
  //g_print("acknowledge end:%d, %d,%d!\n",wakeup->fds[0], i);
}
...
g_wakeup_signal (GWakeup *wakeup)
{
  ...
  if (wakeup->fds[1] == -1)
	...
	//g_print("send:%d,%d,%d,%d!\n",wakeup->fds[0], res, ++i, sizeof(guint64));
      	}
