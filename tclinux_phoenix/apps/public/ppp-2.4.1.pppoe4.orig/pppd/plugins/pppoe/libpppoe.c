...
int discover_timeout[] = { 3, 3, 3, 10, 10, 10, 60*7 };
int max_count = sizeof(discover_timeout)/sizeof(int), max_wait_position = sizeof(discover_timeout)/sizeof(int) - 1;
extern int isDemand();
extern int checkUptream(int *total_time);
#endif

#if defined(TCSUPPORT_CT_JOYME) || defined(TCSUPPORT_CT_JOYME2)
int discover_timeout_yh[] = { 5, 10, 20, 40, 90 };
...
int session_connect(struct session *ses)
{
    ...
    int ret;
#if defined(TCSUPPORT_CT_PON_SC)
	int detectUpStream = 0, discover_continue = 0;
	int wait_wan_timeout = 10;
#endif
...
#if defined(TCSUPPORT_CT_PON_SC)
	ses->wait_discover_timeout = discover_timeout[max_wait_position];
#endif
...
#if defined(TCSUPPORT_CT_PON_SC)
		if ( ses->retransmits >= max_wait_position )
		{
			if ( isDemand() )
			{
				if ( 0 == detectUpStream )
				{
					tv.tv_sec = wait_wan_timeout;
					ses->wait_discover_timeout -= wait_wan_timeout;
				}
				else
				{
					if ( ses->wait_discover_timeout > 0 )
					{
						tv.tv_sec = ses->wait_discover_timeout;
						ses->wait_discover_timeout = 0;
					}
					else
						tv.tv_sec = 1;
				}
				...
				tv.tv_sec = discover_timeout[max_wait_position];
		}
		else
			tv.tv_sec = discover_timeout[ses->retransmits];

		if ( !(isDemand() && ses->retransmits >= max_count
			&& discover_continue) )
			++ses->retransmits;
#else
#if defined(TCSUPPORT_CT_JOYME) || defined(TCSUPPORT_CT_JOYME2)
         ...
         if(wait_position_yh < max_coun_yh) {
			tv.tv_sec = discover_timeout_yh[wait_position_yh];
			wait_position_yh++;
		 }
		 else {
			tv.tv_sec = discover_timeout_yh[max_coun_yh-1];
		 }
#else
#if defined(TCSUPPORT_CT_PON_CZ_GD)
		tv.tv_sec = 1 << (ses->retransmits%3 + 1);
		++ses->retransmits;
#else
	    ++ses->retransmits;
	...
	if( ret == 0 ){
		...
		if( ret != 0 )
			...
			if ( isDemand() && ses->retransmits >= max_count )
			{
				...
				{
					if ( 0 == detectUpStream )
						detectUpStream = checkUptream( &ses->wait_discover_timeout );

					if ( detectUpStream )
						tcdbg_printf("\n ppp ondemand: detect upstream \n");

					if ( 0 == detectUpStream
						&& ses->wait_discover_timeout <= 0 )
					{
						break;  // restart the ppp loop when there is upstream.
					}

					discover_continue = 1;
					continue;
				}
				else if ( 0 == detectUpStream )
				{
					break; // restart the ppp loop when there is upstream.
				}
