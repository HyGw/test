/*
 *   $Id: //BBN_Linux/Branch/Branch_for_Rel_CMCC_7526_20161014/tclinux_phoenix/apps/public/radvd-1.5/send.c#1 $
 *
 *   Authors:
...
send_ra(int sock, struct Interface *iface, struct in6_addr *dest)
{
	...
	while(prefix)
	{
		if( prefix->enabled )
		{
			...
			if(radvd_exiting == 1){
				pinfo->nd_opt_pi_valid_time	= htonl(0);
