...
json_t *
json_new_child (json_type_e type, const char *text_value)
{
	json_t *value = NULL;
	int length = 0;
	...
	if(text_value){
		length = strlen(text_value) + 1;
		value->text = malloc (length * sizeof (char));
		if (value->text == NULL)
		{
			if(value) json_free_value(&value);
		}
		strcpy(value->text, text_value);	
	}
...
json_err_e
json_copy_label (json_t *root, json_t *srcparent)
{
	...
	label = json_new_string(srcparent->text);
	if (label == NULL)
		return JSON_MEMORY;

	value = json_new_child(srcparent->child->type, srcparent->child->text);
	if(NULL == value)
		goto err;
	...
	value = NULL;
	/*insert value and check for error */
	error = json_insert_child (root, label);
	if (error != JSON_OK)
		goto err;
...
enum json_error
json_tree_to_buffer (json_t *root, rcstring **prcbuf){
	json_t *cursor;
	rcstring *output;
...
json_tree_to_string (json_t * root, char **text)
{
	...
	*text = NULL;
	ret = json_tree_to_buffer(root, &output);
	if(ret == JSON_OK){
		*text = rcs_unwrap (output);
	}
	return ret;
...
char *
json_find_first_value (const json_t * object, const char *text_label)
{
	json_t *cursor;
...
json_t *
json_find_label (const json_t * object, const char *text_label)
{
	json_t *cursor,*cursorchild;
	...
	for (cursor = object->child; cursor != NULL; cursor = cursor->next)
	{
		//printf("find label is %s, %d\n",cursor->text, cursor->type);
		if (strcmp (cursor->text, text_label) == 0)
			return cursor;
		if(cursor->child){
			if(cursorchild = json_find_label(cursor->child,text_label))
				return cursorchild;
		}
	}
...
char *
json_find_value (const json_t * object, const char *parameter)
{
	json_t *cursori, *cursorj;
	char *value = NULL;
	...
	for (cursori = object->child; cursori != NULL; cursori = cursori->next)
	{
		//printf("\nfind text is %s, %d\n",cursori->text, object->type);

		if (strcmp (cursori->text, parameter) == 0){
			return cursori->child->text;			
		}
		else if(cursori->child){
			//printf("\nread child text is %s, %d\n",cursori->child->text, object->type);
			if((value = json_find_value (cursori->child, parameter))!=NULL){
				return value;
			}
		}
