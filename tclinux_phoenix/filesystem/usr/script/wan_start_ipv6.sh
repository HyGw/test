	...
	if [ "$TCSUPPORT_MULTISERVICE_ON_WAN" != "" ] && [ "$TCSUPPORT_WAN_PTM" != "" -o "$TCSUPPORT_WAN_ETHER" != "" ] && [ "$isPTMETHER" = "1" ]; then
		...
		if [ "$MSS" = "0" ] || [ "$MSS" = "" ] ; then
			if [ "$TCSUPPORT_FON_V2" != "" ] ; then
				iptables -I FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
			else
			iptables -A FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
			fi
		else
			if [ "$TCSUPPORT_FON_V2" != "" ] ; then
				iptables -I FORWARD -o ppp$i -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS
		else
