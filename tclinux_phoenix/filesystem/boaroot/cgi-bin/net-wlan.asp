...
<% end if %>
		...
		if (curUserName != sptUserName)
			getElement("11NgiControl").style.display = "none";
                  ...
                  <TABLE cellSpacing=0 cellPadding=0 border=0>
                    ...
                    <TR>
                      ...
                      <TD>
					  	<span id="inpsw"><INPUT style="FONT-FAMILY: '宋体'" type=password maxLength=65 name=wlWpaPsk value='<%if tcWebApi_get("WLan_Entry","WPAPSK","h") <> "N/A" then tcWebApi_get("WLan_Entry","WPAPSK","s") end if%>'></span>
			...
			<% if tcwebApi_get("WebCustom_Entry","isMultiSupported","h") = "Yes" then %>	
			<input name="btnStartWPS" value="<%IF tcWebApi_get("Info_WLan","wlanWPStimerRunning_0","h") = "0" then asp_Write("Start WPS") end if%> <%IF tcWebApi_get("Info_WLan","wlanWPStimerRunning_0","h") = "1" then asp_Write("Stop WPS") end if%><%IF tcWebApi_get("Info_WLan","wlanWPStimerRunning_0","h") = "N/A" then asp_Write("Start WPS") end if%>" onClick="doStartWPS();" type="button"> 
			<% else %>
