...
#if 0
		if (stat & MANTIS_UART_RXFIFO_EMPTY) {
			dprintk(verbose, MANTIS_ERROR, 1, "RX Fifo Empty");
			return -EIO;
		}

		if (!(stat & MANTIS_UART_RXFIFO_DATA)) {
			dprintk(verbose, MANTIS_ERROR, 1, "RX Fifo No Data");
			return -EIO;
		}

		if (stat & MANTIS_UART_FRAME_ERR) {
			dprintk(verbose, MANTIS_ERROR, 1, "UART Framing error");
			return -EIO;
		}

		if (stat & MANTIS_UART_PARITY_ERR) {
...
#else
	INIT_WORK(&mantis->uart_work, mantis_uart_work, mantis);
#endif
