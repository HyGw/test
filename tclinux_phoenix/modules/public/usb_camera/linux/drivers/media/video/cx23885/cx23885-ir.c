...
{
#else
void cx23885_ir_rx_work_handler(void *arg)
{
...
{
#else
void cx23885_ir_tx_work_handler(void *arg)
{
	struct work_struct *work = arg;
#endif
	struct cx23885_dev *dev =
...
#if 0
	/*
	 * FIXME - push more data to the IR subdevice's Tx FIFO, if we have any
	 * more to send.  Maybe something like this....
	 */
	int ret = 0;
	u8 buf[512];
	size_t count;
	...
	/* Fetch more ir blasting data from somewhere: fill buf and set count */
	/* Shovel it over to the subdev */
	ret = v4l2_subdev_call(sd, ir, tx_write, buf, count, num);
	/* Do something if ret < 0 */
	/* Do something if num != count: i.e. all the data was not written */
#endif
}
