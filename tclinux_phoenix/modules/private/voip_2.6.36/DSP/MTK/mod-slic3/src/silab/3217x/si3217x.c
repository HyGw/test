/*
** Si3217x.c
** Si32176 function table interface for fxs
*/
...
DEFINE_SPINLOCK(silabs_spi_if_lock_si3217x); //dyma_131230
int silabs_spi_if_flags_si3217x;  //dyma_131230
...
typedef enum {
	...
	SILAB_CHIP_TYPE_SI32171 = 1,
	SILAB_CHIP_TYPE_SI32172 = 2,
	//SILAB_CHIP_TYPE_SI32172 = 3,  // SI32173 is not  defined in Si_voice.h, so skip num 3, but it is defined in AN340-for_C_Rev1_1.pdf
	SILAB_CHIP_TYPE_SI32174 = 4,  // SI32173 is defined in Si_voice.h, but it is not defined in AN340-for_C_Rev1_1.pdf
	SILAB_CHIP_TYPE_SI32175 = 5,
	SILAB_CHIP_TYPE_SI32176 = 6,
	SILAB_CHIP_TYPE_SI32177 = 7,
	SILAB_CHIP_TYPE_SI32178 = 8,
	SILAB_CHIP_TYPE_SI32179 = 9
} siChipType_e;
...
/* FXS on device 0*/
//#define FXS_LINE    0
#define SI32176_CHAN_NUM 1	/* 1 FXS */
#define SI32176_FXS_NUM	1
#define SI32176_FXO_NUM	0
...
static int  si32176NumProslic = 0;
static int  si32176NumChannel = 0;
static int  si32176deviceId = 0;
...
static chanPvt_t chanPvt[SI32176_CHAN_NUM];
static chanState *ports = NULL;
static slicPresetParam_t slicPreset;
atomic_t si3217x_slic_init_flag = ATOMIC_INIT(0);
...
slicFuncTbl si32176Func[] = {
	si32176_deviceVerify,
	si32176_deviceInit,
	si32176_shutdownHw,
	si32176_paramReset,
	si32176_phoneRingStart,
	si32176_phoneRingStop,
	si32176_eventProcess,
	si32176_fxsSetLineState,
	si32176_fxsGetLineState,
	si32176_fxsSetBatteryPol,
	si32176_fxsGetBatteryPol,
	si32176_fxsGetHookState,
	si32176_ringState,
	si32176_fxsGetPulseStatus,
	si32176_fxsSetGainLv,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	si32176_dumpregister,
	slicFunc_dummy,
	si32176_LtTestStart,
	si32176_getLtTestResult, //Roger_slicon_lt
	slicFunc_dummy, //dyma_20140428 metering
	si32176_SetLoopback,	//FXS_SET_LOOPBACK
	si32176_SetRingParam,
	si32176_SetDcFeedParam,	/* added by mtk08176_20150627  */
	si32176_PowerSaveMode,
};
...
static void silab_writeSi32176DirectSpi(vint channel, uint8 regAddr, uint8 writeData);
static uint8 silab_readSi32176DirectSpi(vint  channel, uint8 regAddr);
static void silab_writeSi32176IndirectSpi(vint channel, uint16 iAddr, uint32 data32);
static uint32 silab_readSi32176IndirectSpi(vint channel, uint16 iAddr);

static uint8 silab_bitReverse(uint8 fiveBits);
static void silab_dumpReg_si3217x(uint8 chan);

int si32176_deviceVerify(char* buf){
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "* Si32176 * deviceVerify ProSLIC_Version %s\r\n\r\n",ProSLIC_Version());
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "=================================\n");
	for(i = SI3217X_COM_REG_ID; i <= SI3217X_COM_REG_TEST_MODE; i++) {
	...
	if(SI3217X_REVC!=reversion){
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_ERROR, "ERROR! Channel %d Reversion %d conflict with ZyXEL patch file\n", channel, reversion);
		return _TDI_FAIL;
	}
	
	if (SI3217X_PART_NUM_SI32176!= part_num) {
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "si32176_deviceVerify Success!\r\n\r\n");

	devInfo->deviceNum += 1;
	devInfo->fxsNum += SI32176_FXS_NUM;
	devInfo->fxoNum += SI32176_FXO_NUM;
	devInfo->slicType = SILICON_32176;
	devInfo->pcm1ChNum += SI32176_FXS_NUM+SI32176_FXO_NUM;
	devInfo->pcm2ChNum += 0;

	return _TDI_SUCCESS;
}

int si32176_deviceInit(char* buf)
{
	...
	uint8	retValue = 0;
	uint8	data;
	int numTicFxs = SI32176_CHAN_NUM;
	int init_ret=0;
	unsigned char devCnt=0;

	if(pinitSetting == NULL) {
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_ERROR, "ERROR! si32176_deviceInit pinitSetting NULL\n");
		return _TDI_FAIL;
	}
	
    devCnt = pinitSetting->chan;

	si32176deviceId = pinitSetting->chan;	
	memset(chanPvt, 0, sizeof(chanPvt_t)*(SI32176_CHAN_NUM));
	/*
	...
	*/
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "\r\nsi32176NumProslic = %d\n", si32176NumProslic);
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "si32176NumChannel = %d\n", si32176NumChannel);
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "si32176deviceId = %d\n", si32176deviceId);
	...
	*/
	if(NULL == ProHWIntf)
	{
		...
		*/
		for (index = 0; index < si32176NumProslic; index++) 
		{
		...
		*/
		for (index= 0; index < si32176NumChannel; index++) {
			...
			chanPvt[index].chipType = SILAB_CHIP_TYPE_SI32176;
	...
	** dc-dc powerup, etc.)
	*/
	if (ProSLIC_Init(arrayOfProslicChans,si32176NumChannel)) {
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "%s %d initialization in Silabs Si3217x failed \n",__FILE__,__LINE__);
	}
	...
	*/
	ProSLIC_LBCal(arrayOfProslicChans,si32176NumChannel);
	...
	*/
	index = 0;
	slot = si32176deviceId * 16;

	si32176_paramReset(buf);
	...
	for(index = 0; index < si32176NumChannel; index++)
	{
		...
		ProSLIC_LoadRegTables(ports[index].ProObj, NULL, regTable, 1);  // si32176NumChannel);
		...
		data = silab_ctrl_ReadRegisterWrapper(NULL,index, SI3217X_COM_REG_PCMMODE);
		data = (data & ~(0x3)) | SILAB_PCM_LINEARMODE;	 /* PCM_FMT[1:0] */
		silab_ctrl_WriteRegisterWrapper(NULL,index, SI3217X_COM_REG_PCMMODE, data);

		/* Set DTX data to be driven on positive edge of PCLK */
		data = silab_ctrl_ReadRegisterWrapper(NULL,index, SI3217X_COM_REG_PCMTXHI);
		silab_ctrl_WriteRegisterWrapper(NULL,index, SI3217X_COM_REG_PCMTXHI, (data & ~SILAB_PCM_TX_EDGE));
		...
		ProSLIC_AudioGainSetup(ports[index].ProObj,pinitSetting->listenSlicVal,pinitSetting->speakSlicVal,slicPreset.impCountryIdx);  //dyma_131212
		//silab_writeSi32176IndirectSpi(index, 544, 0x8000000);  /* TX AGC Gain */ // unit gain = 0x8000000   //dyma_131212
		//silab_writeSi32176IndirectSpi(index, 545, 0x1009B9C);  /* RX AGC Gain */ // unit gain = 0x2000000   //dyma_131212
		slot+=16;//xflu add for channel 1 no data 20131022
	}
	atomic_set(&si3217x_slic_init_flag,1);
	/*
...
int si32176_paramReset(char* buf)
{
	_TDI_initSetting_t *pinitSetting = (_TDI_initSetting_t *)buf;
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "___ COUNTRY SETTING ___\r\nring_idx : %d\r\n imp_idx : %d\r\n\r\n",slicPreset.ringCountryIdx,slicPreset.impCountryIdx);
	return _TDI_SUCCESS;
}

int si32176_phoneRingStart(char* buf)
{
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "%s", __func__);
	return _TDI_SUCCESS;
}

int si32176_phoneRingStop(char* buf)
{
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "%s", __func__);
	return _TDI_SUCCESS;
}

int si32176_eventProcess(char* buf)
{
	return _TDI_SUCCESS;
}

//Roger_silicon_lt
int si32176_LtTestStart(char* buf) {

	uint8 i,index;
	uint8 backup_register[SI3217X_COM_REG_TEST_MODE+1];
	uint8 testresult=0;
	_TDI_devicelinetest_t* linetest=(_TDI_devicelinetest_t*)buf;
	
	//return if slic is at init state
	if(atomic_read(&si3217x_slic_init_flag)==0)
		return _TDI_FAIL;

	if(linetest->chan<si32176NumChannel)
	{
...
int* si32176_getLtTestResult(void)
{
	return Silicon_getLtTestResult();

}

int si32176_dumpregister(char* buf)
{
	char chanCnt = *buf;
	silab_dumpReg_si3217x(chanCnt);
	return _TDI_SUCCESS;
}

int si32176_shutdownHw(char* buf)
{
	silab_free();
	return _TDI_SUCCESS;
}

int si32176_fxsSetLineState(char* buf)
{
	...
	if(pre_state[plineInfo->chan]!=plineInfo->state){
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si32176_fxsSetLine[%d](%d) state 0x%x ",
		plineInfo->chan,
		chanPvt[plineInfo->chan].chanType,
		plineInfo->state);
	}
	#endif
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"_%d_si32176_fxsSetLine[%d]state 0x%x ",
	...
	case TIC_STATE_REV_OHT:
        ...
        default:
            SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si32176_State UNKNOWN !\n");
...
int si32176_fxsGetLineState(char* buf)
{
	vint temp;
	_TDI_lineInfo_t* plineInfo = (_TDI_lineInfo_t*)buf;
...
//	 printk("%s : retTicStat=%d \n",__func__,plineInfo->retTicStat);
	if(plineInfo->retTicStat!=pre_stateType[plineInfo->chan]){
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"_%d_si32176_fxsGetLine(%d)Ret[%d]=State[%d]\n",
		fxs_intr_cnt, //dyma_131227
		plineInfo->chan,
		temp,
...
int si32176_fxsGetBatteryPol(char* buf)
{
	vint polarity;
...
int si32176_fxsSetBatteryPol(char* buf)
{
	_TDI_batteryPolInfo_t* pBatteryPolInfo = (_TDI_batteryPolInfo_t* )buf;
...
int si32176_fxsGetHookState(char* buf)
{
	uint8 hook;
   ...
   /*
     ...
     */
    if(ports != NULL && atomic_read(&si3217x_slic_init_flag) ==1)
	...
	{
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si32176_fxsGetChan [%d] HookState [%d]\n",pHookStat->chan,hook); 
...
int si32176_ringState(char* buf)
{
	int state;
    int count;
    int ret;
	...
	{
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si32176_ringState [%d] count[%d]\n",state,count); 
...
int si32176_fxsGetPulseStatus(char* buf)
{
	dbg_printf("%s", __func__);
	return _TDI_SUCCESS;
}

int si32176_fxsSetGainLv(char* buf)
{
...
int si32176_SetLoopback(char *buf)
{
	...
	if(loopbackSetting->chan >= SI32176_CHAN_NUM)
		return _TDI_FAIL;
...
/* added by mtk08176_20150627  */
int si32176_SetRingParam(char *buf)
{
	int i=0;
	int index = 0;
	...
	for(i=0;profileRingTable[i].profileID != 0;i++){
		if(profileRingTable[i].amplitudeSlab == ringParams->amplitudeSlab){			
			...
			SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "si32176 channel(%d) ring profile ID is %d,amplitude is %d(V)\n",index,profileRingTable[i].profileID,profileRingTable[i].amplitudeSlab);
			break;	
		}
	}
	if(profileRingTable[i].profileID == 0)
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_WARNING, "No  such profile of amplitudeSlab\n");	
}

int si32176_SetDcFeedParam(char *buf)
{
	int i=0;
	int index = 0;
	...
	for(i=0;profileDCFeedTable[i].profileID != 0;i++){
		if(profileDCFeedTable[i].ilaSlab == dcFeedParams->ilaSlab){	
			...
			SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "si32176 channel(%d) ring profile ID is %d,ilaSlab is %d(ma)\n",index,profileDCFeedTable[i].profileID,profileDCFeedTable[i].ilaSlab);
			break;	
		}
	}
	if(profileDCFeedTable[i].profileID == 0)
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_WARNING, "No  such profile of ilaSlab\n");	
}
...
static TIC_Return silab_alloc(int numTicFxs)
{
    ...
    /*
		numTicFxs = 1
		CHAN_PER_DEVICE = 1 (support 1 fxs chaanel on si32176)
		si32176NumProslic = 1 (1 slic)
		si32176NumChannel = 1 (2 fxs)
		(ProslicDeviceType **) ProSLICDevices : 1 
		(chanState *) ports : 1
		(proslicChanType_ptr *) arrayOfProslicChans: 1
	 */
    si32176NumProslic = numTicFxs/CHAN_PER_DEVICE;
    si32176NumChannel = numTicFxs;

    /* Allocate required resources (arrays) */
	ProSLICDevices = (ProslicDeviceType **) kmalloc(sizeof(ProslicDeviceType *)*si32176NumProslic, GFP_KERNEL);
    if (NULL == ProSLICDevices) {
        return (TIC_ERROR_NOMEM);
    }

	ports = (chanState *) kmalloc(sizeof(chanState)*si32176NumChannel, GFP_KERNEL);
    if (NULL == ports) {
		kfree(ProSLICDevices);
        return (TIC_ERROR_NOMEM);
    }
	
	arrayOfProslicChans = (proslicChanType_ptr *)kmalloc(sizeof(proslicChanType_ptr)*si32176NumChannel, GFP_KERNEL);
...
static void silab_free(
        void)
{
	atomic_set(&si3217x_slic_init_flag,0);	
...
static int silab_ctrl_WriteRegisterWrapper (void * hSpiGci, uInt8 channel,  uInt8 regAddr, uInt8 data)
{
	...
	spin_lock_irqsave(&silabs_spi_if_lock_si3217x, silabs_spi_if_flags_si3217x);  //dyma_131230
    	silab_writeSi32176DirectSpi(channel, regAddr, data);
	spin_unlock_irqrestore(&silabs_spi_if_lock_si3217x, silabs_spi_if_flags_si3217x);  //dyma_131230
	//up(&SPI_SEM);
    return 0;
}
...
static uInt8 silab_ctrl_ReadRegisterWrapper (void * hSpiGci, uInt8 channel,uInt8 regAddr)
{
	...
	//down(&SPI_SEM); //dyma_131230
	spin_lock_irqsave(&silabs_spi_if_lock_si3217x, silabs_spi_if_flags_si3217x);  //dyma_131230
	ret=silab_readSi32176DirectSpi(channel, regAddr);
	spin_unlock_irqrestore(&silabs_spi_if_lock_si3217x, silabs_spi_if_flags_si3217x);  //dyma_131230
	//up(&SPI_SEM);
    return ret;
}
...
static int silab_ctrl_WriteRAMWrapper (void * hSpiGci, uInt8 channel, uInt16 ramAddr, ramData data)
{
	//down(&SPI_SEM); //dyma_131230
	spin_lock_irqsave(&silabs_spi_if_lock_si3217x, silabs_spi_if_flags_si3217x);  //dyma_131230
    	silab_writeSi32176IndirectSpi(channel, ramAddr, data);
	spin_unlock_irqrestore(&silabs_spi_if_lock_si3217x, silabs_spi_if_flags_si3217x);  //dyma_131230
	//up(&SPI_SEM); 
    return 0;
}
...
static uInt32 silab_ctrl_ReadRAMWrapper (void * hSpiGci, uInt8 channel, uInt16 ramAddr)
{
	...
	//down(&SPI_SEM); //dyma_131230
	spin_lock_irqsave(&silabs_spi_if_lock_si3217x, silabs_spi_if_flags_si3217x);  //dyma_131230
	ret=silab_readSi32176IndirectSpi(channel, ramAddr);
	spin_unlock_irqrestore(&silabs_spi_if_lock_si3217x, silabs_spi_if_flags_si3217x);  //dyma_131230
	//up(&SPI_SEM);
...
/*
 * ======== silab_writeSi32176DirectSpi() ========
 * Write a byte on SPI to Si32176 direct register
 *
 ...
 * Return Value: none
 */
static void silab_writeSi32176DirectSpi(
    vint   channel,
    uint8  regAddr,
    uint8  writeData)
{
	...
	deviceId =  si32176deviceId + _TDI_SLIC_SPI_ID; 
    /*
	...
	#if 0
	if(regAddr==SI3217X_COM_REG_LINEFEED){    //dyma_131227
...
/*
 * ======== silab_readSi32176DirectSpi() ========
 * Receive a byte on SPI from Si32176 direct register
 *
 * Input Values: Channel Num.
                 Address of register to read
 * Return Value: 1 byte of data received.
 */
static uint8 silab_readSi32176DirectSpi(
    vint  channel,
    uint8 regAddr)
{
    uint8  retVal;
...
/*
 * ======== silab_writeSi32176IndirectSpi() ========
 * Write a byte on SPI to Si32176 RAM
 *
 * Input Value: Channel Num.
                Address to write to
				1 byte of data to write
 * Return Value: none
 */
static void silab_writeSi32176IndirectSpi(
    vint    channel,
    uint16  iAddr,
    uint32  data32)
{
    ...
    while (0 != (status = (silab_readSi32176DirectSpi(channel, 4) & 0x01) ));

    /* Write the high part of address */
    silab_writeSi32176DirectSpi(channel, 5, addrHi);

    /* Write data to ram */
    data = (uint8)((data32 << 3) & 0xFF);
    silab_writeSi32176DirectSpi(channel, 6, data);

    data = (uint8)((data32 >> 5) & 0xFF);
    silab_writeSi32176DirectSpi(channel, 7, data);

    data = (uint8)((data32 >> 13) & 0xFF);
    silab_writeSi32176DirectSpi(channel, 8, data);

    data = (uint8)((data32 >> 21) & 0xFF);
    silab_writeSi32176DirectSpi(channel, 9, data);

    /* Write the low part of address */
    silab_writeSi32176DirectSpi(channel, 10, addrLo);
...
/*
 * ======== silab_readSi32176IndirectSpi() ========
 * Read a byte on SPI from Si32176 RAM
 *
 * Input Value: Channel Num.
                Address to write to
				1 byte of data to write
 ...
 */
static uint32 silab_readSi32176IndirectSpi(
    vint   channel,
    uint16 iAddr)
{
    volatile uint8 status;
    ...
    /* Read bit 4 ~ 0 */
    retVal = (silab_readSi32176DirectSpi(channel, 6) >> 3) & 0xFF;

    /* Read bit 12 ~ 5 */
    data = (uint32)silab_readSi32176DirectSpi(channel, 7);
    retVal = retVal | (data << 5);
        
    /* Read bit 20 ~ 13 */
    data = (uint32)silab_readSi32176DirectSpi(channel, 8);
    retVal = retVal | (data << 13);
        
    /* Read bit 28 ~ 21 */
    data = (uint32)silab_readSi32176DirectSpi(channel, 9);
...
static void silab_dumpReg_si3217x(uint8 chan)
{
	...
	printk("=================================\n");
	for(i = SI3217X_COM_REG_ID; i <= SI3217X_COM_REG_TEST_MODE; i++) {
		value = silab_readSi32176DirectSpi(chan, i);
		if((i%10) == 0) {
			printk("%02d:", i);
		}
		printk(" %02x", value);
...
int si32176_PowerSaveMode(int enable)
{
