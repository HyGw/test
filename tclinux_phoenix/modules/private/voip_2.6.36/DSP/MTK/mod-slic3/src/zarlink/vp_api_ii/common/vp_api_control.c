...
extern int downSlicGainFlag;
extern _TDI_gainSetting_t lastGainSetting[DSP_MAX_CHAN];  /*upgrade_to_4_channel*/
#endif
...
#ifdef FAXLAB_DET
	if(downSlicGainFlag==0){
		int channelID=-1;
		Vp890DeviceObjectType *pDevObj=pLineCtx->pDevCtx->pDevObj;
		channelID=pDevObj->deviceId;
		lastGainSetting[channelID].chan=channelID;
		lastGainSetting[channelID].speakSlicVal=txLevel;
		lastGainSetting[channelID].listenSlicVal=rxLevel;
	}
#endif
