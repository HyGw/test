...
static VpLineCtxType           lineCtx_ptr[LE9662_CHAN_NUM];
static Vp886LineObjectType     lineObj_ptr[LE9662_CHAN_NUM];
VpEventType   le9662legerityEventObj; // a temp var need not duplicate  [ZL88601_CHAN_NUM]; // 1=>2
le9662LineStat_t le9662LineStat[LE9662_CHAN_NUM]; // 1=>2

int le9662_Init_Cmp = _TDI_FALSE;
int le9662_Init_Line_Cmp[LE9662_CHAN_NUM]= {_TDI_FALSE,_TDI_FALSE};
int le9662_Get_Mask_Cmp = _TDI_FALSE;
int le9662_Line_Cal_Cmp[LE9662_CHAN_NUM]= {_TDI_FALSE,_TDI_FALSE};
...
unsigned int le9662_event_process_flags; //dyma_20140218

/*20110714 PTChen add for slic country*/
const VpProfileDataType *DEVICE_PRO_le9662;
const VpProfileDataType *AC_PRO_FXS_le9662;
const VpProfileDataType *DC_PRO_FXS_le9662;
const VpProfileDataType *RINGING_PRO_FXS_le9662;
const VpProfileDataType *AC_PRO_FXO_le9662;
const VpProfileDataType *CONFIG_PRO_FXO_le9662;
const VpProfileDataType *RINGING_CADENCE_PRO_le9662;
const VpProfileDataType *CALL_ID_PRO_le9662;
const VpProfileDataType *LE9662_METERING_PRO; //dyma_20140218
...
slicFuncTbl le9662Func[]={
		le9662_deviceVerify,
		le9662_deviceInit,
		le9662_shutdownHw,
		le9662_paramReset,
		le9662_phoneRingStart,
		le9662_phoneRingStop,
		le9662_eventProcess,
		le9662_fxsSetLineState,
		le9662_fxsGetLineState,
		le9662_fxsSetBatteryPol,
		le9662_fxsGetBatteryPol,
		le9662_fxsGetHookState,
		le9662_ringState,
		le9662_fxsGetPulseStatus,
		le9662_fxsSetGainLv,
		...
		le9662_dumpregister,
		slicFunc_dummy,
		le9662_LtTestStart,//Roger_lt
		le9662_getLtTestResult,//Roger_lt
		le9662_fxsSetMetering,  //dyma_20140428 metering
		le9662_SetLoopback,
		le9662_SetRingParam,
		le9662_SetDcFeedParam
};
...
hwProfList_t le9662_country_mapping[] =
{
...
*
*******************************************************************************/
void le9662_slicParamReset(int idx){
	/*====== decide profile according to Country code =====*/
	/*Reset by default table*/
	DEVICE_PRO_le9662 = LE9662_DEV_PROFILE_47UH;
	AC_PRO_FXS_le9662 = AC_FXS_VE960_RF14_600R_DEF;
	DC_PRO_FXS_le9662 = DC_FXS_VE960_ABS100V_DEF;
	RINGING_PRO_FXS_le9662 = RING_VE960_ABS100V_DEF;
	AC_PRO_FXO_le9662 = VP_PTABLE_NULL;
	CONFIG_PRO_FXO_le9662 = VP_PTABLE_NULL;
	CALL_ID_PRO_le9662 = VP_PTABLE_NULL;
	LE9662_METERING_PRO = METER_LE9662_12KHZ; //dyma_20140428

	if((idx >= C_DEF) && (idx < C_NUM)){
		DEVICE_PRO_le9662 = le9662_country_mapping[idx].pDevice;
		AC_PRO_FXS_le9662 = le9662_country_mapping[idx].pAcFXS;
		DC_PRO_FXS_le9662 = le9662_country_mapping[idx].pDcFXS;
		RINGING_PRO_FXS_le9662 = le9662_country_mapping[idx].pRingingFXS;
		AC_PRO_FXO_le9662 = le9662_country_mapping[idx].pAcFXO;
		CONFIG_PRO_FXO_le9662 = le9662_country_mapping[idx].pConfigFXO;
		CALL_ID_PRO_le9662 = le9662_country_mapping[idx].pCallID;
		LE9662_METERING_PRO = le9662_country_mapping[idx].pMetering; //dyma_20140428 
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "%s Successful(CountryCode=%s)\n",__func__,\
				le9662_country_mapping[idx].name);
	}
	else{
 ...
 */
int le9662_paramReset(char* buf){
//	int country = *(int *)buf;
	uint8 i=0;
...
/*_____________________________________________________________________________
 **      function name: le9662_deviceVerify
 **      descriptions:
 **            driver verify
 **      parameters:
 **				void
...
int le9662_deviceVerify(char* buf){
	...
	{
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_ERROR, "le9662_deviceVerify already! Bypass 2nd verify!\n");
		return _TDI_FAIL;
	}
	
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "le9662_deviceVerify Start\r\n");
	
	while(((res_arr[0]!=0x8) ||(res_arr[1]!=0x75))&&((res_arr[0]!=0x2) ||(res_arr[1]!=0x73))){ /*le9642/le9662*/
		VpMpiCmd(devInfo->deviceNum, VP886_EC_1, VP886_R_RCNPCN_RD, VP886_R_RCNPCN_LEN, res_arr);
		
		RCNPCN_rd_cnt++;
		if(RCNPCN_rd_cnt>100)
		{
			SLIC_DBG_MSG(_SLIC_DBG_TYPE_ERROR, "le9662_deviceVerify RCNPCN mismatch!\n");
			return _TDI_FAIL;
		}
	}
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "devInfo->deviceNum=%d res_arr[0]=%x res_arr[0]=%x\n",devInfo->deviceNum,res_arr[0],res_arr[1]);

	le9662_DevPhyLocation = devInfo->deviceNum;
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "le9662_deviceVerify Success! \r\nrd_cnt=%d\r\n",RCNPCN_rd_cnt);
	...
	devInfo->deviceNum += 1;
	devInfo->fxsNum += LE9662_FXS_NUM;
	devInfo->fxoNum += LE9662_FXO_NUM;
	devInfo->slicType = ZARLINK_9642;
	devInfo->pcm1ChNum += LE9662_FXS_NUM+LE9662_FXO_NUM;
	devInfo->pcm2ChNum +=0;
	return _TDI_SUCCESS;
...
/*_____________________________________________________________________________
 **      function name: le9662_deviceInit
 **      descriptions:
 **            driver init
 **      parameters:
 **				void
...
int le9662_deviceInit(char* buf){
	int                  devCnt;
	int                  lineCnt;
    ...
    timeSlotIdx = devCnt;
        ...
        //dyma_20140218
	le9662_Init_Line_Cmp[0] = _TDI_FALSE;
	le9662_Init_Line_Cmp[1] = _TDI_FALSE;
	le9662_Line_Cal_Cmp[0]	 = _TDI_FALSE;
	le9662_Line_Cal_Cmp[1]	 = _TDI_FALSE;
	
	le9662_Init_Cmp = _TDI_FALSE;
	le9662_Init_Cnt = 0;
	...
	if(devCnt!=le9662_DevPhyLocation){
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_WARNING, "Bypass le9662_deviceInit on Device %d!\n",devCnt);  //dyma_130118
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "le9662_deviceInit %d\n",devCnt);  //dyma_130118 
	...
	for (lineCnt=0; lineCnt < LE9662_CHAN_NUM; lineCnt++) {
    ...
    le9662_slicParamReset(pinitSetting->hwProfId);
    status = VpInitDevice(&devCtx_ptr , DEVICE_PRO_le9662, AC_PRO_FXS_le9662, DC_PRO_FXS_le9662, RINGING_PRO_FXS_le9662, AC_PRO_FXO_le9662, CONFIG_PRO_FXO_le9662);
    if (VP_STATUS_SUCCESS != status) {
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "le9662 Init "); //dyma_20140903  

    while (le9662_Init_Cmp != _TDI_TRUE) {
		le9662_Init_Cnt++;
		...
		mdelay(10);
		le9662_eventProcess((char *)&devCnt);
    }
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "\r\nle9662_Init_Cnt = %d\n",le9662_Init_Cnt);

	if(le9662_Init_Cmp == FALSE)
	{
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_ERROR,"le9662_Init_Cnt=%d, TIMEOUT !!! \n",le9662_Init_Cnt);
		atomic_set(&le9662_init_ongoing,0); //dyma_20140218
		VpRegisterDump(&devCtx_ptr);
		return _TDI_FAIL;
	}
...
//	eventMask.fxo &= ~VP_LINE_EVID_POLREV;
   ...
   // VpSetOption(VP_NULL, &devCtx_ptr, VP_OPTION_ID_PULSE_MODE, &pulseMode);
    ...
    le9662LineStat[0].fxsHookStat = TIC_IMMEDIATE_ONHOOK;
	le9662LineStat[1].fxsHookStat = TIC_IMMEDIATE_ONHOOK;
	
	//dyma_20140428 metering
	for (lineCnt=0; lineCnt < LE9662_CHAN_NUM; lineCnt++) {
		VpInitMeter(&(lineCtx_ptr[lineCnt]),LE9662_METERING_PRO);
	}
...
/*_____________________________________________________________________________
 **      function name: le9662_fxsSetLineState
 **      descriptions:
 **            set ring state
 **      parameters:
 **				void
...
/*_____________________________________________________________________________
 **      function name: le9662_fxsGetLineState
 **      descriptions:
 **            set ring state
 **      parameters:
 **				void
...
/*_____________________________________________________________________________
 **      function name: le9662_fxsGetBatteryPol
 **      descriptions:
 **            set ring state
 **      parameters:
 **				void
...
/*_____________________________________________________________________________
 **      function name: le9662_fxsSetBatteryPol
 **      descriptions:
...
/*_____________________________________________________________________________
 **      function name: le9662_fxsGetHookState
 **      descriptions:
 **            get hook polarity
 **      parameters:
 **				void
...
int le9662_fxsGetHookState(char* buf){
	...
	//dbg_printf("%s",__func__);
	pHookStat->hookStat = le9662LineStat[pHookStat->chan].fxsHookStat;
	return _TDI_SUCCESS;
}

/*_____________________________________________________________________________
 **      function name: le9662_ringState
 **      descriptions:
 **            get ring state
 **      parameters:
 **				void
...
int le9662_fxsSetGainLv(char* buf){
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "%s - %x / %x \n",__func__,gainSetting->speakSlicVal,gainSetting->listenSlicVal);

	VpSetRelGain(&lineCtx_ptr[gainSetting->chan],gainSetting->speakSlicVal,gainSetting->listenSlicVal, 0);
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "%s - %x / %x \n",__func__,gainSetting->speakSlicVal,gainSetting->listenSlicVal);
...
int le9662_eventProcess(char *buf){
...
//	int chan=0;
	...
	if(devCnt!=le9662_DevPhyLocation){
		//printk("Bypass le9662_eventProcess on Device %d!\n",devCnt);  //dyma_130118
		return _TDI_SUCCESS;
	}

	spin_lock_irqsave(&le9662_event_process_lock, le9662_event_process_flags); //dyma_20140218
    ...
    if(VpGetEvent(legerityDev_ptr,&le9662legerityEventObj)){
		
		ret_event_process_chan_id = le9662legerityEventObj.channelId;
	
        //if ((TRUE == legerityEventStatus) && (VP_STATUS_SUCCESS == le9662legerityEventObj[devCnt].status)) {
        if ((VP_STATUS_SUCCESS == le9662legerityEventObj.status)
			&&(VP_STATUS_SUCCESS==le9662legerityEventObj.eventData)){
			
			SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"Chk[%d]:Dev[%d]chan[%d][%d]:st0=%d,ed0=%d: Cat=0x%x Id=0x%x\n",
			devCnt,
			le9662legerityEventObj.deviceId,
			le9662legerityEventObj.channelId,
			ret_event_process_chan_id,
			le9662legerityEventObj.status,
			le9662legerityEventObj.eventData,
			le9662legerityEventObj.eventCategory,
			le9662legerityEventObj.eventId); 
            ...
            if (VP_EVCAT_RESPONSE == le9662legerityEventObj.eventCategory) {
                switch (le9662legerityEventObj.eventId) {
                    case VP_DEV_EVID_DEV_INIT_CMP:
		                ...
		                for (lineNum=0; lineNum < LE9662_CHAN_NUM; lineNum++) {   //line init
					...
					case VP_EVID_CAL_CMP:
						...
						le9662_Line_Cal_Cmp[ret_event_process_chan_id] = _TDI_TRUE;
						SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"Calibration done on lineNum %d (%d/%d)\n", ret_event_process_chan_id,cmpCounter,LE9662_CHAN_NUM);
						if (cmpCounter == LE9662_CHAN_NUM) {
							/* Device Initialization is complete */
							le9662_Init_Cmp = _TDI_TRUE;
            ...
            else if (VP_EVCAT_SIGNALING == le9662legerityEventObj.eventCategory) {
                switch (le9662legerityEventObj.eventId) {
                    case VP_LINE_EVID_HOOK_OFF:
						SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"chan %d HOOK_OFF\n\n",ret_event_process_chan_id);
                    	le9662LineStat[ret_event_process_chan_id].fxsHookStat = TIC_IMMEDIATE_OFFHOOK;
                        break;
                    case VP_LINE_EVID_HOOK_ON:
						SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"chan %d HOOK_ON\n\n",ret_event_process_chan_id);
                    	le9662LineStat[ret_event_process_chan_id].fxsHookStat = TIC_IMMEDIATE_ONHOOK;
                        break;
                    default:
                        /* Do nothing */
                        break;
            ...
            else if(VP_EVCAT_FXO == le9662legerityEventObj.eventCategory){
				SLIC_DBG_MSG(_SLIC_DBG_TYPE_WARNING,"#le9662 not support FXO\n\n");
            }
        }
		else{
	...
	spin_unlock_irqrestore(&le9662_event_process_lock, le9662_event_process_flags); //dyma_20140218
	...
	if (_TDI_VP890_SIGREG_LIU & mpiData ) {     //off-hook state
		if (le9662LineStat.fxsHookStat != TIC_IMMEDIATE_OFFHOOK) {
			le9662LineStat.fxsHookStat  = TIC_IMMEDIATE_OFFHOOK;
		}
		else { }
	} else {                                    // on-hook state
		if (le9662LineStat.fxsHookStat != TIC_IMMEDIATE_ONHOOK) {
			le9662LineStat.fxsHookStat  = TIC_IMMEDIATE_ONHOOK;
		}
	}*/
...
//Roger_lt
int le9662_LtTestStart(char* buf) {
	_TDI_devicelinetest_t* linetest=(_TDI_devicelinetest_t*)buf;

	if(atomic_read(&le9662_init_ongoing))	//dyma_20140218
		return _TDI_FAIL;

	if(linetest->chan<LE9662_CHAN_NUM)	
...
int* le9662_getLtTestResult(void)
{
...
int le9662_SetRingParam(char *buf)
{
	...
	if(0 == ringParams->type){
		ret = VpConfigLine(&(lineCtx_ptr[ringParams->chanId]), VP_NULL, VP_NULL, RINGING_PRO_FXS_le9662);
	}
	else if(1 == ringParams->type){
		ret = VpConfigLine(&(lineCtx_ptr[ringParams->chanId]), VP_NULL, VP_NULL, RING_MISLIC_ABS100V_CHINA_TRAPE_9642);
	}
	else{
		return _TDI_FAIL;
	}
...
int le9662_fxoGetRingCadence(char* buf){
	event_printf("%s - %d\n",__func__,le9662LineStat.fxoRingStat);
	return le9662LineStat.fxoRingStat;
}
int le9662_fxoGetHookState(char* buf){
	dbg_printf("%s - %d \n",__func__,le9662LineStat.fxoHookStat);
    return le9662LineStat.fxoHookStat;
}
...
int le9662_fxoGetPSTNStats(char* buf){
	//printk("%s  \n",__func__);
	return le9662LineStat.fxoLineConn;
