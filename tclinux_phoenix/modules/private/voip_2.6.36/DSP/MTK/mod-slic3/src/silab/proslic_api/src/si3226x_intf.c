...
**
** $Id: si3226x_intf.c 5419 2016-01-13 00:40:56Z nizajerk $
**
** SI3226X_Intf.c
...
#define SI3226X_PRAM_VBATH_EXPECT_SAVE	796
...
int Si3226x_SetLinefeedStatus (proslicChanType *pProslic,uInt8 newLinefeed)
{
  ...
  else
  {
    WriteReg (pProHW,pProslic->channel,IRQEN1,
              Si3226x_General_Configuration.irqen1);
  }
  return 0;
}
                             ...
                             uInt8 step_delay,uInt8 step_num)
{
  ...
  if(ReadReg(pProHW,pProslic->channel,USERSTAT) & SIL_MWI_USTAT_SET)
  {
    vpk_mag = ReadRAM(pProHW,pProslic->channel,SI3226X_PRAM_VBATH_NEON);
    vbath_low = ReadRAM(pProHW,pProslic->channel,SI3226X_PRAM_VBATH_EXPECT_SAVE);

    if(flash_on)
    {
      vbath = ReadRAM(pProHW,pProslic->channel,SI3226X_PRAM_VBATH_EXPECT_SAVE);
