...
int le89116_FXS_Init_Cmp = _TDI_FALSE;
int le89116_Get_Mask_Cmp = _TDI_FALSE;
int le89116_Line_Cal_Cmp= _TDI_FALSE;
static atomic_t le89116_init_ongoing=ATOMIC_INIT(0);
...
int le89116_immediateHookState;

#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,31)
extern void VpSysSemaphoreInit(int semaphoredId);
#endif

slicFuncTbl le89116Func[]={
		le89116_deviceVerify,
		le89116_deviceInit,
		le89116_shutdownHw,
		le89116_paramReset,
		le89116_phoneRingStart,
		le89116_phoneRingStop,
		le89116_eventProcess,
		le89116_fxsSetLineState,
		le89116_fxsGetLineState,
		le89116_fxsSetBatteryPol,
		le89116_fxsGetBatteryPol,
		le89116_fxsGetHookState,
		le89116_ringState,
		le89116_fxsGetPulseStatus,
		le89116_fxsSetGainLv,
		slicFunc_dummy,
		slicFunc_dummy,
		slicFunc_dummy,
		slicFunc_dummy,
		slicFunc_dummy,
		slicFunc_dummy,
		le89116_dumpregister,
		slicFunc_dummy,
		le89116_LtTestStart,//Roger_lt
		le89116_getLtTestResult,//Roger_lt
		slicFunc_dummy,  //dyma_20140428 metering
		le89116_SetLoopback,  //FXS_SET_LOOPBACK
		le89116_SetRingParam,	//mtk07059_20150423
		le89116_SetDcFeedParam,	//mtk07059_20150616
};

/*20110714 PTChen add for slic country*/
const VpProfileDataType *DEVICE_PRO;
const VpProfileDataType *AC_PRO_FXS;
const VpProfileDataType *DC_PRO_FXS;
const VpProfileDataType *RINGING_PRO_FXS;
const VpProfileDataType *AC_PRO_FXO;
const VpProfileDataType *CONFIG_PRO_FXO;
const VpProfileDataType *RINGING_CADENCE_PRO;
const VpProfileDataType *CALL_ID_PRO;

/*20110714 PTChen add for slic country*/  //dyma_20140428 
hwProfList_t le89116_country_mapping[] =
{
...
*
*******************************************************************************/
void le89116_slicParamReset(int idx){
	/*====== decide profile according to Country code =====*/
	/*Reset by default table*/
	DEVICE_PRO = DEV_PROFILE_BuckBoost;
	AC_PRO_FXS = AC_FXS_RF50_600R_DEF;
	DC_PRO_FXS = DC_FXS_DEF;
	RINGING_PRO_FXS = RING_25HZ_DEF;
	AC_PRO_FXO = VP_PTABLE_NULL;
	CONFIG_PRO_FXO = VP_PTABLE_NULL;
	CALL_ID_PRO = VP_PTABLE_NULL;

	if((idx >= C_DEF) && (idx < C_NUM)){
		DEVICE_PRO = le89116_country_mapping[idx].pDevice;
		AC_PRO_FXS = le89116_country_mapping[idx].pAcFXS;
		DC_PRO_FXS = le89116_country_mapping[idx].pDcFXS;
		RINGING_PRO_FXS = le89116_country_mapping[idx].pRingingFXS;
		AC_PRO_FXO = le89116_country_mapping[idx].pAcFXO;
		CONFIG_PRO_FXO = le89116_country_mapping[idx].pConfigFXO;
		CALL_ID_PRO = le89116_country_mapping[idx].pCallID;
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "Successful(CountryCode=%s)\n",\
				le89116_country_mapping[idx].name);
	}
	else{
 ...
 */
int le89116_paramReset(char* buf){
//	int country = *(int *)buf;
	return _TDI_SUCCESS;
}

/*_____________________________________________________________________________
 **      function name: le89116_deviceVerify
 **      descriptions:
 **            driver verify
 **      parameters:
 **				void
...
int le89116_deviceVerify(char* buf){
	...
	devInfo->deviceNum += 1;
	devInfo->fxsNum += LE89116_FXS_NUM;
	devInfo->fxoNum += LE89116_FXO_NUM;
	devInfo->pcm1ChNum += LE89116_FXS_NUM+LE89116_FXO_NUM;
	devInfo->pcm2ChNum += 0;
	return _TDI_SUCCESS;
}

/*_____________________________________________________________________________
 **      function name: le89116_deviceInit
 **      descriptions:
 **            driver init
 **      parameters:
 **				void
...
int le89116_deviceInit(char* buf){
	int                  devCnt;
    //int                  lineCnt;
    VpStatusType          status;
	...
	le89116_Line_Cal_Cmp	 = _TDI_FALSE;
	le89116_FXS_Init_Cmp = _TDI_FALSE;

	atomic_set(&le89116_init_ongoing,1);
   ...
   /*      struct proc_dir_entry *slicRegister_proc=0;
		struct proc_dir_entry   *dir1=0;
		struct proc_dir_entry   *dir2=0;
			
		printk("Create slicRegister check proc \n");



		dir1 = proc_mkdir("fxs", NULL);
		dir2 = proc_mkdir("zalink", dir1);
        slicRegister_proc = create_proc_entry("slicRegister", 0, dir2);
        slicRegister_proc->read_proc = slic_register_read_proc;
        slicRegister_proc->write_proc = slic_register_write_proc;
		*/
        ...
        if (VP_STATUS_SUCCESS != VpMakeDeviceObject(VP_DEV_890_SERIES,
			...
			atomic_set(&le89116_init_ongoing,0);
            return _TDI_FAIL;
        }
		/*
		...
		le89116_slicParamReset(pinitSetting->hwProfId);
		status = VpInitDevice(&devCtx_ptr, DEVICE_PRO, AC_PRO_FXS, DC_PRO_FXS, RINGING_PRO_FXS, VP_PTABLE_NULL, VP_PTABLE_NULL);
	    ...
	    while (le89116_FXS_Init_Cmp != TRUE) {
	    	event_printf(">>1.5\n");
	        /*
	         * Wait for event indicating that the initialization is complete
	         */
			mdelay(10); /*SERENA_MODIFY*/
			le89116_eventProcess((char *)&devCnt);  /* Only run for one line per device */
	    }
	...
	eventMask.process &= ~(VP_LINE_EVID_SIGNAL_CMP);
		 ...
		 * Calibrate Lines
		 */
		/*le89116_Line_Cal_Cmp = FALSE;
		VpCalLine(&lineCtx_ptr);
		while (le89116_Line_Cal_Cmp != TRUE) {
         ...
         */
        le89116_immediateHookState = TIC_IMMEDIATE_ONHOOK;
		atomic_set(&le89116_init_ongoing,0);
    //}
    return _TDI_SUCCESS;
}
...
int le89116_shutdownHw(char* buf){
    int    lineCnt;
	if(atomic_read(&le89116_init_ongoing))
	{
		return _TDI_FAIL;
	}
...
/*_____________________________________________________________________________
 **      function name: le89116_fxsSetLineState
 **      descriptions:
 **            set ring state
 **      parameters:
 **				void
...
/*_____________________________________________________________________________
 **      function name: le89116_fxsGetLineState
 **      descriptions:
 **            set ring state
 **      parameters:
 **				void
...
/*_____________________________________________________________________________
 **      function name: le89116_fxsGetBatteryPol
 **      descriptions:
 **            set ring state
 **      parameters:
 **				void
...
/*_____________________________________________________________________________
 **      function name: le89116_fxsSetBatteryPol
 **      descriptions:
...
/*_____________________________________________________________________________
 **      function name: le89116_fxsGetHookState
 **      descriptions:
 **            get hook polarity
 **      parameters:
 **				void
...
int le89116_fxsGetHookState(char* buf){
	...
	event_printf("%s",__func__);
	pHookStat->hookStat = le89116_immediateHookState;
	return _TDI_SUCCESS;
}

/*_____________________________________________________________________________
 **      function name: le89116_ringState
 **      descriptions:
 **            get ring state
 **      parameters:
 **				void
...
int le89116_fxsSetGainLv(char* buf){
	...
	if(atomic_read(&le89116_init_ongoing))
		return _TDI_FAIL;
	VpSetRelGain(&lineCtx_ptr,gainSetting->speakSlicVal,gainSetting->listenSlicVal, 0);
	dbg_printf("%s %x/%x \n",__func__,gainSetting->speakSlicVal,gainSetting->listenSlicVal);
	return _TDI_SUCCESS;
}
int le89116_fxsGetGain(char* buf){
	if(atomic_read(&le89116_init_ongoing))
		return _TDI_FAIL;
	event_printf("%s",__func__);
	return _TDI_SUCCESS;
...
int le89116_eventProcess(char* buf){
            ...
            if (VP_EVCAT_RESPONSE == legerityEventObj.eventCategory) {
                ...
                switch (legerityEventObj.eventId) {
                    case VP_LINE_EVID_HOOK_OFF:
                    	...
                    	le89116_immediateHookState = TIC_IMMEDIATE_OFFHOOK;
                        break;
...
// 						break;
	...
	if (_TDI_VP890_SIGREG_LIU & mpiData ) {     //off-hook state
		if (le89116_immediateHookState != TIC_IMMEDIATE_OFFHOOK) {
			le89116_immediateHookState  = TIC_IMMEDIATE_OFFHOOK;
		}
		else { }
	} else {                                    // on-hook state
		if (le89116_immediateHookState != TIC_IMMEDIATE_ONHOOK) {
			le89116_immediateHookState  = TIC_IMMEDIATE_ONHOOK;
		}
	}*/
...
//Roger_lt
int le89116_LtTestStart(char* buf) {
	if(atomic_read(&le89116_init_ongoing))
		return _TDI_FAIL;
...
int* le89116_getLtTestResult(void)
{	
	if(atomic_read(&le89116_init_ongoing))
		return _TDI_FAIL;
...
int le89116_SetLoopback(char *buf)
{
	...
	if(atomic_read(&le89116_init_ongoing))	//dyma_20140218
		return _TDI_FAIL;
	if(loopbackSetting->chan >= LE89116_FXS_NUM)
		return _TDI_FAIL;
...
int le89116_SetRingParam(char* buf){
...
//	ringParams->type, ringParams->frequency, ringParams->amplitude, ringParams->dcBias);	
    ...
    if(0 == ringParams->type){		
        ret = VpConfigLine(&(lineCtx_ptr), VP_NULL, VP_NULL, RINGING_PRO_FXS);
    }
    else if(1 == ringParams->type){     
        ret = VpConfigLine(&(lineCtx_ptr), VP_NULL, VP_NULL, RING_25HZ_VE890_IB100V_TRAP);
    }   
    else{
        return _TDI_FAIL;   
    }   
...
int le89116_SetDcFeedParam(char* buf)
{
