    ...
    bool callback)
{
    if (!callback) {
        ...
        /* Cannot run if Ringing Power Management is currently adapting */
        if ((pDevObj->options.adaptiveRinging.power != VP_ADAPTIVE_RINGING_DISABLED) && 
            ...
            Vp886PushTestEvent(pLineCtx, VP_TEST_STATUS_LINE_NOT_READY);
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886TestPrepare: Cannot run test in ringing with RPM enabled"));
            return VP_STATUS_SUCCESS;
        }
    ...
    Vp886TestHeapType *pTestHeap)
{
    ...
    switch (pTestHeap->nextState) {
        ...
        case PREPARE_SETUP: {
            ...
            VpSlacRegRead(NULL, pLineCtx, VP886_R_SWCTRL_RD, VP886_R_SWCTRL_LEN, pTestHeap->scratch.swCtrl);
