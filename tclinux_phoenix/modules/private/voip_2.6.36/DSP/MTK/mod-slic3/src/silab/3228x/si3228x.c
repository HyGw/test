/*
** Si3228x.c
** Si3228x function table interface for fxs
*/
...
//#include "si3228x_revb_intf.h"
//#include "si3228x_revc_intf.h"
#include "si3228x_registers.h"
#include "si3228x_LCCB_constants.h"
#include "ddr_slic3.h"
...
DEFINE_SPINLOCK(silabs_spi_if_lock_si3228x); //dyma_131230
int silabs_spi_if_flags_si3228x;  //dyma_131230
...
typedef enum {
	...
	SILAB_CHIP_TYPE_SI32281 = 1,
	SILAB_CHIP_TYPE_SI32282 = 2,
	SILAB_CHIP_TYPE_SI32283 = 3,  // SI32173 is not  defined in Si_voice.h, so skip num 3, but it is defined in AN340-for_C_Rev1_1.pdf
	SILAB_CHIP_TYPE_SI32284 = 4,  // SI32173 is defined in Si_voice.h, but it is not defined in AN340-for_C_Rev1_1.pdf
	SILAB_CHIP_TYPE_SI32285 = 5,
	SILAB_CHIP_TYPE_SI32286 = 6,
	SILAB_CHIP_TYPE_SI32287 = 7,
	SILAB_CHIP_TYPE_SI32288 = 8,
	SILAB_CHIP_TYPE_SI32289 = 9
} siChipType_e;
...
/* FXS on device 0*/
//#define FXS_LINE    0
#define SI3228X_CHAN_NUM 2	/* 1 FXS */
#define SI3228X_FXS_NUM	2
#define SI3228X_FXO_NUM	0
...
static int  si3228xNumProslic = 0;
static int  si3228xNumChannel = 0;
static int  si3228xdeviceId = 0;
...
static chanPvt_t chanPvt[SI3228X_CHAN_NUM];
static chanState *ports = NULL;
static slicPresetParam_t slicPreset;
static uint8 si3228x_DevPhyLocation=0xff;
atomic_t si3228x_slic_init_flag = ATOMIC_INIT(0);
...
slicFuncTbl si3228xFunc[] = {
	si3228x_deviceVerify,
	si3228x_deviceInit,
	si3228x_shutdownHw,
	si3228x_paramReset,
	si3228x_phoneRingStart,
	si3228x_phoneRingStop,
	si3228x_eventProcess,
	si3228x_fxsSetLineState,
	si3228x_fxsGetLineState,
	si3228x_fxsSetBatteryPol,
	si3228x_fxsGetBatteryPol,
	si3228x_fxsGetHookState,
	si3228x_ringState,
	si3228x_fxsGetPulseStatus,
	si3228x_fxsSetGainLv,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	si3228x_dumpregister,
	slicFunc_dummy,
	si3228x_LtTestStart, // si3228x_LtTestStart,
	si3228x_getLtTestResult, // si3228x_getLtTestResult, //Roger_slicon_lt
	slicFunc_dummy, //dyma_20140428 metering
	si3228x_SetLoopback,	//FXS_SET_LOOPBACK
	si3228x_SetRingParam,
	si3228x_SetDcFeedParam,	/* added by mtk08176_20150627  */
	si3228x_PowerSaveMode,
};
...
static void silab_writeSi3228xDirectSpi(vint channel, uint8 regAddr, uint8 writeData);
static uint8 silab_readSi3228xDirectSpi(vint  channel, uint8 regAddr);
static void silab_writeSi3228xIndirectSpi(vint channel, uint16 iAddr, uint32 data32);
static uint32 silab_readSi3228xIndirectSpi(vint channel, uint16 iAddr);

static uint8 silab_bitReverse(uint8 fiveBits);
static void silab_dumpReg_si3228x(uint8 chan);
...
int si3228x_deviceVerify(char* buf){
	...
	if(si3228x_DevPhyLocation==0xff)
	{
		si3228x_DevPhyLocation = devInfo->deviceNum;
	}

	if(si3228x_DevPhyLocation!=devInfo->deviceNum)
	{
		printk("Bypass si3228x_deviceVerify on device %d\n", channel);
		return _TDI_FAIL;
	}

	printk("\r\n* Si3228x * deviceVerify ProSLIC_Version %s\r\n\r\n",ProSLIC_Version());
	...
	printk("Channel %d, Part_num 0x%x, Revision 0x%x\n", channel,part_num,reversion);
	

	#if 0
	for(i = SI3228X_RAM_IRNGNG_SENSE; i <= SI3228X_RAM_ANALOG3_TEST_MUX; i++) {
	...
	if(0x3!=reversion){
		printk("ERROR! Channel %d Reversion %d conflict\n", channel, reversion);
		return _TDI_FAIL;
	}
	
	if (0x3!=part_num && 0x2!=part_num){
		printk("ERROR! Channel %d Part number 0x%x\n", channel, part_num);
		return _TDI_FAIL;
	}
	...
	if(retValue!=0x40)
	{
		printk("AN340_4.2.2.2 Channel %d Verification Fail !\n",channel);
		return _TDI_FAIL;
	}
	if(0x2==part_num){
		printk("si32285_deviceVerify Success!\r\n\r\n");
	}else if(0x3==part_num){
		printk("si32287_deviceVerify Success!\r\n\r\n");
	}
		
	printk("si3228x_deviceVerify Success!\r\n\r\n");

	devInfo->deviceNum += 1;
	devInfo->fxsNum += SI3228X_FXS_NUM;
	devInfo->fxoNum += SI3228X_FXO_NUM;
	devInfo->slicType = SILICON_3228X;
	devInfo->pcm1ChNum += SI3228X_FXS_NUM+SI3228X_FXO_NUM;
	devInfo->pcm2ChNum += 0;
...
int si3228x_deviceInit(char* buf)
{
	...
	uint8	retValue = 0;
	uint8	data;
	int numTicFxs = SI3228X_CHAN_NUM;
	int init_ret=0;
	unsigned char devCnt=0;

	if(pinitSetting == NULL) {
		printk("ERROR! si3228x_deviceInit pinitSetting NULL\n");
		return _TDI_FAIL;
	}
	
    devCnt = pinitSetting->chan;
	if(devCnt!=si3228x_DevPhyLocation){
		printk("Bypass si3228x_deviceInit on Device %d!\r\n\r\n",devCnt);  //dyma_130118
		return _TDI_SUCCESS;
	}

	si3228xdeviceId = pinitSetting->chan;	
	memset(chanPvt, 0, sizeof(chanPvt_t)*(SI3228X_CHAN_NUM));
	/*
	...
	*/
	...
	printk("\r\nsi3228xNumProslic = %d\n", si3228xNumProslic);
	printk("si3228xNumChannel = %d\n", si3228xNumChannel);
	printk("si3228xdeviceId = %d\n", si3228xdeviceId);
	...
	*/
	if(NULL == ProHWIntf)
	{
		...
		*/
		for (index= 0; index < si3228xNumChannel; index++) {
			printk("ProSLIC_createChannel = %d\n", ProSLIC_createChannel(&(ports[index].ProObj)));
			printk("ProSLIC_SWInitChan = %d\n", ProSLIC_SWInitChan (ports[index].ProObj, index ,SI3228X_TYPE, ProSLICDevices[index/CHAN_PER_DEVICE], ProHWIntf));
			...
			chanPvt[index].chipType = SILAB_CHIP_TYPE_SI32283;
			printk("chanPvt[%d].chanType %d\n",index, chanPvt[index].chanType);
			printk("chanPvt[%d].chipType %d\n",index, chanPvt[index].chipType);
		}
	...
	*/
	...
	if (ProSLIC_Init(arrayOfProslicChans,si3228xNumChannel)) {
		printk("%s %d initialization in Silabs Si3228x failed \n",__FILE__,__LINE__);
	}
	...
	*/
	ProSLIC_LBCal(arrayOfProslicChans,si3228xNumChannel);
	...
	*/
	index = 0;
	slot = si3228xdeviceId * 16;

	si3228x_paramReset(buf);
	...
	for(index = 0; index < si3228xNumChannel; index++)
	{
		...
		ProSLIC_LoadRegTables(ports[index].ProObj, NULL, regTable, 1);  // si3228xNumChannel);
		...
		ProSLIC_AudioGainSetup(ports[index].ProObj,pinitSetting->listenSlicVal,pinitSetting->speakSlicVal,slicPreset.impCountryIdx);  //dyma_131212
		//silab_writeSi3228xIndirectSpi(index, 544, 0x8000000);  /* TX AGC Gain */ // unit gain = 0x8000000   //dyma_131212
		//silab_writeSi3228xIndirectSpi(index, 545, 0x1009B9C);  /* RX AGC Gain */ // unit gain = 0x2000000   //dyma_131212
		slot+=16;//xflu add for channel 1 no data 20131022
	}
	atomic_set(&si3228x_slic_init_flag,1);
...
int si3228x_LtTestStart(char* buf) {
	...
	uint8 i,index;
	uint8 backup_register[USERMODE_ENABLE+1];
	uint8 testresult=0;
	_TDI_devicelinetest_t* linetest=(_TDI_devicelinetest_t*)buf;
	
	//return if slic is at init state
	if(atomic_read(&si3228x_slic_init_flag)==0)
		return _TDI_FAIL;

	if(linetest->chan<si3228xNumChannel)
	{
		//backup slic register before line test
		for(i = ID; i <= USERMODE_ENABLE; i++) {
...
int si3228x_fxsSetLineState(char* buf)
{
	...
	if(pre_state[plineInfo->chan]!=plineInfo->state){
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si3228x_fxsSetLine[%d](%d) state 0x%x ",
		plineInfo->chan,
		chanPvt[plineInfo->chan].chanType,
		plineInfo->state);
	}
	#endif
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"_%d_si3228x_fxsSetLine[%d]state 0x%x ",
	...
	case TIC_STATE_REV_OHT:
        ...
        default:
            SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si3228x_State UNKNOWN !\n");
...
//	 printk("%s : retTicStat=%d \n",__func__,plineInfo->retTicStat);
	if(plineInfo->retTicStat!=pre_stateType[plineInfo->chan]){
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"_%d_si3228x_fxsGetLine(%d)Ret[%d]=State[%d]\n",
		fxs_intr_cnt, //dyma_131227
		plineInfo->chan,
		temp,
...
int si3228x_fxsGetHookState(char* buf)
{
   ...
   /*
     ...
     */
    if(ports != NULL && atomic_read(&si3228x_slic_init_flag) ==1)
	...
	{
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si3228x_fxsGetChan [%d] HookState [%d]\n",pHookStat->chan,hook); 
...
int si3228x_ringState(char* buf)
{
	...
	{
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si3228x_ringState [%d] count[%d]\n",state,count); 
...
int si3228x_SetLoopback(char *buf)
{
	...
	if(atomic_read(&si3228x_slic_init_flag)==0)
		return _TDI_FAIL;
	if(loopbackSetting->chan >= SI3228X_CHAN_NUM)
		return _TDI_FAIL;
...
int si3228x_SetRingParam(char *buf)
{
	...
	for(i=0;profileRingTable[i].profileID != 0;i++){
		if(profileRingTable[i].amplitudeSlab == ringParams->amplitudeSlab){			
			...
			printk("si3228x channel(%d) ring profile ID is %d,amplitude is %d(V)\n",index,profileRingTable[i].profileID,profileRingTable[i].amplitudeSlab);
			break;	
		}
	}
	if(profileRingTable[i].profileID == 0)
	printk("No  such profile of amplitudeSlab\n");	
}

int si3228x_SetDcFeedParam(char *buf)
{
	int i=0;
	int index = 0;
...
static TIC_Return silab_alloc(int numTicFxs)
{
    ...
    /*
		numTicFxs = 1
		CHAN_PER_DEVICE = 1 (support 1 fxs chaanel on si3228x)
		si3228xNumProslic = 1 (1 slic)
		si3228xNumChannel = 1 (2 fxs)
		(ProslicDeviceType **) ProSLICDevices : 1 
		(chanState *) ports : 1
		(proslicChanType_ptr *) arrayOfProslicChans: 1
	 */
    si3228xNumProslic = numTicFxs/CHAN_PER_DEVICE;
    si3228xNumChannel = numTicFxs;

    /* Allocate required resources (arrays) */
	ProSLICDevices = (ProslicDeviceType **) kmalloc(sizeof(ProslicDeviceType *)*si3228xNumProslic, GFP_KERNEL);
    if (NULL == ProSLICDevices) {
        return (TIC_ERROR_NOMEM);
    }

	ports = (chanState *) kmalloc(sizeof(chanState)*si3228xNumChannel, GFP_KERNEL);
    if (NULL == ports) {
		kfree(ProSLICDevices);
        return (TIC_ERROR_NOMEM);
    }
	
	arrayOfProslicChans = (proslicChanType_ptr *)kmalloc(sizeof(proslicChanType_ptr)*si3228xNumChannel, GFP_KERNEL);
...
static void silab_free(
        void)
{
	atomic_set(&si3228x_slic_init_flag,0);	
...
static int silab_ctrl_WriteRegisterWrapper (void * hSpiGci, uInt8 channel,  uInt8 regAddr, uInt8 data)
{
	...
	spin_lock_irqsave(&silabs_spi_if_lock_si3228x, silabs_spi_if_flags_si3228x);  //dyma_131230
    	silab_writeSi3228xDirectSpi(channel, regAddr, data);
	spin_unlock_irqrestore(&silabs_spi_if_lock_si3228x, silabs_spi_if_flags_si3228x);  //dyma_131230
	//up(&SPI_SEM);
    return 0;
}
...
static uInt8 silab_ctrl_ReadRegisterWrapper (void * hSpiGci, uInt8 channel,uInt8 regAddr)
{
	...
	//down(&SPI_SEM); //dyma_131230
	spin_lock_irqsave(&silabs_spi_if_lock_si3228x, silabs_spi_if_flags_si3228x);  //dyma_131230
	ret=silab_readSi3228xDirectSpi(channel, regAddr);
	spin_unlock_irqrestore(&silabs_spi_if_lock_si3228x, silabs_spi_if_flags_si3228x);  //dyma_131230
	//up(&SPI_SEM);
    return ret;
}
...
static int silab_ctrl_WriteRAMWrapper (void * hSpiGci, uInt8 channel, uInt16 ramAddr, ramData data)
{
	//down(&SPI_SEM); //dyma_131230
	spin_lock_irqsave(&silabs_spi_if_lock_si3228x, silabs_spi_if_flags_si3228x);  //dyma_131230
	silab_writeSi3228xIndirectSpi(channel, ramAddr, data);
	spin_unlock_irqrestore(&silabs_spi_if_lock_si3228x, silabs_spi_if_flags_si3228x);  //dyma_131230
	//up(&SPI_SEM); 
    return 0;
}
...
static uInt32 silab_ctrl_ReadRAMWrapper (void * hSpiGci, uInt8 channel, uInt16 ramAddr)
{
	...
	//down(&SPI_SEM); //dyma_131230
	spin_lock_irqsave(&silabs_spi_if_lock_si3228x, silabs_spi_if_flags_si3228x);  //dyma_131230
	ret=silab_readSi3228xIndirectSpi(channel, ramAddr);
	spin_unlock_irqrestore(&silabs_spi_if_lock_si3228x, silabs_spi_if_flags_si3228x);  //dyma_131230
	//up(&SPI_SEM);
...
/*
 * ======== silab_writeSi3228xDirectSpi() ========
 * Write a byte on SPI to Si3228x direct register
 *
 ...
 * Return Value: none
 */
static void silab_writeSi3228xDirectSpi(
    vint   channel,
    uint8  regAddr,
    uint8  writeData)
{
	...
	deviceId =  si3228xdeviceId + _TDI_SLIC_SPI_ID; 
    /*
...
/*
 * ======== silab_readSi3228xDirectSpi() ========
 * Receive a byte on SPI from Si3228x direct register
 *
 * Input Values: Channel Num.
                 Address of register to read
 * Return Value: 1 byte of data received.
 */
static uint8 silab_readSi3228xDirectSpi(
    vint  channel,
    uint8 regAddr)
{
    uint8  retVal;
...
/*
 * ======== silab_writeSi3228xIndirectSpi() ========
 * Write a byte on SPI to Si3228x RAM
 *
 * Input Value: Channel Num.
                Address to write to
				1 byte of data to write
 * Return Value: none
 */
static void silab_writeSi3228xIndirectSpi(  
    vint    channel,
    uint16  iAddr,
    uint32  data32)
{
    ...
    while (0 != (status = (silab_readSi3228xDirectSpi(channel, 4) & 0x01) ));

    /* Write the high part of address */
    silab_writeSi3228xDirectSpi(channel, 5, addrHi);

    /* Write data to ram */
    data = (uint8)((data32 << 3) & 0xFF);
    silab_writeSi3228xDirectSpi(channel, 6, data);

    data = (uint8)((data32 >> 5) & 0xFF);
    silab_writeSi3228xDirectSpi(channel, 7, data);

    data = (uint8)((data32 >> 13) & 0xFF);
    silab_writeSi3228xDirectSpi(channel, 8, data);

    data = (uint8)((data32 >> 21) & 0xFF);
    silab_writeSi3228xDirectSpi(channel, 9, data);

    /* Write the low part of address */
    silab_writeSi3228xDirectSpi(channel, 10, addrLo);
...
/*
 * ======== silab_readSi3228xIndirectSpi() ========
 * Read a byte on SPI from Si3228x RAM
 *
 * Input Value: Channel Num.
                Address to write to
				1 byte of data to write
 ...
 */
static uint32 silab_readSi3228xIndirectSpi(
    vint   channel,
    uint16 iAddr)
{
    volatile uint8 status;
    ...
    /* Read bit 4 ~ 0 */
    retVal = (silab_readSi3228xDirectSpi(channel, 6) >> 3) & 0xFF;

    /* Read bit 12 ~ 5 */
    data = (uint32)silab_readSi3228xDirectSpi(channel, 7);
    retVal = retVal | (data << 5);
        
    /* Read bit 20 ~ 13 */
    data = (uint32)silab_readSi3228xDirectSpi(channel, 8);
    retVal = retVal | (data << 13);
        
    /* Read bit 28 ~ 21 */
    data = (uint32)silab_readSi3228xDirectSpi(channel, 9);
...
static void silab_dumpReg_si3228x(uint8 chan)
{
	...
	printk("=================================\n");
	for(i = ID; i <= USERMODE_ENABLE; i++) {
		value = silab_readSi3228xDirectSpi(chan, i);
		if((i%10) == 0) {
			printk("%02d:", i);
		}
		printk(" %02x", value);
