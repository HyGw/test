...
int fxs_intr_cnt_1; //dyma_131227
int pre_fxs_intr_cnt_1; //dyma_131227
int write_LINEFEED_intr_cnt_1; //dyma_131227

DEFINE_SPINLOCK(silabs_spi_if_lock_1); //dyma_131230
int silabs_spi_if_flags_1; //dyma_131230
...
static int  si32260deviceId = 1;  //dyma_131216 evaluate default value
...
atomic_t slic_init_flag_1 = ATOMIC_INIT(0);
...
slicFuncTbl si32260Func_1[] = {
	si32260_1_deviceVerify,
	si32260_1_deviceInit,
	si32260_1_shutdownHw,
	si32260_1_paramReset,
	si32260_1_phoneRingStart,
	si32260_1_phoneRingStop,
	si32260_1_eventProcess,
	si32260_1_fxsSetLineState,
	si32260_1_fxsGetLineState,
	si32260_1_fxsSetBatteryPol,
	si32260_1_fxsGetBatteryPol,
	si32260_1_fxsGetHookState,
	si32260_1_ringState,
	si32260_1_fxsGetPulseStatus,
	si32260_1_fxsSetGainLv,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	slicFunc_dummy,
	si32260_1_dumpregister,
	slicFunc_dummy,
	si32260_1_LtTestStart,
	si32260_1_getLtTestResult,//Roger_silicon_lt
	slicFunc_dummy,//dyma_20140428 metering
	si32260_1_SetLoopback,//FXS_SET_LOOPBACK
	si32260_1_powerSaveMode,
};

static TIC_Return silab_alloc(int numTicFxs);
static void silab_free(void);

static uInt8 silab_ctrl_ReadRegisterWrapper_1 (void * hSpiGci, uInt8 channel,uInt8 regAddr); 
static int silab_ctrl_WriteRegisterWrapper_1 (void * hSpiGci, uInt8 channel, uInt8 regAddr, uInt8 data);
...
int si32260_1_deviceVerify(char* buf){
	...
	{
		si32260_DevPhyLocation = devInfo->fxsNum;
	}
	
	if(si32260_DevPhyLocation!=devInfo->fxsNum)
	{
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_ERROR, "Bypass si32260_1_deviceVerify on device %d\n", channel);
		return _TDI_FAIL;
	}
    ...
    while (0x1F!=(silab_ctrl_ReadRegisterWrapper_1(NULL,channel-1, 3)&0x1F)){
	...
	retValue = silab_ctrl_ReadRegisterWrapper_1 (NULL,channel-1, 0);
	...
	retValue = silab_ctrl_ReadRegisterWrapper_1 (NULL,channel-1, 47);
	...
	silab_ctrl_WriteRegisterWrapper_1 (NULL,channel-1, 14, 0x40);
	retValue = silab_ctrl_ReadRegisterWrapper_1 (NULL,channel-1, 14);
	if(retValue!=0x40)
	{
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "si32260_1_deviceVerify %d Success!\r\n\r\n",si32260_DevPhyLocation);
	...
	devInfo->pcm1ChNum += 0;
	devInfo->pcm2ChNum += SI32260_FXS_NUM+SI32260_FXO_NUM;
...
int si32260_1_deviceInit(char* buf)
{
	...
	if(pinitSetting == NULL) {
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_ERROR, "ERROR! si32260_1_deviceInit pinitSetting NULL\n");
		return _TDI_FAIL;
	}
	
    devCnt = pinitSetting->chan;
	if(devCnt!=si32260_DevPhyLocation){
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_WARNING, "Bypass si32260_1_deviceInit on Device %d!\r\n\r\n",devCnt);  //dyma_130118
		return _TDI_SUCCESS;
	}
	...
	*/
	...
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "\r\nsi32260_1NumProslic = %d\n", si32260NumProslic);
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "si32260_1NumChannel = %d\n", si32260NumChannel);
	SLIC_DBG_MSG(_SLIC_DBG_TYPE_INFO, "si32260_1deviceId = %d\n", si32260deviceId);
	...
	*/
	if(NULL == ProHWIntf)
	{
		...
		*/
		SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "ProSLIC_createControlInterface = %d\n", ProSLIC_createControlInterface(&ProHWIntf));
		...
		*/
		...
		{
			SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "ProSLIC_createDevice = %d\n", ProSLIC_createDevice (&(ProSLICDevices[index])));
		}
		...
		*/
		...
		ProSLIC_setControlInterfaceWriteRegister (ProHWIntf, silab_ctrl_WriteRegisterWrapper_1);
		ProSLIC_setControlInterfaceReadRegister (ProHWIntf, silab_ctrl_ReadRegisterWrapper_1);
	...
	*/
	...
	for(index = 0; index < si32260NumChannel; index++)
	{
		...
		data = silab_ctrl_ReadRegisterWrapper_1 (NULL,index, PCMMODE);
		data = (data & ~(0x3)) | SILAB_PCM_LINEARMODE;	 /* PCM_FMT[1:0] */
		silab_ctrl_WriteRegisterWrapper_1 (NULL,index, PCMMODE, data);

		/* Set DTX data to be driven on positive edge of PCLK */
		data = silab_ctrl_ReadRegisterWrapper_1 (NULL,index, PCMTXHI);
		silab_ctrl_WriteRegisterWrapper_1 (NULL,index, PCMTXHI, (data & ~SILAB_PCM_TX_EDGE));
...
int si32260_1_LtTestStart(char* buf)
{
	...
	if(atomic_read(&slic_init_flag_1)==0)
		return _TDI_FAIL;
	...
	if(linetest->chan<si32260NumChannel)
{
		...
		backup_register[i] = silab_ctrl_ReadRegisterWrapper_1(NULL,linetest->chan-SLIC_Offset, i);	
		}
		
	 	//excute line test function 
		testresult=Silicon_LineTestStart(linetest,arrayOfProslicChans[linetest->chan-SLIC_Offset]);

		//restore slic register after line test
		for(i = ID; i <= PDN_STAT; i++) {
		silab_ctrl_WriteRegisterWrapper_1(NULL,linetest->chan-SLIC_Offset, i, backup_register[i]); 
		}
	}
	else
		return _TDI_FAIL;
...
int si32260_1_fxsSetLineState(char* buf)
{
	...
	SLIC_1_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"_%d_si32260_1_fxsSetLine[%d]state 0x%x ",
		fxs_intr_cnt_1, //dyma_131227
		plineInfo->chan,
		plineInfo->state);
	
	if(chanPvt[plineInfo->chan-SLIC_Offset].chanType != SILAB_CHAN_TYPE_FXS) {
		return _TDI_SUCCESS;
	}
...
#endif
        default:
            SLIC_1_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si32260_1_State UNKNOWN !\n");
			//pre_state[plineInfo->chan]=0xff;
            return _TDI_FAIL;
    }
		
	SLIC_1_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"convert to Type %d\n",stateType);
	//pre_state[plineInfo->chan]=plineInfo->state;

    ProSLIC_SetLinefeedStatus(ports[plineInfo->chan-SLIC_Offset].ProObj, stateType);	 
...
//	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "%s \n",__func__);
	if (atomic_read(&slic_init_flag_1) ==0)
		return _TDI_FAIL;
    ...
    temp = silab_ctrl_ReadRegisterWrapper_1(NULL,plineInfo->chan-SLIC_Offset, 30) & 0x07;
...
//	 SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "%s : retTicStat=%d \n",__func__,plineInfo->retTicStat);
	if(plineInfo->retTicStat!=pre_stateType[plineInfo->chan-SLIC_Offset]){
		SLIC_1_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"_%d_si32260_1_fxsGetLine(%d)Ret[%d]=State[0x%x]\n",
		fxs_intr_cnt_1, //dyma_131227
		plineInfo->chan,
		temp,
		plineInfo->retTicStat);
		pre_stateType[plineInfo->chan-SLIC_Offset]=plineInfo->retTicStat;
	}
...
int si32260_1_fxsGetBatteryPol(char* buf)
{
	...
	if(chanPvt[pBatteryPolInfo->chan-SLIC_Offset].chanType != SILAB_CHAN_TYPE_FXS) {
     ...
     */
    polarity = (silab_ctrl_ReadRegisterWrapper_1(NULL,pBatteryPolInfo->chan-SLIC_Offset, 30) & 0x03);
...
int si32260_1_fxsSetBatteryPol(char* buf)
{
	...
	if (TIC_BATT_POL_REVERSE == pBatteryPolInfo->polarity) {
         ...
         */
        silab_ctrl_WriteRegisterWrapper_1 (NULL,pBatteryPolInfo->chan-SLIC_Offset, LINEFEED, SILAB_LINE_STATE_REVERSE_ACTIVE);
    }
    else {
        /*
         ...
         */
        silab_ctrl_WriteRegisterWrapper_1 (NULL,pBatteryPolInfo->chan-SLIC_Offset, LINEFEED, SILAB_LINE_STATE_FORWARD_ACTIVE);
    }
...
int si32260_1_fxsGetHookState(char* buf)
{
   ...
   /*
     ...
     */
     if(ports != NULL && atomic_read(&slic_init_flag_1) ==1)
    	ProSLIC_ReadHookStatus(ports[pHookStat->chan-SLIC_Offset].ProObj, &hook);
	 else
		return _TDI_FAIL;
		
	if(pre_hook[pHookStat->chan-SLIC_Offset]!=hook)
	{
		SLIC_1_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"_%d_si32260_1_fxsGetChan [%d] HookState [%d]\n",fxs_intr_cnt_1,pHookStat->chan,hook);  //dyma_131227
		pre_hook[pHookStat->chan-SLIC_Offset]=hook;
	}
...
int si32260_1_ringState(char* buf)
{
	...
	if(pre_state[pRingSetting->chan-SLIC_Offset]!=state)
	{
		SLIC_1_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"si32260_1_ringState [%d] count[%d]\n",state,count); 
		pre_state[pRingSetting->chan-SLIC_Offset]=state;
	}
...
SILAB_RING_STATE_RUN:
    ...
    switch (state) {
        ...
        case _TDI_RING_STATE_INIT:
			...
			SLIC_1_DBG_MSG(_SLIC_DBG_TYPE_DEBUG,"|chan=%d,state=%d,count=%d\n|make=%d	%d	%d\n|break=%d	%d	%d\n|rep=%d,numRing=%d\n",
			pRingSetting->chan,
			pRingSetting->state,
			pRingSetting->count,
			pRingSetting->make1s,
...
int si32260_1_fxsSetGainLv(char* buf)
{
	...
	ProSLIC_AudioGainSetup(ports[gainSetting->chan-SLIC_Offset].ProObj,gainSetting->listenSlicVal,gainSetting->speakSlicVal,slicPreset.impCountryIdx);
...
static int silab_ctrl_WriteRegisterWrapper_1 (void * hSpiGci, uInt8 channel,  uInt8 regAddr, uInt8 data)
{
	...
	spin_lock_irqsave(&silabs_spi_if_lock_1, silabs_spi_if_flags_1);  //dyma_131230
    silab_writeSi32260DirectSpi(channel, regAddr, data);
	spin_unlock_irqrestore(&silabs_spi_if_lock_1, silabs_spi_if_flags_1);  //dyma_131230
	//up(&SPI_SEM);
    return 0;
}
    ...
    uint8  writeData)
{
	...
	if(regAddr==LINEFEED){    //dyma_131227
		//if((fxs_intr_cnt_1-write_LINEFEED_intr_cnt_1)<20)
		SLIC_1_DBG_MSG(_SLIC_DBG_TYPE_DEBUG," write LINEFEED interval = %d\r\n\r\n",
			fxs_intr_cnt_1-write_LINEFEED_intr_cnt_1 //dyma_131227
			);
		write_LINEFEED_intr_cnt_1 = fxs_intr_cnt_1;
	}
	#endif
	
	SPI_bytes_write(SLIC_TYPE_SILICON,spi_interface_type,deviceId, ctrl, regAddr, &writeData, 1);
	/*#ifdef SLIC_ISI
	//SPI_bytes_write_zsi_isi(deviceId, ctrl, regAddr, &writeData, 1);
    ISI_bytes_write(deviceId, ctrl, regAddr, &writeData, 1);
	#else
    //SPI_bytes_write(deviceId, ctrl, regAddr, &writeData, 1);
    SPI_bytes_write_silicon(deviceId, ctrl, regAddr, &writeData, 1);
	#endif*/
    ...
    uint8 regAddr)
{
	...
	vint deviceId;
//	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "si32260deviceId = %d\n", si32260deviceId);
	deviceId =  si32260deviceId + _TDI_SLIC_SPI_ID; 
    /* 
     * Send Control byte, bit 6 set to 1 for reading
     */
    ctrl = (0x60 | silab_bitReverse(channel));
//	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "deviceId = %d\n", deviceId);
//	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "ctrl = %d\n", ctrl);
//	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "regAddr = %d\n", regAddr);
//	SLIC_DBG_MSG(_SLIC_DBG_TYPE_DEBUG, "retVal = %d\n", retVal);

	SPI_bytes_read(SLIC_TYPE_SILICON,spi_interface_type,deviceId, ctrl, regAddr, &retVal, 1);

	/*#ifdef SLIC_ISI
	//SPI_bytes_read_zsi_isi(deviceId, ctrl, regAddr, &retVal, 1);    
    ISI_bytes_read(deviceId, ctrl, regAddr, &retVal, 1); 
	#else
	//SPI_bytes_read(deviceId, ctrl, regAddr, &retVal, 1);
    SPI_bytes_read_silicon(deviceId, ctrl, regAddr, &retVal, 1); 
	#endif*/
