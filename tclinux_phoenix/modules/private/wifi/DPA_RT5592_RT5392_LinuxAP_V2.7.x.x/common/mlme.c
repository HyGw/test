...
{
	if (((pAd->chipCap.avg_rssi_all < -79) && (pAd->CommonCfg.BBPCurrentBW == BW_20))
		|| ((pAd->chipCap.avg_rssi_all < -76) && (pAd->CommonCfg.BBPCurrentBW == BW_40))) 
	{
		/* NO Dynamic VGA */
		return;
	}
	
	if ((pAd->CommonCfg.MO_Cfg.bDyncVGAEnable) && (pAd->MacTab.Size > 0))
	{
		if (pAd->Mlme.PeriodicRound %MLME_TASK_EXEC_MULTIPLE == 0)
		{
			...
			UCHAR RegValue = 0;
#ifdef ED_MONITOR
			UINT VgaLowerBound = (pAd->ed_chk != 0)?0x14:0x20;
#else
			UINT VgaLowerBound = 0x20;
#endif

			RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R66, &BbpReg);
			DBGPRINT(RT_DEBUG_TRACE,("one second False CCA=%d ,BbpReg=0x%x\n",pAd->RalinkCounters.OneSecFalseCCACnt,BbpReg));
			
			

			if (pAd->RalinkCounters.OneSecFalseCCACnt > pAd->CommonCfg.MO_Cfg.nFalseCCATh)
			{
				if (BbpReg < (pAd->CommonCfg.MO_Cfg.Stored_BBP_R66 + VgaLowerBound))
				{
					BbpReg += 0x4;
					AsicBBPWriteWithRxChain(pAd, BBP_R66, BbpReg, RX_CHAIN_ALL);
					DBGPRINT(RT_DEBUG_WARN,
					("pAd->CommonCfg.MO_Cfg.Stored_BBP_R66=0x%x one second False CCA=%d, fixed R66 at 0x%x\n",
					pAd->CommonCfg.MO_Cfg.Stored_BBP_R66, pAd->RalinkCounters.OneSecFalseCCACnt, BbpReg));
					
					/* R68=0d; R70 = 0x05; R73=0x10; */
					RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R68, 0x0D);
					RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R70, 0x05);
					RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R73, 0x10);

					/* LPF corner RF R31[7:3] =  (default RF R31[7:3] + 0x05) */
					RT30xxReadRFRegister(pAd, RF_R31, (PUCHAR)&RFValue);
					RFValue &= 0x07;
					if((pAd->chipCap.default_rf_r31_b3to7 + 5) >= 31)
						RFValue |= (0x1F << 3);						
					else
						RFValue |= ((pAd->chipCap.default_rf_r31_b3to7 + 5) << 3);
					RT30xxWriteRFRegister(pAd, RF_R31, RFValue);
				}

				/* set high or mid LNA gain */
				if(BbpReg == (pAd->CommonCfg.MO_Cfg.Stored_BBP_R66 + VgaLowerBound) && (pAd->chipCap.avg_rssi_all > -50))
				{					
					/* set middle gain */
					RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R65, &RegValue);
					RegValue |= 0x08;
					RegValue &= 0xfb; /* BBP_R65[3:2] set to b'10 */
				...
				else
				{
					/* set high gain */
					RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R65, &RegValue);
					RegValue |= 0x0C; /* BBP_R65[3:2] set to b'11 */
			...
			{				
				if (BbpReg > pAd->CommonCfg.MO_Cfg.Stored_BBP_R66)
				{
					...
					/* default R68=0b; R70 = 0x06; R73=0x13; */
					...
					RT30xxReadRFRegister(pAd, RF_R31, (PUCHAR)&RFValue);
					RFValue &= 0x07;
					RFValue |= (pAd->chipCap.default_rf_r31_b3to7 << 3);
					RT30xxWriteRFRegister(pAd, RF_R31, RFValue);
    ...
    IN  RTMP_ADAPTER   *pAd)
{
	...
	for(apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++)
	{
		...
		if (pAd->ApCfg.MBSSID[apidx].PhyMode == PHY_11B)
			bitmap &= 0x0F;

		if ((pAd->ApCfg.MBSSID[apidx].PhyMode == PHY_11A)
#ifdef DOT11_N_SUPPORT
				|| (pAd->ApCfg.MBSSID[apidx].PhyMode == PHY_11AN_MIXED)
				|| (pAd->ApCfg.MBSSID[apidx].PhyMode == PHY_11AGN_MIXED)
				|| (pAd->ApCfg.MBSSID[apidx].PhyMode == PHY_11N_5G)
#endif /* DOT11_N_SUPPORT */
		)
	{
		bitmap &= 0xFF0;
	...
	printk("######Set Support Rate : phymode=%d bABandPhyMode=%d\n", pAd->ApCfg.MBSSID[apidx].PhyMode, bABandPhyMode);
    ...
    IN  RTMP_ADAPTER   *pAd)
{
	...
	{
		case 1:
			pAd->CommonCfg.ExpectedSuppHTMCSSet[1] = 0;
			pAd->CommonCfg.ExpectedSuppHTMCSSet[2] = 0;
			pAd->CommonCfg.ExpectedSuppHTMCSSet[3] = 0;
			break;
		case 2:
			pAd->CommonCfg.ExpectedSuppHTMCSSet[2] = 0;
			pAd->CommonCfg.ExpectedSuppHTMCSSet[3] = 0;
