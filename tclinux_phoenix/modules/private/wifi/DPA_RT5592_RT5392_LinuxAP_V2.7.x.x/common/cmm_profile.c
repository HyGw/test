...
int wlan_chip_type = 0;// default 0 is RT5390R, 1 is RT5390H
...
/*	tmpbuf = kmalloc(MAX_PARAM_BUFFER_SIZE, MEM_ALLOC_FLAG);*/
	...
	do
	{
		/* Distinguish RT5390H or RT5390R */
		if (RTMPGetKeyParameter("RT5390Type", tmpbuf, 25, pBuffer, TRUE))
		 { 
		 	if(!strcmp(tmpbuf, "RT5390H"))
		 		wlan_chip_type = 1;
		 	else
		 		wlan_chip_type = 0;
		}
		...
		#ifdef RALINK_ATE 
		/* EfuseBufferMode */ 
		/* For backward compatible, the usage is the same as E2pAccessMode = 4 */ 
...
#ifdef NEW_ACS_ALGO
				if(RTMPGetKeyParameter("FALSE_CCA_THRESHOLD", tmpbuf, 32, pBuffer, TRUE))
				{
					pAd->FALSE_CCA_THRESHOLD = simple_strtol(tmpbuf, 0, 10);

					DBGPRINT(RT_DEBUG_TRACE, ("FALSE_CCA_THRESHOLD=%d\n", pAd->FALSE_CCA_THRESHOLD));
				}
#endif //NEW_ACS_ALGO
