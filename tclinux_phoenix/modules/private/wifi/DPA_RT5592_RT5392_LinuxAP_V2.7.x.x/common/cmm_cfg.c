	...
	IN	PSTRING			arg)
{
	...
	if (RTMPCheckPhyMode(pAd, pAd->RFICType, &WirelessMode) == FALSE)
		return FALSE;
	...
	IN	PSTRING			arg)
{
	...
	if (ScanRunning(pAd)) {
	    DBGPRINT (RT_DEBUG_TRACE, ("(%s): Still in Scanning(%d), not doing this time!\n", __FUNCTION__, pAd->Mlme.ApSyncMachine.CurrState));
	    return TRUE;
	}
