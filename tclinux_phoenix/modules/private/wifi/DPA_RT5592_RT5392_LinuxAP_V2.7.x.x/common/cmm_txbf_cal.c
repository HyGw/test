...
extern NDIS_SPIN_LOCK divider_cali_lock;
...
static void CaptureRFCal(PRTMP_ADAPTER pAd, COMPLEX_VALUE iqData[][3], int txAnt)
{
	...
	ULONG	IrqFlags = 0;
	RTMP_MAC_SHR_MSEL_PROTECT_UNLOCK(pAd, IrqFlags);  // to prevent lock twice in BBP IO
...
#endif /* SPECIFIC_BCN_BUF_SUPPORT */
#endif /* RTMP_MAC_PCI */
	pAd->divider_cali_js = jiffies; //keep the start time
#ifdef RALINK_ATE
	if (!ATE_ON(pAd))
#endif /* RALINK_ATE */
	if (jiffies_to_msecs(pAd->divider_cali_js - pAd->divider_cali_je) < DIVIDER_CALI_THRESHOLD) {
		printk ("%s: the delta time is too short:%u\n", __FUNCTION__, (pAd->divider_cali_js - pAd->divider_cali_je));
		RTMP_SEM_UNLOCK(&divider_cali_lock);
