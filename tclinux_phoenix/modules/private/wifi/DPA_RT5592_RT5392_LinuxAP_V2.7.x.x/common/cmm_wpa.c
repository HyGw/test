    ...
    do
    {
		...
		if ((pEntry->WepStatus == Ndis802_11Encryption2Enabled) && (peerKeyInfo.KeyDescVer != KEY_DESC_TKIP))
        {
    	    ...
    	    break;
                ...
                else if (CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_USE_SHA256) && ((peerKeyInfo.KeyDescVer != KEY_DESC_EXT) || (peerKeyInfo.KeyDescVer != KEY_DESC_OSEN)))
                {
	...
	if (MsgType != EAPOL_PAIR_MSG_1)
	{
        ...
        else if (EapolKeyInfo.KeyDescVer == KEY_DESC_OSEN)   /* AES-128 */
        {
...

			{
				...
				int k=0;
				DBGPRINT(RT_DEBUG_ERROR, ("PTK=>"));
				for(k=0;k<32;k++)
					DBGPRINT(RT_DEBUG_ERROR, ("%02x", *(tmp+k)));
