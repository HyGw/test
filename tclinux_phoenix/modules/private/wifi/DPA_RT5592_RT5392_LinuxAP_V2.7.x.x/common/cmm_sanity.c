    ...
    while ((Length + 2 + pEid->Len) <= MsgLen)    
    {
		...
		break;
			...
			*/
			...
			{
				VHT_OP_IE *pVhtOpIe = (VHT_OP_IE *)&pEid->Octet[0]; 

				VhtChWidth = pVhtOpIe->vht_op_info.ch_width; 
				VhtCentralCh0 = pVhtOpIe->vht_op_info.center_freq_1; 
			}
			break;
		...
		if ((pAd->LatchRfRegs.Channel > 14) && ((Sanity & 0x4) == 0))
		{
			...
			else
			{
				if(VhtCentralCh0 != 0)
				{
					...
					if (VhtCentralCh0 > pAd->LatchRfRegs.Channel)
						ChDiff = VhtCentralCh0 - pAd->LatchRfRegs.Channel;
					else
						ChDiff = pAd->LatchRfRegs.Channel - VhtCentralCh0;

					/* For ChWidth 20Mhz ChDiff should be 0 and for 40Mhz ChDiff should be 2 */
					if ((VhtChWidth == 0) && (ChDiff > 2))
					{
						/* Channel info is not valid - BRCM 802.11ac AP in 802.11a mode
						   sends out irregular beacons on a different channel including
						   VHT_OP_IE. Channel info in VHT_OP_IE can be used to filter
						   out such beacons */ 
						DBGPRINT(RT_DEBUG_TRACE, ("Beacon/Probe Resp incorrect VHT channel detected\n"));
						goto SanityCheck;
					}
				}
...
BOOLEAN PeerProbeReqSanity(
    ...
    IN VOID *Msg, 
    IN ULONG MsgLen, 
  	OUT PPEER_PROBE_REQ_PARAM ProbeReqParam)
{
    PFRAME_802_11 Fr = (PFRAME_802_11)Msg;
    UCHAR		*Ptr;
        ...
        switch(eid)
        {
	        case IE_VENDOR_SPECIFIC:
				if (eid_len <= 4)
					break;
#ifdef RSSI_FEEDBACK
                if (ProbeReqParam->bRssiRequested && NdisEqualMemory(eid_data, RALINK_OUI, 3) && (eid_len == 7))
                {
