				...
				
				{
					...
					if (pEntry && i < MAX_LEN_OF_MAC_TABLE)
					{
						if ((pEntry->HTPhyMode.field.MODE == MODE_HTMIX || pEntry->HTPhyMode.field.MODE == MODE_HTGREENFIELD) &&
						    (((pAd->CommonCfg.TxStream == 3) && (pEntry->HTPhyMode.field.MCS == MCS_23 || pEntry->HTPhyMode.field.MCS == MCS_22)) ||
						    ((pAd->CommonCfg.TxStream == 2) && (pEntry->HTPhyMode.field.MCS == MCS_15 || pEntry->HTPhyMode.field.MCS == MCS_14)) ||
						     ((pAd->CommonCfg.TxStream == 1) && (pEntry->HTPhyMode.field.MCS == MCS_7 || pEntry->HTPhyMode.field.MCS == MCS_6)))
						)
						{
							ProtCfg.word = 0x01744004;	/*duplicaet legacy 24M. BW set 1.*/
							ProtCfg4.word = 0x03f44084;
                    			                DBGPRINT(RT_DEBUG_TRACE, ("Conditional Disable RTS/CTS ------\n"));
						}
					}
				}
