...
NDIS_SPIN_LOCK divider_cali_lock;
...
/*		pBeaconBuf = kmalloc(MAX_BEACON_SIZE, MEM_ALLOC_FLAG);*/
		...
		*/
		...
		NdisAllocateSpinLock(pAd, &divider_cali_lock);
...
static UINT8 NICGetBandSupported(
		IN	PRTMP_ADAPTER	pAd)
{
    if (IS_RT3593_NEW(pAd))
            pAd->Antenna.field.BoardType = 0;
...
/*	EDCA_PARM DefaultEdcaParm;*/
	...
	/* Init Divider Calibration Jiffies */
	pAd->divider_cali_js = jiffies;
	pAd->divider_cali_je = jiffies;
...
#ifdef CONFIG_AP_SUPPORT
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		for(j = BSS0; j < pAd->ApCfg.BssidNum; j++)
		{
			...
			pAd->ApCfg.MBSSID[j].SuppRateBitmap = 0xFFF;
			pAd->ApCfg.MBSSID[j].SuppHTRateBitmap = 0xFFFF;
...
pAd->BbpResetFlagCountVale = 20;
	...
	pAd->CommonCfg.MO_Cfg.nFalseCCATh = 200;
	pAd->CommonCfg.MO_Cfg.nLowFalseCCATh = 30;
	pAd->chipCap.default_rf_r31_b3to7 = 0x80 >> 3;
...
#ifdef MULTIPLE_CARD_SUPPORT
{
		extern BOOLEAN RTMP_GetCardID(PRTMP_ADAPTER pAd);

		if ( RTMP_GetCardID(pAd) == FALSE)
			DBGPRINT(RT_DEBUG_ERROR, ("Error! Failded to get a card ID.\n"));
}
#endif /* MULTIPLE_CARD_SUPPORT */
