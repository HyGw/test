...
int wlan_led_control = WIFI_CHIP_CONTROL;
#else
...
#ifdef TCSUPPORT_LED_SWITCH_BUTTON
	if(WLAN_LED_RECOVER == action)
	{
		wlan_led_control = WIFI_CHIP_CONTROL;
		if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RADIO_OFF))           // if radio on,turn on the wlan led
		{
			RTMPSetLED(pAd,LED_LINK_UP);
		}
	}
	else if(WLAN_LED_OFF == action)
	{
		wlan_led_control = WIFI_CHIP_CONTROL;
		...
		wlan_led_control = WIFI_CHIP_NO_CONTROL;
	}
	else if(WLAN_LED_ON == action)
	{
		wlan_led_control = WIFI_CHIP_CONTROL;
		RTMPSetLED(pAd,LED_LINK_UP);
		wlan_led_control = WIFI_CHIP_NO_CONTROL;
...
unsigned int BBUWlanGetLed(int gpio){
	...
	RTMP_IO_READ32(pAd, GPIO_CTRL_CFG, &gpioctl);
	RTMP_IO_READ32(pAd, GPIO_CTRL_CFG, &gpioctl); //read twice for rt63365
	if(gpio&WLAN_GPIO_MODE_DATE){
		gpio &= ~WLAN_GPIO_MODE_DATE;
		if(gpio>7)
			return(gpioctl&((1<<(gpio))<<16));
		else
...
#if defined(TCSUPPORT_POWERSAVE_ENABLE) || defined(TCSUPPORT_WLAN_GPIO) || defined(TCSUPPORT_LED_SWITCH_BUTTON)
