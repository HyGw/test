...
#ifdef TCSUPPORT_WLAN_PERSSID_RATELIMIT
int wlan_pktqosInit(wlan_pktqosToken_t *token, u32 bandwidth){

	token->tokens = bandwidth <<10;  /*  n kbps <<10 == n*1024 bit/s Lansy.shi 20111117 */
	token->ctokens = token->tokens ;
	token->c_time=0;
	token->l_time=0;
...
void wlan_UpdateTokens(wlan_pktqosToken_t *token, u32 bandwidth)
{
	u32 cur_tokens = 0;
	u32 new_tokens = 0;
	
	token->c_time = (jiffies*1000)/HZ;
	new_tokens = min((token->c_time-token->l_time)*bandwidth, token->tokens);
	token->l_time = token->c_time;
	cur_tokens = token->ctokens;
	token->ctokens =min(cur_tokens+new_tokens, token->tokens);	
}

int wlan_pktqosCheckToken(wlan_pktqosToken_t *token, u32 bandwidth, u32 size){

	wlan_UpdateTokens(token, bandwidth);

	if(token->ctokens < (size<<3)){ /*  size<<3  bytes to bits */
		//0: drop
		return 0;
	}
	else{
		token->ctokens -= size<<3;
	}
...
int rt28xx_packet_xmit(void *skbsrc)
{
	...
	VOID *pAd = NULL;
#ifdef TCSUPPORT_WLAN_PERSSID_RATELIMIT
	unsigned char FromWhichBSSID=0;
#endif
/*	int status = 0; */
...
#ifdef TCSUPPORT_WLAN_PERSSID_RATELIMIT
	FromWhichBSSID = RTMP_GET_PACKET_NET_DEVICE_MBSSID(skb);

	if(FromWhichBSSID<min(MAX_MBSSID_NUM((PRTMP_ADAPTER)pAd), WLAN_PORTS_NUM)) {
		if(wlan_ratelimit[FromWhichBSSID<<1] != 0) {
			if(!wlan_pktqosCheckToken(&wlan_pktqosToken[FromWhichBSSID<<1], wlan_ratelimit[FromWhichBSSID<<1], skb->len)) {
				/* release packet */
