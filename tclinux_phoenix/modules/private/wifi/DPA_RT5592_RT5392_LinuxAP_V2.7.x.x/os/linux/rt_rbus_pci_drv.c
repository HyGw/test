...
static inline void rt2860_int_enable(PRTMP_ADAPTER pAd, unsigned int mode)
{
	u32 regValue;
#ifdef BB_SOC
	/*we use polling mode when ADSL initilzing, and the polling function is 
	ISR of wifi driver. This way need a lock to protect to pAd->int_disable_mask.
	shnwind 20110105.*/
	unsigned long flags=0;
	
	RTMP_INT_LOCK(&pAd->Pollingmode.PollingModeLock, flags);
#endif
	

	pAd->int_disable_mask &= ~(mode);
	regValue = pAd->int_enable_reg & ~(pAd->int_disable_mask | INT_AC0_DLY | INT_RX);
...
#ifdef BB_SOC
	RTMP_INT_UNLOCK(&pAd->Pollingmode.PollingModeLock, flags);
#endif		
...
#endif /* WORKQUEUE_BH */
{
	...
	{
		RTMP_INT_LOCK(&pAd->LockInterrupt, flags);
		pAd->int_disable_mask &= ~(RT2860_INT_RX_DLY); 
		RTMP_INT_UNLOCK(&pAd->LockInterrupt, flags);
		return;
	}
#ifdef UAPSD_SUPPORT
    ...
    pObj = (POS_COOKIE) pAd->OS_Cookie;
	RTMP_INT_LOCK(&pAd->LockInterrupt, flags);
	pAd->int_pending &= ~(RT2860_INT_RX_DLY);
	RTMP_INT_UNLOCK(&pAd->LockInterrupt, flags);
#ifdef CONFIG_AP_SUPPORT	
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	...
	if (pAd->int_pending & RT2860_INT_RX_DLY || bReschedule) 
	{
#ifdef WORKQUEUE_BH
	...
	/* enable RxINT again */
	rt2860_int_enable(pAd, RT2860_INT_RX_DLY);
	RTMP_INT_UNLOCK(&pAd->LockInterrupt, flags);
...
#endif /* WORKQUEUE_BH */
{
	...
	{
		RTMP_INT_LOCK(&pAd->LockInterrupt, flags);
		pAd->int_disable_mask &= ~(RT2860_INT_TX_DLY); 
		RTMP_INT_UNLOCK(&pAd->LockInterrupt, flags);
		return;
	}
...
/*	printk("ac0_dma_done_process\n"); */
	...
	RTMP_INT_LOCK(&pAd->LockInterrupt, flags);
	pAd->int_pending &= ~RT2860_INT_TX_DLY;
	RTMP_INT_UNLOCK(&pAd->LockInterrupt, flags);
...
/*	RTMPHandleMgmtRingDmaDoneInterrupt(pAd); */
	...
	RTMP_INT_LOCK(&pAd->LockInterrupt, flags);
	/* double check to avoid lose of interrupts  */
	if ((pAd->int_pending & RT2860_INT_TX_DLY) || bReschedule)
	{
#ifdef WORKQUEUE_BH
	...
	rt2860_int_enable(pAd, RT2860_INT_TX_DLY);
	RTMP_INT_UNLOCK(&pAd->LockInterrupt, flags);    
}
...
/*	if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE)) */
	{
		RTMP_IO_READ32(pAd, INT_SOURCE_CSR, &IntSource.word);
#if defined(BB_SOC) && defined(CONFIG_MIPS_RT63365)
		// bugfix for 12258. shnwind 1209		
		if((check_rt63365ver1() == 1) && ((IntSource.word & 0xfff00000) == 0x53900000)){
			printk("interrupt %x\n",IntSource.word);
			return  IRQ_HANDLED;
		}
#endif		
...
redo: 
	...
	if (IntSource.word & RT2860_INT_RX_DLY) {
		RTMP_INT_LOCK(&pAd->LockInterrupt, flags);
		pAd->int_pending |= RT2860_INT_RX_DLY;
		
		if ((pAd->int_disable_mask & RT2860_INT_RX_DLY) == 0) 
			rt2860_int_disable(pAd, RT2860_INT_RX_DLY);
	...
	if (IntSource.word & RT2860_INT_TX_DLY) {
		RTMP_INT_LOCK(&pAd->LockInterrupt, flags);
		pAd->int_pending |= RT2860_INT_TX_DLY;
		
		if ((pAd->int_disable_mask & RT2860_INT_TX_DLY) == 0) 
			rt2860_int_disable(pAd, RT2860_INT_TX_DLY);
