...
INT Set_AP_SSID_Proc(
    IN  PRTMP_ADAPTER   pAdapter, 
    IN  PSTRING          arg);
//TCP ACK Aggregate Patch 2013_0911  
//==========================================
INT Set_ACKQ_Proc(
	IN  PRTMP_ADAPTER   pAdapter, 
    IN  PSTRING          arg);
...
//Brian
    ...
    {"SupportRate",                                         Set_SupportRate_Proc},
    {"SupportHTRate",                                         Set_SupportHTRate_Proc},
#endif
#endif
	...
	case OID_802_11_WNM_BTM_REQ:
		{
    		...
    		else if (((pHSCtrl->HotSpotEnable) && (pAd->ApCfg.MBSSID[pObj->ioctl_if].AuthMode == Ndis802_11AuthModeOpen)) || ((pEntry->WpaState == AS_PTKINITDONE) && (pEntry->GTKState = REKEY_ESTABLISHED)))
    		{
    			printk("btm1\n");
				Send_BTM_Req(pAd,
		...
		case OID_802_11_SECURITY_TYPE:
			{
				...
				SecurityType->auth_mode = pMbss->AuthMode;
				SecurityType->encryp_type = pMbss->WepStatus; 
		...
		case OID_802_11_HS_BSSID:
			{
				...
				pMbss = &pAd->ApCfg.MBSSID[pObj->ioctl_if];
				wrq->u.data.length = 6;
				Status = copy_to_user(wrq->u.data.pointer, pMbss->Bssid, 6); 
			}
			break;
#ifdef CONFIG_HOTSPOT_R2			
	...
	IN	PSTRING			arg)
{
	...
	pAdapter->ACKQEN = (UCHAR) simple_strtol(arg, 0, 10);
	...
	IN	PSTRING			arg)
{
	...
	pAdapter->AckTimeout = 0;
	pAdapter->AckNOTimeout = 0;
	pAdapter->Ack1 = 0;
	pAdapter->Ack2 = 0;
	pAdapter->Ack3 = 0;
	pAdapter->Ack4 = 0;
	...
	IN	PSTRING			arg)
{
	...
	pAdapter->AckWaitTime = (UCHAR) simple_strtol(arg, 0, 10);
	
	printk("wait time=%d\n", pAdapter->AckWaitTime);
	...
	IN	PSTRING			arg)
{
	...
	pAdapter->Acklen = (UCHAR) simple_strtol(arg, 0, 10);
	...
	IN	PSTRING			arg)
{
	...
	//TCP ACK Aggregate Patch 2013_0911  
	//==========================================	
	printk("\n%-19s%-4s%-4s%-4s%-4s%-8s%-7s%-7s%-7s%-10s%-6s%-6s%-6s%-6s%-7s%-7s%-7s\n",
		   "MAC", "AID", "BSS", "PSM", "WMM", "MIMOPS", "RSSI0", "RSSI1", 
		   "RSSI2", "PhMd", "BW", "MCS", "SGI", "STBC", "Idle", "Rate", "badatheros","QosMap");
	//==========================================
...
//TCP ACK Aggregate Patch 2013_0911  
//==========================================				
			printk("%-7d", (int)pEntry->bIAmBadAtheros);	
//==========================================	
#ifdef CONFIG_HOTSPOT_R2			
...
//TCP ACK Aggregate Patch 2013_0911
//==========================================	
	sprintf(msg+strlen(msg), "ack timeout: = %d\n", pAd->AckTimeout);
	sprintf(msg+strlen(msg), "ack no timeout: = %d\n", pAd->AckNOTimeout);
	sprintf(msg+strlen(msg), "ack1: = %d\n", pAd->Ack1);
	sprintf(msg+strlen(msg), "ack2: = %d\n", pAd->Ack2);
	sprintf(msg+strlen(msg), "ack3: = %d\n", pAd->Ack3);
	sprintf(msg+strlen(msg), "ack4: = %d\n", pAd->Ack4);
	sprintf(msg+strlen(msg), "ack5: = %d\n", pAd->Ack5);
	sprintf(msg+strlen(msg), "ack6: = %d\n", pAd->Ack6);
	sprintf(msg+strlen(msg), "ack7: = %d\n", pAd->Ack7);
	sprintf(msg+strlen(msg), "ack8: = %d\n", pAd->Ack8);
	sprintf(msg+strlen(msg), "ack9: = %d\n", pAd->Ack9);
	sprintf(msg+strlen(msg), "ack10: = %d\n", pAd->Ack10);
//==========================================	
...
{	
	...
	/* && pAd->ed_vga_at_lowest_gain */  //unmark this if chip has dync VGA
	)
	||
	(IS_RT5392(pAd) && (pAd->ed_trigger_cnt > pAd->ed_learning_time_threshold))
	)
	{
	...
	IN UINT32 ANQPRspLen)
{
	...
	UINT32 Len = 0, i, j, QueryRspOffset = 0;
	INT32 Ret;
	BOOLEAN Cancelled;
	BOOLEAN IsFound = FALSE;
	...
	if (ANQPRspLen <= pHSCtrl->MMPDUSize)
	{
		...
		if (!IsFound) 
		{
			DBGPRINT(RT_DEBUG_OFF, ("%s Can not find peer address[%02x:%02x:%02x:%02x:%02x:%02x] in GASPeerList (InitRsp)\n", __FUNCTION__, PeerMACAddr[0],PeerMACAddr[1],PeerMACAddr[2],PeerMACAddr[3],PeerMACAddr[4],PeerMACAddr[5]));			
			goto error1;
		}
	...
	IN PCHAR arg)
{
 	if(strlen(arg) == 0)
        ...
        IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
        {
            POS_COOKIE pObj;
            pObj = (POS_COOKIE) pAd->OS_Cookie;
            pAd->ApCfg.MBSSID[pObj->ioctl_if].SuppRateBitmap = (ULONG) simple_strtol(arg, 0, 16);

    		DBGPRINT(RT_DEBUG_ERROR, ("[MAX RATE]%s:: apidx=%d, SupportRate=%1x\n", __FUNCTION__
                , pObj->ioctl_if, pAd->ApCfg.MBSSID[pObj->ioctl_if].SuppRateBitmap));
        }
#endif /* CONFIG_AP_SUPPORT */
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	  	{
		  	POS_COOKIE pObj;
		  	pObj = (POS_COOKIE) pAd->OS_Cookie;
		 	 pAd->ApCfg.MBSSID[pObj->ioctl_if].SuppHTRateBitmap = (ULONG) simple_strtol(arg, 0, 16);

		  	DBGPRINT(RT_DEBUG_ERROR, ("[MAX RATE]%s:: apidx=%d, SupportHTRate=%1x\n", __FUNCTION__
			  , pObj->ioctl_if, pAd->ApCfg.MBSSID[pObj->ioctl_if].SuppHTRateBitmap));
	  	}
