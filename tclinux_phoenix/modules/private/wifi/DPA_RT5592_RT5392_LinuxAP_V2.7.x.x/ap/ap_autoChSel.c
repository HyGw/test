	...
	)
{
	...
	if (bootUP)
	{
		ch_sel_time = BOOTUP_AUTO_CHANNEL_SEL_TIMEOUT;
	}
	else
	{
		ch_sel_time = AUTO_CHANNEL_SEL_TIMEOUT;
	}
	pAd->pChannelInfo->chanbusytime[ch_index] = (BusyTime * 100) / ch_sel_time;
#else
	...
	)
{
	...
	int candidate_ch;
#ifdef NEW_ACS_ALGO
	int candidate_ext_ch_offset=EXTCHA_ABOVE;
	UINT32 candidate_value; /* false cca and ch busy time */
	UINT32 min_candidate_value; /* min false cca and ch busy time */
	UINT32 threshold;
#endif //NEW_ACS_ALGO
	...
	{
#ifdef NEW_ACS_ALGO
		DBGPRINT(RT_DEBUG_TRACE, ("Channel %d : Dirty = %ld, False CCA = %u, Busy Time = %u, False CCA + Busy Time= %u,  Skip Channel = %s\n",
#else	
	...
	min_falsecca = 0xFFFFFFFF;
	min_candidate_value = 0xFFFFFFFF;
#else
	...
	for (channel_idx = 0; channel_idx < pAd->ChannelListNum; channel_idx++)
	{
		...
		if (pAd->FALSE_CCA_THRESHOLD > 0)	//old way : falseCCA < Threshold
		{
			 if (pChannelInfo->FalseCCA[channel_idx] <= pAd->FALSE_CCA_THRESHOLD) 
		{
			UINT32 dirtyness = pChannelInfo->dirtyness[channel_idx];
				int ExtChannel_Offset = EXTCHA_ABOVE;
			...
			*/
			if ((pAd->CommonCfg.RegTransmitSetting.field.BW == BW_40)
				...
				if ((min_dirty > dirtyness))
					...
					candidate_ext_ch_offset = ExtChannel_Offset;
					NdisMoveMemory(candidate_ExChannel, ExChannel, 2);
					}
				}
					}
		else    //new way : min falseCCA+busytime
					{
			UINT32 dirtyness = pChannelInfo->dirtyness[channel_idx];
			int ExtChannel_Offset = EXTCHA_ABOVE;

			candidate_value = pChannelInfo->FalseCCA[channel_idx]  + 
#ifdef AP_QLOAD_SUPPORT
						pChannelInfo->chanbusytime[channel_idx];
#else
						0;
			...
			*/
			...
			if (min_candidate_value > candidate_value)
						{
				min_candidate_value = candidate_value;
				min_dirty = dirtyness;
				candidate_ch = channel_idx;
				candidate_ext_ch_offset = ExtChannel_Offset;
				NdisMoveMemory(candidate_ExChannel, ExChannel, 2);
				DBGPRINT(RT_DEBUG_TRACE, ("CCA + Ch busy time value : Min candidate[%d] (Include extension channel) ==> Select Channel %d \n", 
					min_candidate_value, ch));
			}
			else	 if (min_candidate_value == candidate_value  &&  (min_dirty > dirtyness))
			{
				min_dirty = dirtyness;
				candidate_ch = channel_idx;
				candidate_ext_ch_offset = ExtChannel_Offset;
				NdisMoveMemory(candidate_ExChannel, ExChannel, 2);
				DBGPRINT(RT_DEBUG_TRACE, ("CCA + Ch busy time value : Min candidate[%d]  dirtyness[%d] (Include extension channel) ==> Select Channel %d \n", 
					min_candidate_value, dirtyness, ch));
						}
					}
				}
	
#else //<--- not NEW_ACS_ALGO
	...
	if (candidate_ch >= 0)
	{
		...
		if (pAd->CommonCfg.RegTransmitSetting.field.BW == BW_40)
		{
			if(ch > 14)
			{
				if((candidate_ch + 4) >= pAd->ChannelListNum)
					candidate_ext_ch_offset = EXTCHA_BELOW;
				if((candidate_ch - 4) <= 0)
					candidate_ext_ch_offset = EXTCHA_ABOVE; 
			}
			else
			{
				if((ch + 4) >= 11)
			...
			pAd->CommonCfg.RegTransmitSetting.field.EXTCHA = candidate_ext_ch_offset;
			DBGPRINT(RT_DEBUG_TRACE, ("ExtChannel Offset : %s %d\n", (candidate_ext_ch_offset ==1) ? "ABOVE" : "BELOW",candidate_ext_ch_offset));
		}
#endif //NEW_ACS_ALGO
	...
	candidate_ch = -1;
#ifdef NEW_ACS_ALGO
	if (pAd->FALSE_CCA_THRESHOLD > 0)
		threshold = pAd->FALSE_CCA_THRESHOLD;
	else
	...
	for (channel_idx = 0; channel_idx < pAd->ChannelListNum; channel_idx++)
	{
		if (pChannelInfo->SkipList[channel_idx] == TRUE)
			continue;
#ifdef NEW_ACS_ALGO		
		if (pChannelInfo->FalseCCA[channel_idx] > threshold)
#else
		if (pChannelInfo->FalseCCA[channel_idx] > CCA_THRESHOLD)
#endif
		{
...
#ifdef NEW_ACS_ALGO
			UpdateChannelInfo(pAd, i,Alg, TRUE);
#else
