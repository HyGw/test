...
SendAssocResponse:		
	...
	SupRateLen = pAd->ApCfg.MBSSID[pEntry->apidx].SupRateLen;
#else
	...
	if (bACLReject == TRUE)
	{
	    ...
	    MakeOutgoingFrame(pOutBuffer,               &FrameLen,
	                      ...
	                      1,                        &SupRateLen,
#ifdef DYNAMIC_RX_RATE_ADJ
						SupRateLen,					pAd->ApCfg.MBSSID[pEntry->apidx].SupRate,
#else
...
#ifdef DYNAMIC_RX_RATE_ADJ
	if ((pAd->ApCfg.MBSSID[pEntry->apidx].ExtRateLen) && (PhyMode != PHY_11B))
#else
...
/*                          1,                        &pAd->ApCfg.ErpIeContent, */
                          1,                        &ExtRateIe,
#ifdef DYNAMIC_RX_RATE_ADJ
							1,							&pAd->ApCfg.MBSSID[pEntry->apidx].ExtRateLen,
							pAd->ApCfg.MBSSID[pEntry->apidx].ExtRateLen,	pAd->ApCfg.MBSSID[pEntry->apidx].ExtRate,
...
#ifdef DYNAMIC_RX_RATE_ADJ
		NdisMoveMemory(HtCapabilityRsp.MCSSet, pAd->ApCfg.MBSSID[pEntry->apidx].ExpectedSuppHTMCSSet, 4);
#endif /* DYNAMIC_RX_RATE_ADJ */
	...
	/* qosmap IE */
	DBGPRINT(RT_DEBUG_TRACE,("entry=%d\n", pEntry->QosMapSupport));
...
{
	if((pEntry->SupportRateMode & (SUPPORT_OFDM_MODE)) == 0)
...
#endif /* DOT11N_DRAFT3 */
#ifdef CONFIG_HOTSPOT_R2
				if (ExtCapInfo.QosMap)
					pEntry->QosMapSupport = 1;
#endif					
#ifdef DOT11V_WNM_SUPPORT
				if(IS_BSS_TRANSIT_MANMT_SUPPORT(pAd, pEntry->apidx))
				{
					if(ExtCapInfo.BssTransitionManmt)
						pEntry->BssTransitionManmtSupport = 1;
				}
				if(IS_WNMDMS_SUPPORT(pAd, pEntry->apidx))
				{
					if(ExtCapInfo.DMSSupport)
						pEntry->DMSSupport = 1;
				}
...
/*				for (i=15; i>=0; i--) */
				...
				// conditionally follow peer's capability
				if (pAd->ApCfg.MBSSID[pEntry->apidx].WepStatus == Ndis802_11WEPDisabled) {
					pEntry->MpduDensity = 0;
				} else if (pHtCapability->HtCapParm.MpduDensity >= 6)
					pEntry->MpduDensity = 6;
				else
				pEntry->MpduDensity = pHtCapability->HtCapParm.MpduDensity;
				DBGPRINT(RT_DEBUG_TRACE, ("Conditional Density (%d)------\n", pEntry->MpduDensity));
