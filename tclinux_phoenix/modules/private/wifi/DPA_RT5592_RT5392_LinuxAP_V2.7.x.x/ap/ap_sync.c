	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	for(apidx=0; apidx<pAd->ApCfg.BssidNum; apidx++)
	{
		...
		if (((ProbeReqParam.SsidLen == 0) && (! pAd->ApCfg.MBSSID[apidx].bHideSsid)) ||
#ifdef WSC_AP_SUPPORT
			...
			((ProbeReqParam.SsidLen == 32) && MAC_ADDR_EQUAL(Addr3, pAd->ApCfg.MBSSID[apidx].Bssid) && (pAd->ApCfg.MBSSID[apidx].bHideSsid == 0)) ||
#endif /* WSC_AP_SUPPORT */
		((ProbeReqParam.SsidLen == pAd->ApCfg.MBSSID[apidx].SsidLen) && NdisEqualMemory(ProbeReqParam.Ssid, pAd->ApCfg.MBSSID[apidx].Ssid, (ULONG) ProbeReqParam.SsidLen)))
			;
		else
			continue; /* check next BSS */
#ifdef CONFIG_HOTSPOT
		if (ProbeReqforHSAP(pAd, apidx, &ProbeReqParam))
			;
		else
			continue; /* check next BSS */
#endif
	...
	BND_STRG_CHECK_CONNECTION_REQ(	pAd,
		...
		else
		{
			...
			if ((pHSCtrl->HotSpotEnable == 0) && (pHSCtrl->bASANEnable == 1) && (pAd->ApCfg.MBSSID[apidx].AuthMode == Ndis802_11AuthModeWPA2))
			{
				RSNIe = IE_WPA;
				MakeOutgoingFrame(pOutBuffer+FrameLen,        &TmpLen,
						  1,                            &RSNIe,
		 ...
		 */
{
	...
	if (ProbeReqParam.bRequestRssi == TRUE)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("SYNC - Send PROBE_RSP to %02x:%02x:%02x:%02x:%02x:%02x...\n", ProbeReqParam.Addr2[0],ProbeReqParam.Addr2[1],ProbeReqParam.Addr2[2],ProbeReqParam.Addr2[3],ProbeReqParam.Addr2[4],ProbeReqParam.Addr2[5] ));
...
#ifdef NEW_ACS_ALGO			
			UpdateChannelInfo(pAd, pAd->ApCfg.current_channel_index, pAd->ApCfg.AutoChannelAlg, FALSE);
#else
