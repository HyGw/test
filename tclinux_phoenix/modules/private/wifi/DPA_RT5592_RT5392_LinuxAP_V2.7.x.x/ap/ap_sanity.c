    ...
    while (((UCHAR *)eid_ptr + eid_ptr->Len + 1) < ((UCHAR *)Fr + MsgLen))
    {
		...
		case IE_EXT_CAPABILITY:
			if (eid_ptr->Len >= sizeof(EXT_CAP_INFO_ELEMENT))
			{
				...
				tmp = (unsigned char *)pExtCapInfo;
				DBGPRINT(RT_DEBUG_ERROR,("0x%x:%x:%x:%x:%x:%x\n",*(tmp), *(tmp+1), *(tmp+2), *(tmp+3), *(tmp+4), *(tmp+5)));
				DBGPRINT(RT_DEBUG_ERROR,("*ExtCapInfo.QosMap=%d\n", pExtCapInfo->QosMap));
			}
					...
					{
						{
							...
							printk("!!!!!!not found OSEN IE,%x:%x:%x:%x\n", *tmp, *(tmp+1), *(tmp+2), *(tmp+3));
