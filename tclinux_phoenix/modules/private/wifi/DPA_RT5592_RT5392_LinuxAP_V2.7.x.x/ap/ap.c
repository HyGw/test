    ...
    IN  MAC_TABLE_ENTRY *pEntry)
{
    ...
    if((pEntry->AuthMode == Ndis802_11AuthModeWPA) || (pEntry->AuthMode == Ndis802_11AuthModeWPA2) || (pAd->ApCfg.MBSSID[apidx].IEEE8021X == TRUE))
	{
		...
		if (pEntry)
        {
			...
			if ((pMbss->HotSpotCtrl.HotSpotEnable == 1) && (pMbss->AuthMode == Ndis802_11AuthModeWPA2) && (pEntry->hs_info.ppsmo_exist == 1))
            {
                ...
                printk("event eapol start, %x:%x:%x:%x\n",FrameBuf[offset+sizeof(eapol_start_1x_hdr)+4],FrameBuf[offset+sizeof(eapol_start_1x_hdr)+5],FrameBuf[offset+sizeof(eapol_start_1x_hdr)+6],FrameBuf[offset+sizeof(eapol_start_1x_hdr)+7]);
            }
