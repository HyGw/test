...
#ifdef TCSUPPORT_WLAN_PERSSID_RATELIMIT
extern u32 wlan_ratelimit[WLAN_PORTS_NUM<<1];
extern wlan_pktqosToken_t wlan_pktqosToken[WLAN_PORTS_NUM<<1];
extern int wlan_pktqosCheckToken(wlan_pktqosToken_t *token,int bandwidth, u32 size);
#endif
	...
	IN UINT8 UserPriority)
{
	...
	if (((pMacEntry->TXBAbitmap & (1<<UserPriority)) == 0) 
		...
		&& (!CLIENT_STATUS_TEST_FLAG(pMacEntry, fCLIENT_STATUS_PMF_CAPABLE) || ((pMacEntry->PortSecured == WPA_802_1X_PORT_SECURED) && CLIENT_STATUS_TEST_FLAG(pMacEntry, fCLIENT_STATUS_PMF_CAPABLE)))
#endif		
			 /*
...
inline BOOLEAN ApAllowToSendPacket(
	IN RTMP_ADAPTER *pAd,
...
//==========================================				
			{
				...
				if (TypeLen == 0x0800)
				{
					if (pAd->ACKQEN == 1)
					{
						if ((pktLen >= 48) && (pSrcBuf[23] == 0x06) && (pSrcBuf[47] == 0x10))
						{
							...
							if (qlen == 1)
							{
								...
								{
									//printk("found tcp ack..\n");
									RTMPSetTimer(&pEntry->QueueAckTimer, pAd->AckWaitTime);
									pEntry->QueueAckTimerRunning = TRUE;
								}
							}
							else if (qlen >= pAd->Acklen)
							{
								if (pEntry->QueueAckTimerRunning == TRUE)
								{
					...
					else
					{
						...
						if ((qlen = skb_queue_len(&pEntry->ack_queue)) > 0)
						{
							while (1)
							{
								pPacket2 = skb_dequeue(&pEntry->ack_queue);
								if (pPacket2 == NULL)
									break;
			
								APSendPacket(pAd, pPacket2);	
							}
						}
			APSendPacket(pAd, pPacket);
		}
	...
	IN	PNDIS_PACKET	pPacket)
{
	...
	*/
	UserPriority = 0;
	QueIdx		 = QID_AC_BE;
	if (RTMPCheckEtherType(pAd, pPacket, pMacEntry, OPMODE_AP, &UserPriority, &QueIdx, &Wcid) == FALSE)
	{
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
		return NDIS_STATUS_FAILURE;
	}
...
/*FOR DMS */
		if (pTxBlk->Wcid != MCAST_WCID)
		{	/*		printk(" build header %02x:%02x:%02x:%02x:%02x:%02x \n",pTxBlk->pMacEntry->Addr[0], */
			/*			pTxBlk->pMacEntry->Addr[1],pTxBlk->pMacEntry->Addr[2],pTxBlk->pMacEntry->Addr[3], */
			/*			pTxBlk->pMacEntry->Addr[4],pTxBlk->pMacEntry->Addr[5]); */
			COPY_MAC_ADDR(pHeader_802_11->Addr1, pTxBlk->pMacEntry->Addr);/* DA */
		}
		else
#endif /* DOT11V_WNM_SUPPORT */
						  ...
						  IN  ULONG           nonBEpackets)
{
	...
	)
	{
		...
		else
		{
			...
			{
				/* enable AC0(BE) TX_OP */
				UCHAR	txop_value_burst = 0x30;	/* default txop for Tx-Burst */
				UCHAR   txop_value;
...
#ifdef IDS_SUPPORT
		if ((pHeader->FC.FrDs == 0) && (pRxWI->WirelessCliID == RESERVED_WCID)) /* not in RX WCID MAC table */
		{
...
#ifdef TCSUPPORT_WLAN_PERSSID_RATELIMIT
	if(FromWhichBSSID<min(MAX_MBSSID_NUM(pAd), WLAN_PORTS_NUM)) {
		if(wlan_ratelimit[(FromWhichBSSID<<1)+1] != 0) {
			if(!wlan_pktqosCheckToken(&wlan_pktqosToken[(FromWhichBSSID<<1)+1], wlan_ratelimit[(FromWhichBSSID<<1)+1], pRxBlk->DataSize)) {
				/* release packet */
				RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
				return;
			}
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pRxBlk->pData, pRxBlk->DataSize, FromWhichBSSID))
	{
			...
			pEntry->pMbss->WNMCtrl.ProxyARPEnable) 
		{
			...
			if (IsIpv6DuplicateAddrDetect(pAd, pData, &Offset))
			{
				...
				printk("AddIPv6ProxyARPEntry:offset=%d\n", Offset);
	...
	else 
	{
...
#endif /* DOT11_N_SUPPORT */
		{
			...
			if (pEntry)
			{
			    ...
			    if ((*(pData+1) == EAPOLStart) && (pMbss->HotSpotCtrl.HotSpotEnable == 1) && (pMbss->AuthMode == Ndis802_11AuthModeWPA2) && (pEntry->hs_info.ppsmo_exist == 1))
				{
