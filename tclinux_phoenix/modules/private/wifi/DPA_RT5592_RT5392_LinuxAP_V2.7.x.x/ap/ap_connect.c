...
#ifdef DYNAMIC_RX_RATE_ADJ
	if ((pAd->ApCfg.MBSSID[apidx].ExtRateLen) && (PhyMode != PHY_11B))
#else
	...
	IN INT				apidx) 
{
	...
	*(ptr + 4) |= (pMbss->TimBitmaps[WLAN_CT_TIM_BCMC_OFFSET] & 0x01); 
	...
	else if (pMbss->AuthMode >= Ndis802_11AuthModeWPA)
	{
		...
		if ((pMbss->HotSpotCtrl.HotSpotEnable == 0) && (pMbss->HotSpotCtrl.bASANEnable == 1) && (pMbss->AuthMode == Ndis802_11AuthModeWPA2))
		{
			RSNIe = IE_WPA;
			MakeOutgoingFrame(pBeaconFrame+FrameLen,        &TmpLen,
						  1,                            &RSNIe,
...
/*		USHORT	b2lTmp, b2lTmp2; // no use */
	...
	*/
...
#if defined(DOT11N_DRAFT3) || defined(CONFIG_DOT11V_WNM)
	/* 7.3.2.27 Extended Capabilities IE */
	{
		...
		{
			extCapInfo.ProxyARPSupport = 1;
		}
#ifdef CONFIG_HOTSPOT_R2		
		if (pMbss->WNMCtrl.WNMNotifyEnable)
		{
			extCapInfo.WNMNotifySupport = 1;
		}
    ...
    IN PRTMP_ADAPTER pAd)
{
    ...
    /* do shared mem mapping check, do Mailbox check
     * if MCU is processing command, please wait..
     */
	...
	RTMP_IO_READ32(pAd, PBF_SYS_CTRL, &Val);

    do
    {
        RTMP_IO_READ32(pAd, H2M_MAILBOX_CSR, &H2MMailbox.word);
        /* check if MCU command is done */
	...
	RTMP_IO_READ32(pAd, PBF_SYS_CTRL, &Val2);

    if (i > 0)
    {
        DBGPRINT(RT_DEBUG_ERROR,("MCU is processing command. count %d , shared mem select (%x/%x)\n", i, Val, Val2));
    }

	if (Val2 & (1UL << 19))
		DBGPRINT(RT_DEBUG_ERROR,("%s: already in high memory %x\n", __FUNCTION__, Val2));

	if ((i >= 100) || (Val2 & 0x4000))
	{
		UINT32 val, val2, val3;

	    DBGPRINT_ERR(("H2M_MAILBOX still hold by MCU(%d). polling fail in ISR(%x)\n", 
			H2MMailbox.field.Owner,
			in_interrupt()));

		RTMP_IO_READ32(pAd, H2M_MAILBOX_CSR, &val);
		RTMP_IO_READ32(pAd, H2M_MAILBOX_CID, &val2);
		RTMP_IO_READ32(pAd, H2M_MAILBOX_STATUS, &val3);

		DBGPRINT(RT_DEBUG_ERROR,("%s: before H2M_MAILBOX_CSR %x/%x, H2M_MAILBOX_CID %x, H2M_MAILBOX_STATUS %x\n", 
		__FUNCTION__, H2MMailbox.word, val, val2, val3));
