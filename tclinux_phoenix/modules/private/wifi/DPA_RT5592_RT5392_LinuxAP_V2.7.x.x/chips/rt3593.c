...
RTMP_REG_PAIR	RT3593_MACRegTable[] =	{
    ...
    {AMPDU_MAX_LEN_20M1S, 0x66655443}, /* WCNAE00087166 */
	{AMPDU_MAX_LEN_40M1S, 0x77766554}, /* WCNAE00087166 */
};
...
static VOID RT3593_SpursEnhance(PRTMP_ADAPTER pAd, UCHAR Channel)
{
	UCHAR BbpReg = 0;
	USHORT value = 0;
	BOOLEAN isOn = FALSE;
	...
	/*Read eFuse to Turn on/off Bit7 for filter */
	RT28xx_EEPROM_READ16(pAd, (0x34), value);
	value = (value & ~0x0FFF) >> 12;
	
	if (Channel > 14)
	{
		//[15:14] 5G Tx high Power setting
		isOn = ((value >> 2) == 0x01);
		printk("5. %d\n", (value >> 2));
	...
	{
		//[13:12] 2.4G Tx high Power setting
		isOn = ((value & ~0xc) == 0x01);
		printk("2. %d\n", (value & ~0xc));
	}
	
	printk("%s: e2p[%02x]  ===> %d, %d\n", __FUNCTION__, value, Channel, isOn);	
	...
	IN	BOOLEAN						bScan)
{
	...
	/*UCHAR PreRFValue = 0;*/
	RT3593_SpursEnhance(pAd, Channel);
	/*
...
#ifdef ED_MONITOR
INT RT3593_set_ed_cca(RTMP_ADAPTER *pAd, BOOLEAN enable)
{
	UINT32 mac_val;
	UCHAR bbp_val = 0;
	...
	if (enable) {
		...
		bbp_val = (pAd->CommonCfg.Channel > 14) ? \
				  0x9c : 0x9a; // bit 0~3 for threshold
