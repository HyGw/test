...
#define AP_DRIVER_VERSION			"2.7.1.7"
#ifdef MULTIPLE_CARD_SUPPORT
#ifdef BB_SOC
#define PROFILE_PATH_1			"/etc/Wireless/RT2860AP/RT2860AP0.dat"
#define PROFILE_PATH_2			"/etc/Wireless/RT2860AP/RT2860AP1.dat"
#else
#define PROFILE_PATH_1			"/etc/Wireless/RT2860/RT2860.dat"
...
typedef struct _OS_FS_INFO_
{
	...
	int				fsgid;
#else
	kuid_t			fsuid; 	/* saved UID of the task */
	kgid_t			fsgid; 	/* saved GID of the task */
#endif
	mm_segment_t	fs;
} OS_FS_INFO;
...
//bug fix 12258. shnwind 1209
extern char check_rt63365ver1(void);
#define RTMP_IO_READ32(_A, _R, _pV)								\
{																\
    if ((_A)->bPCIclkOff == FALSE)                                  \
    {                                                               \
		(*(_pV) = readl((void *)((_A)->CSRBaseAddress + (_R))));			\
		if((check_rt63365ver1() == 1) && (_R != MAC_CSR0) && ((*_pV & 0xfff00000)== 0x53900000)){ \
			(*(_pV) = readl((void *)((_A)->CSRBaseAddress + (_R))));			\
		} \    
    }                                                               \
    else															\
...
#define RTMP_GET_PACKET_DISASSOC(_p)        (RTPKT_TO_OSPKT(_p)->cb[CB_OFF+11] & RTMP_PACKET_SPECIFIC_DISASSOC)
