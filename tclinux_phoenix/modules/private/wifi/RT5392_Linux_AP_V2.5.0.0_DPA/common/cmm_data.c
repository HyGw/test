		...
		else
		{
			...
			if (bUseDataQ)
			{
				...
				if (retryCnt == 0)
				{
					...
					DBGPRINT(RT_DEBUG_ERROR, ("Qidx(%d)(bUseDataQ: %d), not enough space in DataRing, MgmtRingFullCount=%ld!\n",
											QueIdx, bUseDataQ, pAd->RalinkCounters.MgmtRingFullCount));
				}
			}
#endif // RTMP_MAC_PCI //
			//DBGPRINT(RT_DEBUG_ERROR, ("Qidx(%d), not enough space in MgmtRing, MgmtRingFullCount=%ld!\n",
				//						QueIdx, pAd->RalinkCounters.MgmtRingFullCount));
			DBGPRINT(RT_DEBUG_ERROR, ("Qidx(%d)(bUseDataQ: %d), not enough space in MgmtRing, MgmtRingFullCount=%ld!\n",
								QueIdx, bUseDataQ, pAd->RalinkCounters.MgmtRingFullCount));
		}
	} while (retryCnt > 0);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
	RTMP_INT_LOCK(&pAd->MgmtRingLock, IrqFlags);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
			RTMP_INT_UNLOCK(&pAd->MgmtRingLock, IrqFlags);
#else
	...
	IN  PRTMP_ADAPTER  pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		for (apidx = MAIN_MBSSID; apidx < pAd->ApCfg.BssidNum; apidx++)
		{
			...
			pAd->ApCfg.MBSSID[apidx].WscControl.WscState = WSC_STATE_INIT;
			pAd->ApCfg.MBSSID[apidx].WscControl.WscRetryCount = 0;
#endif // WSC_AP_SUPPORT //
	...
	PMAC_TABLE_ENTRY entry)
{
	...
	NdisAllocateSpinLock(&entry->drop_mask_lock);
	...
	BOOLEAN				enable)
{
	...
	DBGPRINT(RT_DEBUG_INFO, ("%s(): Aid = %u, type = %u, enable = %u\n",
				__FUNCTION__, entry->Aid, type, enable));
#endif
