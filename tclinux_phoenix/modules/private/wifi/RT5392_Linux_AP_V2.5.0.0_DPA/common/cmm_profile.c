...
static void rtmp_read_key_parms_from_file(IN  PRTMP_ADAPTER pAd, PSTRING tmpbuf, PSTRING buffer)
{
	...
	PSTRING		macptr;						
	INT			i = 0, idx;
	ULONG		KeyType[MAX_MBSSID_NUM(pAd)];
	ULONG		KeyIdx;
	...
	IN PSTRING	pBuffer)
{
	...
	do
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			if(RTMPGetKeyParameter("BssidNum", tmpbuf, 25, pBuffer, TRUE))
			{
				...
				if(pAd->ApCfg.BssidNum > MAX_MBSSID_NUM(pAd))
				{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("BssidNum=%d(MAX_MBSSID_NUM(pAd) is %d)\n", pAd->ApCfg.BssidNum,MAX_MBSSID_NUM(pAd)));
				}
				else
		...
		/* 
			For RT5392 temperature compensation, always turn the SW algorithm on.
			If users do not need this function, suggest to turn off the EEPROM switch (0x36 bit1) rather than the SW one.
		*/
		if (IS_RT5392(pAd))
			pAd->CommonCfg.TempComp = 2;
...
//DFSIndoor
	...
	//#endif
		...
		if (RTMPGetKeyParameter("EDCCA_AP_STA_TH", tmpbuf, 32, pBuffer, TRUE))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("pAd->ed_sta_threshold = %u\n", count));
		}
		...
		if (RTMPGetKeyParameter("EDCCA_AP_AP_TH", tmpbuf, 32, pBuffer, TRUE))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("pAd->ed_ap_threshold = %u\n", count));
		}
		...
		if (RTMPGetKeyParameter("EDCCA_AP_RSSI_TH", tmpbuf, 32, pBuffer, TRUE))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("pAd->ed_rssi_threshold = %u\n", count));
		}		
#endif /* CONFIG_AP_SUPPORT */
		...
		if (RTMPGetKeyParameter("EDCCA_STA_SCANED_AP_TH", tmpbuf, 32, pBuffer, TRUE))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("pAd->ed_ap_scaned = %u\n", count));
		}
		...
		if (RTMPGetKeyParameter("EDCCA_STA_CURRENT_CH_APS_TH", tmpbuf, 32, pBuffer, TRUE))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("pAd->ed_current_ch_aps = %u\n", count));
		}
		...
		if (RTMPGetKeyParameter("EDCCA_FALSE_CCA_TH", tmpbuf, 32, pBuffer, TRUE))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("pAd->false_cca_threshold = %u\n", count));
		}
		...
		if (RTMPGetKeyParameter("EDCCA_BLOCK_CHECK_TH", tmpbuf, 32, pBuffer, TRUE))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("pAd->ed_block_tx_threshold = %u\n", count));
		}
#endif /* ED_MONITOR */			
		//ShortSlot
		...
		if(RTMPGetKeyParameter("EncrypType", tmpbuf, 128, pBuffer, TRUE))
		{
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			{
				...
				//WscSetupLockTimer
				if (RTMPGetKeyParameter("WscSetupLockTimer", tmpbuf, 32, pBuffer,TRUE))
				{
					...
					IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
					{
						for (i = 0; i < MAX_MBSSID_NUM(pAd); i++)
						{
							...
							pWscControl->SetupLockTime = (UINT)simple_strtol(tmpbuf, 0, 10);
							DBGPRINT(RT_DEBUG_ERROR, ("%s - WscSetupLockTimer= (%d)\n", 
								__FUNCTION__, pWscControl->SetupLockTime));
						}       
					}               
#endif /* CONFIG_AP_SUPPORT */
				}       
				...
				//WscMaxPinAttack
				if (RTMPGetKeyParameter("WscMaxPinAttack", tmpbuf, 32, pBuffer,TRUE))
				{
					PWSC_CTRL pWscControl;
					int i = 0;
#ifdef CONFIG_AP_SUPPORT
					IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
					{
						...
						{
							pWscControl = &pAd->ApCfg.MBSSID[i].WscControl;                                               
							pWscControl->MaxPinAttack = (UCHAR)simple_strtol(tmpbuf, 0, 10);
							DBGPRINT(RT_DEBUG_ERROR, ("%s - WscMaxPinAttack= (%d)\n", 
								__FUNCTION__, pWscControl->MaxPinAttack));
						}       
					}               
#endif /* CONFIG_AP_SUPPORT */                                  
				}
