	...
	if (IS_RT5392(pAd))
	{
		...
		if (Channel <= 14)
		{
			...
			if ( (E2PValue & 0x00FF) != 0x00FF )
                                  ...
                                  RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R159, E2PValue & 0x00);
...
#ifdef RT30xx //RT33xx
//#ifdef RTMP_MAC_PCI //Refer to windows driver, USB and PCI need this part.
...
//					Value = (CHAR)((TxPwr[i] >> j*4) & 0x0F);
					...
					if (j < 4)
					{
						// CCK will have 3dBm larger than OFDM
						AdjustMaxTxPwr[i*8+j] = TxPwrInEEPROM + (Value - criterion) + 3;
					}
					else
					{
...
//			RTMP_INT_UNLOCK(&pAd->irq_lock, flags);  
	...
	RT539x_AsicExtraPowerOverMAC(pAd);
