	...
	OUT	STATE_MACHINE_FUNC	Trans[])	
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		for (apidx = 0; apidx < MAX_MBSSID_NUM(pAd); apidx++)
		{
			...
			WSC_TIMER_INIT(pAd, pWScControl, &pWScControl->WscPinAttackCountCheckTimer, pWScControl->WscPinAttackCountCheckTimerRunning, WscPinAttackCountCheckTimeout);
		}
    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
	...
	pWscPeer = WscFindPeerEntry(pAd, pEntry->apidx, pEntry->Addr);
	if (pWscPeer)
	{
	...
	else
	{
		...
		if (pWscPeer)
		{
			...
			NdisMoveMemory(pWscPeer->mac_addr, pEntry->Addr, MAC_ADDR_LEN);
			NdisGetSystemUpTime(&pWscPeer->receive_time);
			insertTailList(&pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.WscPeerList, 
							(PLIST_ENTRY)pWscPeer);
		}
		ASSERT(pWscPeer != NULL);
	}

	WscMaintainPeerList(pAd, pEntry->apidx);
...
{		
	...
	else if (MsgType == WSC_MSG_EAP_REG_RSP_ID)
	{
	...
	else if (MsgType == WSC_MSG_M1)
	{
		UINT32 rv = 0;
		/*
			If Buffalo WPS STA doesn't receive M2D from AP, Buffalo WPS STA will stop 
to do WPS.
	...
	else if (MsgType == WSC_MSG_WSC_ACK)
	{
		DBGPRINT(RT_DEBUG_TRACE, ("WscState: %d\n", pWscControl->WscState));
		if (((pWscControl->WscConfMode & WSC_REGISTRAR) != 0) && 
			/*(pWscControl->bWscTrigger && !pWscControl->bWscAutoTigeer) &&*/
...
#ifdef CONFIG_AP_SUPPORT
			/*
				We don't need to forward this ACK to ER.
			*/
	...
	else if (MsgType == WSC_MSG_WSC_NACK)
	{
			...
			(WscCheckNonce(pAdapter, Elem, FALSE, pWscControl) || WscCheckNonce(pAdapter, Elem, TRUE, pWscControl)))
		{
			...
			*/
			...
			if ((pAdapter->OpMode == OPMODE_AP) && bReSetWscIE)
			{
				WscBuildBeaconIE(pAdapter, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, (pWscControl->EntryIfIdx & 0x0F), NULL, 0);
				WscBuildProbeRespIE(pAdapter, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, (pWscControl->EntryIfIdx & 0x0F), NULL, 0);
		...
		case WSC_MSG_M6:
			...
			{      
				...
				else
				{
					...
					if (pEntry)
						WscDelListEntryByMAC(pAdapter, pEntry->apidx, pEntry->Addr);
				}
			}
			break;
...
#ifdef WSC_V2_SUPPORT
			|| (pWscControl->WscV2Info.bEnableWpsV2 && (!pWscControl->bSetupLock))
#endif // WSC_V2_SUPPORT //
			)
		{
			pWscControl->WscStatus = STATUS_WSC_CONFIGURED;
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
			{
				...
				if (bUPnPMsg || (pEntry && IS_ENTRY_CLIENT(pEntry)))
				{
					WscBuildBeaconIE(pAdapter, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, pWscControl->EntryIfIdx, NULL, 0);
					WscBuildProbeRespIE(pAdapter, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, pWscControl->EntryIfIdx, NULL, 0);
				}
			}
#endif // CONFIG_AP_SUPPORT //
	...
	switch (MsgType)
	{
		case WSC_MSG_M1:
			...
			if (pWscControl->WscState == WSC_STATE_WAIT_M1)
			{
				...
				{
					/* Process M1 and send M2D without tigeer */
					if (((pWscControl->WscMode == 1) && (pWscControl->WscPinCode == 0)) ||
						(((pWscControl->bWscTrigger == FALSE) || (pWscControl->bWscAutoTigeer == TRUE))))
...
#ifdef WSC_V2_SUPPORT
						if (pWscControl->WscV2Info.bEnableWpsV2 && (pAdapter->OpMode == OPMODE_AP))
...
Fail:
	...
	if (pWscControl->WscState == WSC_STATE_CONFIGURED)
	{
		...
		else
		{
			if (pAdapter->OpMode == OPMODE_AP)
			{				
				WscBuildBeaconIE(pAdapter, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, pEntry->apidx, NULL, 0);
				WscBuildProbeRespIE(pAdapter, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, pEntry->apidx, NULL, 0);
				APMakeAllBssBeacon(pAdapter);
				APUpdateAllBeaconFrame(pAdapter);
			}
		}
    ...
    IN  PWSC_CTRL           pWscControl)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
			...
			((nWscState == WSC_STATE_WAIT_DONE) || (nWscState == WSC_STATE_WAIT_ACK)))
		{
			pWscControl->bWscTrigger = FALSE;
			pWscControl->WscConfStatus = WSC_SCSTATE_CONFIGURED;
			WscBuildBeaconIE(pAd, pWscControl->WscConfStatus, FALSE, WscMode, pWscControl->WscConfigMethods, pWscControl->EntryIfIdx, NULL, 0);
			WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, pWscControl->WscConfStatus, FALSE, WscMode, pWscControl->WscConfigMethods, pWscControl->EntryIfIdx, NULL, 0);
	...
	if (pWscControl != NULL)
	{
		...
		if (pWscControl->Wsc2MinsTimerRunning)
		{
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			{
				...
				{
					WscBuildBeaconIE(pWscControl->pAd, IsAPConfigured, FALSE, 0, 0, pWscControl->EntryIfIdx, NULL, 0);
					WscBuildProbeRespIE(pWscControl->pAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, FALSE, 0, 0, pWscControl->EntryIfIdx, NULL, 0);
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
	{
    	...
    	if (pData && (Len > 0))
    	{
    		if (pWscControl->bWscFragment && (pWscControl->bWscFirstOne))
    		{
				...
				FrameLen += TmpLen;
				hex_dump("WscSendMessage", pOutBuffer, FrameLen);
    		}
			else
			{
				MakeOutgoingFrame(pOutBuffer, &FrameLen,
...
//	UCHAR 			Data[256];
	...
	if ((pAd->OpMode == OPMODE_AP) && pWpsCtrl->bSetupLock)
	{
		// AP Setup Lock
	...
	if (pWpsCtrl->WscV2Info.bEnableWpsV2)
	{
		...
		os_alloc_mem(NULL, (UCHAR **)&pWscV2Msg, 512);
		if (pWscV2Msg)
		{
			UCHAR TmpLen = 0;
			pWscV2Msg[0] = 0x00;
			...
			if (b_selRegistrar)
			{
				...
				if (pAuthorizedMACs)
				{
					WscGenV2Msg(WFA_EXT_ID_AUTHORIZEDMACS, 
								pAuthorizedMACs, 
								AuthorizedMACsLen, 
	...
	IN  UCHAR	apidx)
{
				...
				if (pWscCtrl->WscV2Info.bEnableWpsV2 && (WscLen > 0))
				{
					...
					*/
					...
					if (pAuthorizedMACs)
					{
						...
						WscParseV2Msg(WFA_EXT_ID_AUTHORIZEDMACS, pData, WscLen, pAuthorizedMACs, &AuthorizedMACsLen);
					}
				}				
#endif // WSC_V2_SUPPORT //
				break;
	...
	if (wsc_sel_reg == 0x01)
	{
		...
		WscBuildBeaconIE(pAd, WSC_SCSTATE_CONFIGURED, TRUE, wsc_dev_pass_id, wsc_sel_reg_conf_mthd, apidx, pAuthorizedMACs, AuthorizedMACsLen);
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, TRUE, wsc_dev_pass_id, wsc_sel_reg_conf_mthd, apidx, pAuthorizedMACs, AuthorizedMACsLen);
#ifdef WSC_V2_SUPPORT		
	...
	else
	{
		...
		WscBuildBeaconIE(pAd, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, apidx, NULL, 0);
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, apidx, NULL, 0);
	}
	...
	IN  UCHAR       	BssIndex)
{
...
#endif // CONFIG_AP_SUPPORT //
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			{
				IsAPConfigured = pWscControl->WscConfStatus;
				WscBuildBeaconIE(pAd, IsAPConfigured, FALSE, 0, 0, BssIndex, NULL, 0);
				WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, FALSE, 0, 0, BssIndex, NULL, 0);
    ...
    IN NDIS_802_11_WEP_STATUS encryType)
{
	switch(encryType)
	{
		...
		case Ndis802_11Encryption4Enabled:
			return WSC_ENCRTYPE_AES; //To fix the Win7 PBC not connect issue
		 default:
    ...
    IN  USHORT authFlag)
{
	switch(authFlag)
	{
		...
		case (WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK):
			return "WPA2PSK"; //To fix the Win7 PBC not connect issue
...
#ifdef CONFIG_AP_SUPPORT
#ifdef WSC_V2_SUPPORT
			if ((pAd->OpMode == OPMODE_AP) && (pWscControl->WscV2Info.bEnableWpsV2))
			{
	...
	IN  BOOLEAN         bFromUPnP)
{
	...
	WscBuildBeaconIE(pAd, IsAPConfigured, TRUE, WscMode, pWscControl->WscConfigMethods, pWscControl->EntryIfIdx, NULL, 0);
	WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, TRUE, WscMode, pWscControl->WscConfigMethods, pWscControl->EntryIfIdx, NULL, 0);
    ...
    IN ULONG Context)
{
	...
	while (pTask && !pTask->task_killed)
	{
		...
		if (pAd && pAd->pWscElme && (pAd->pWscElme->MsgLen != 0))
		{
...
{	
	...
	{
		INT ap_idx;
		for (ap_idx = 0; ap_idx < MAX_MBSSID_NUM(pAd); ap_idx++)
		{
...
NDIS_STATUS WscThreadInit(RTMP_ADAPTER *pAd)
{
	...
	status = RtmpOSTaskAttach(pTask, write_dat_file_thread, (ULONG)&pAd->wscTask);
	if (status == NDIS_STATUS_SUCCESS)
	{
		...
		NdisAllocateSpinLock(&pAd->WscElmeLock);
	...
	OUT	PUCHAR			pOutBufLen)
{
	...
	hex_dump("WscParseV2Msg - pData", (pData+3), DataLen-3);
	...
	IN PVOID SystemSpecific3)
{
	...
	pMbss = &pAd->ApCfg.MBSSID[pWscControl->EntryIfIdx];
...
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		pWscEnList = &pAd->ApCfg.MBSSID[ApIdx].WscControl.WscPeerList;
#endif // CONFIG_AP_SUPPORT //
	...
	IN  UCHAR			ApIdx)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("WscClearPeerList:: ApIdx = %d\n", ApIdx));
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		pWscEnList = &pAd->ApCfg.MBSSID[ApIdx].WscControl.WscPeerList;
#endif // CONFIG_AP_SUPPORT //
...
{
	PLIST_ENTRY	pListEntry = NULL;
	pListEntry = (PLIST_ENTRY)WscFindPeerEntry(pAd, ApIdx, pMacAddr);
	if (pListEntry)
	{
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		delEntryList(&pAd->ApCfg.MBSSID[ApIdx].WscControl.WscPeerList, pListEntry);
#endif // CONFIG_AP_SUPPORT //
...
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		pWpsCtrl = &pAd->ApCfg.MBSSID[ap_idx].WscControl;
#endif // CONFIG_AP_SUPPORT //

	WscMaintainPeerList(pAd, ap_idx);
	...
	IN PVOID SystemSpecific3)
{
	...
	pWscControl->WscSetupLockTimerRunning = FALSE;
	WscBuildBeaconIE(pAd, pWscControl->WscConfStatus, FALSE, 0, 0, (pWscControl->EntryIfIdx & 0xF), NULL, 0);
	WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, pWscControl->WscConfStatus, FALSE, 0, 0, (pWscControl->EntryIfIdx & 0xF), NULL, 0);
	APMakeAllBssBeacon(pAd);
	APUpdateAllBeaconFrame(pAd);
