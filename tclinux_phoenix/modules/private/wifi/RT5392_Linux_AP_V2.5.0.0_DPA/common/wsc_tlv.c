...
static WSC_TLV_0B wsc_tlv_0b[]=
{
	...
	{/*0,*/0},
	{/*0,*/0},
	/*<Reserved for WFA> 0x1068 ¡V 0x1FFF*/
	...
	OUT	VOID *pbuf)
{
	...
	if (pWscControl->WscV2Info.bEnableWpsV2)
	{
		...
		if (pWscV2Msg)
		{
			...
			pWscV2Msg[1] = 0x37;
			pWscV2Msg[2] = 0x2A;
			WscGenV2Msg(WFA_EXT_ID_VERSION2, &pReg->SelfInfo.Version2, 1, pWscV2Msg+3, &TmpLen);
			WscV2MsgLen = (TmpLen+3); // Length of WFA vendor ID
			templen = AppendWSCTLV(WSC_ID_VENDOR_EXT, pData, (UINT8 *)pWscV2Msg, WscV2MsgLen);
			pData += templen;
			Len   += templen;
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_DEVICE_PWD_ID:
				...
				if(htons(WSC_DEVICEPWDID_DEFAULT) == get_unaligned((PUSHORT) pData))//*(PUSHORT) pData)
				{
					DBGPRINT(RT_DEBUG_TRACE, ("Rx WPS           DPID PIN\n"));
					pWscControl->RegData.SelfInfo.DevPwdId = cpu2be16(DEV_PASS_ID_PIN);
				}
				else if(htons(WSC_DEVICEPWDID_PUSH_BTN) == get_unaligned((PUSHORT) pData))//*(PUSHORT) pData)
				{
			...
			case WSC_ID_VENDOR_EXT:
				...
				if (pWscControl->WscV2Info.bEnableWpsV2)
				{
					...
					WscParseV2Msg(WFA_EXT_ID_VERSION2, pData, WscLen, &pReg->PeerInfo.Version2, &tmp_data_len);
