...
#ifdef RTMP_MAC_PCI
#define FIRMWARE_MINOR_VERSION		30
#endif // RTMP_MAC_PCI //
...
#define NICLF_DEFAULT_USE()	\
	...
	{
#ifdef RTMP_MAC_PCI
		if ((Version == 0x2860) || (Version == 0x3572) || IS_RT3090(pAd)||IS_RT3390(pAd) || IS_RT3593(pAd) || IS_RT5390(pAd))
		{
			pFirmwareImage = FirmwareImage;
...
#ifdef RTMP_MAC_PCI
if (bNeedMcuCmdLock == TRUE)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
			RTMP_INT_LOCK(&pAd->McuCmdLock, IrqFlags);
#else
...
if (bNeedMcuCmdLock == TRUE)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
			RTMP_INT_UNLOCK(&pAd->McuCmdLock, IrqFlags);
#else
