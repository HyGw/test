	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (src && *src)
	{
		...
		else 
		{
				...
				memset(pAd->EEPROMImage, 0xff/*0x00*/, MAX_EEPROM_BIN_FILE_SIZE);
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	UINT EfuseUsageMapSize = 0;
	
	if (IS_RT5390R(pAd))
	{
		EfuseUsageMapSize = 24; /* 29 - 5 */
		DBGPRINT(RT_DEBUG_ERROR, ("RT5390R::NVM is Efuse and its size =%x[%x-%x]\n", 
										29, EFUSE_USAGE_MAP_START, EFUSE_USAGE_MAP_END));
	}
	else
	{
		EfuseUsageMapSize = EFUSE_USAGE_MAP_SIZE - 5;
	...
	if (EfuseFreeBlock > EfuseUsageMapSize)
	{
