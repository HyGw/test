							...
							if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID)
							{
								...
								else if (DevicePasswordID == DEV_PASS_ID_PIN)
								{
									...
									*/
									...
									{
									#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
										RTMP_INT_LOCK(&pWscCtrl->WscPeerListSemLock, IrqFlags);
									#else
										RTMP_SEM_LOCK(&pWscCtrl->WscPeerListSemLock);
									#endif
										pWscPeer = WscFindPeerEntry(pAd, apidx, Fr->Hdr.Addr2);
									#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
										RTMP_INT_UNLOCK(&pWscCtrl->WscPeerListSemLock, IrqFlags);
									#else
									...
									{
										os_alloc_mem(pAd, &pWscPeer, WSC_PEER_ENTRY_SIZE);
										if (pWscPeer)
										{
											NdisZeroMemory(pWscPeer, WSC_PEER_ENTRY_SIZE);
											NdisMoveMemory(pWscPeer->mac_addr, Fr->Hdr.Addr2, MAC_ADDR_LEN);
											NdisGetSystemUpTime(&pWscPeer->receive_time);
											insertTailList(&pAd->ApCfg.MBSSID[apidx].WscControl.WscPeerList, 
...
#ifdef CONFIG_AP_SUPPORT
						if (PeerWscIeLen >= 512)
							DBGPRINT(RT_DEBUG_ERROR, ("PeerProbeReqSanity : PeerWscIeLen = %d (>= 512)\n", PeerWscIeLen));
			...
			*/
			...
			if(pBuf)
			{
				...
				NdisZeroMemory(pBuf, bufLen);
					...
					pprobreq->data[0] = PeerWscIeLen>>8; 					// Wsc IE length high bytes
					pprobreq->data[1] = PeerWscIeLen & 0xff;				// Wsc IE lemgth low bytes					
