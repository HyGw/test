...
void ieee802_1x_tx_key(rtapd *rtapd, struct sta_info *sta, u8 id)
{
	...
	if (sta->ethertype== ETH_P_PRE_AUTH)
	{
		if (RT_ioctl(rtapd, RTPRIV_IOCTL_ADD_PMKID_CACHE, (char *)&WepKey, sizeof(NDIS_802_11_KEY), 0, 0))
		{
	...
	{
		if (RT_ioctl(rtapd, RTPRIV_IOCTL_ADD_WPA_KEY, (char *)&WepKey, sizeof(NDIS_802_11_KEY), sta->ApIdx, 0))
		{
