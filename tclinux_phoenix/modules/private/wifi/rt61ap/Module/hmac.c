...
int hmac_sha_key(const unsigned char key[], u32 key_len, hmac_ctx cx[1])
{
...
void hmac_sha_data(const unsigned char data[], u32 data_len, hmac_ctx cx[1])
{   unsigned int i;
...
void hmac_sha_end(unsigned char mac[], u32 mac_len, hmac_ctx cx[1])
{   unsigned char dig[HASH_OUTPUT_SIZE];
    ...
    for(i = 0; i < (HASH_INPUT_SIZE >> 2); ++i)
        ((u32*)cx->key)[i] ^= 0x36363636 ^ 0x5c5c5c5c;
...
void hmac_sha(const unsigned char key[], u32 key_len,
          const unsigned char data[], u32 data_len,
          unsigned char mac[], u32 mac_len)
{   hmac_ctx    cx[1];
