	...
	IN	UINT			Len)
{
	...
	//2007/07/26:Modified by KH 
	//Alter "pAdapter->PortCfg.MBSSID[MAIN_MBSSID].SharedKey[KeyIdx].KeyLen " to
	if (pAdapter->ApCliTab.ApCliEntry[0].SharedKey[KeyIdx].KeyLen == 0)
		return (FALSE);

	NdisMoveMemory(WEPKEY + 3, pAdapter->ApCliTab.ApCliEntry[0].SharedKey[KeyIdx].Key, pAdapter->ApCliTab.ApCliEntry[0].SharedKey[KeyIdx].KeyLen);	
	//2007/07/26:Modified by KH 
	//Alter "pAdapter->PortCfg.MBSSID[MAIN_MBSSID].SharedKey[KeyIdx].KeyLen " to
	ARCFOUR_INIT(&pAdapter->PrivateInfo.WEPCONTEXT, WEPKEY, pAdapter->ApCliTab.ApCliEntry[0].SharedKey[KeyIdx].KeyLen+ 3);
