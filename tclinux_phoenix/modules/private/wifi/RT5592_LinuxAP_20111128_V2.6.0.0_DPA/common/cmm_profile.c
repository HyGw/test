...
/*DFSIndoor*/
	{
		...
		{
			pRadarDetect->bDFSIndoor = (USHORT) (simple_strtol(tmpbuf, 0, 10) != 0);
			DBGPRINT(RT_DEBUG_TRACE, ("DfsIndoor=%d\n", pRadarDetect->bDFSIndoor));
		}
		{
			INT k=0;
		/*SymRoundFromCfg*/
...
#ifdef DFS_SUPPORT
	{
		...
		if(RTMPGetKeyParameter("CSPeriod", tmpbuf, 10, pBuffer, TRUE))
		{
		    if(simple_strtol(tmpbuf, 0, 10) != 0)
				pRadarDetect->CSPeriod = simple_strtol(tmpbuf, 0, 10);
			else
				pRadarDetect->CSPeriod = 0;
