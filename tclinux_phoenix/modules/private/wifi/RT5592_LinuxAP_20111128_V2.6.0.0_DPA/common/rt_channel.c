...
/*New FCC spec restrict the used channel under DFS */
		...
		if ((pAd->CommonCfg.bIEEE80211H == 1) &&
			...
			(pAd->CommonCfg.RadarDetect.bDFSIndoor == 1))
		{
			if ((channel >= 116) && (channel <=128))
				continue;
				 ...
				 (pAd->CommonCfg.RDDurRegion == FCC) &&
				 (pAd->CommonCfg.RadarDetect.bDFSIndoor == 0))
		{
	...
	IN UCHAR Geography)
{
	...
	for (i = 0; pChRegion->ChDesp[i].FirstChannel != 0; i++)
	{
...
static BOOLEAN N_ChannelGroupCheck(
	IN PRTMP_ADAPTER pAd,
	IN UCHAR Channel)
{
...
{	
	...
	for (i = 0; i < pAd->ChannelListNum; i++)
	{
		if (N_ChannelGroupCheck(pAd, pAd->ChannelList[i].Channel))
		{
			pAd->ChannelListN[j].Channel = pAd->ChannelList[i].Channel;
			pAd->ChannelListN[j].IdxMap = i;			
			DBGPRINT(RT_DEBUG_TRACE,
				("BuildSubListN, Channel = %u, IdxMap = %u\n",
				pAd->ChannelListN[j].Channel, pAd->ChannelListN[j].IdxMap));
			j++;
		}
	}
	if (j == 0)
		DBGPRINT(RT_DEBUG_TRACE, ("No entry in sub channel list.\n"));
	
	if (j < MAX_NUM_OF_SUB_CHANNELS)
		pAd->ChannelListN[j].Channel = 0; /* End point indication*/
}
