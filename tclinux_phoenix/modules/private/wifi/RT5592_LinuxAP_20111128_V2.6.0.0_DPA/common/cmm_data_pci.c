	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->OpMode == OPMODE_AP)
	{
			...
			&& (pAd->CommonCfg.RadarDetect.RDMode == RD_SWITCHING_MODE))
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("RTMPHandlePreTBTTInterrupt::Channel Switching...(%d/%d)\n", pRadarDetect->CSCount, pRadarDetect->CSPeriod));
			
			pRadarDetect->CSCount++;
			if (pRadarDetect->CSCount >= pRadarDetect->CSPeriod)
			{
	...
	IN OUT	UINT32			*pRxPending)
{
	...
	RTMP_INT_LOCK(&pAd->RxRingLock, IrqFlags);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
	RTMP_INT_UNLOCK(&pAd->RxRingLock, IrqFlags);
#else
