...
#ifdef RT5592
		{4, 8,   0,   8,  18,  10,   70,   2, 32500, 200500,  10, 0, 0},		
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	pRadarDetect->InServiceMonitorCount++;
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	if (pRadarDetect->RDMode != RD_SILENCE_MODE)
			return;

	/* channel availability check time is 60sec, use 65 for assurance*/
	if (pRadarDetect->RDCount++ > pRadarDetect->ChMovingTime)
	{
		DBGPRINT(RT_DEBUG_TRACE, ("Not found radar signal, start send beacon and radar detection in service monitor\n\n"));
		AsicEnableBssSync(pAd);
		pRadarDetect->RDMode = RD_NORMAL_MODE;
		return;
	}
}
	...
	IN	PSTRING			arg)
{
	...
	pRadarDetect->CSPeriod = (USHORT) simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_TRACE, ("Set_CSPeriod_Proc::(CSPeriod=%d)\n", pRadarDetect->CSPeriod));
	...
	IN PSTRING arg)
{
	...
	pRadarDetect->ChMovingTime = Value;
...
VOID DFSInit(PRTMP_ADAPTER pAd)
{
	...
	pRadarDetect->CSPeriod = 10;
	pRadarDetect->CSCount = 0;
	pRadarDetect->RDMode = RD_NORMAL_MODE;
	pRadarDetect->bDFSIndoor = 1;
		...
		IN PRTMP_ADAPTER pAd)
{
	...
	if (pRadarDetect->RDMode != RD_SILENCE_MODE)
	{
		pRadarDetect->RDMode = RD_SWITCHING_MODE;
		pRadarDetect->CSCount = 0;
	}
	else
	{
