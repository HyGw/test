...
#ifdef DFS_SUPPORT
	if (pAd->CommonCfg.RadarDetect.dfs_func >= HARDWARE_DFS_V1)
...
#ifdef RTMP_TEMPERATURE_COMPENSATION
#ifdef RT5592
	if (pATEInfo->bTSSICalbrEnable == TRUE)
	{
		if (!IS_RT5592(pAd))                  
		{
				...
				&& (pATEInfo->TxWI.BW == BW_20)	&& (pATEInfo->TxAntennaSel == 1))                  
			{
				if (pATEInfo->Channel <= 14)
				{
					...
					if (pATEInfo->Channel == 7)
					{
						...
						pATEInfo->TssiReadSampleG[1] = BbpData;
					...
					else if (pATEInfo->Channel == 1)
					{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("Read  BBP_R49\n"));
						ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49, &BbpData);
						pATEInfo->TssiReadSampleG[0] = BbpData;
						DBGPRINT(RT_DEBUG_TRACE, ("BBP R49=0x%x\n", BbpData));	
					}
					else if (pATEInfo->Channel == 13)
					{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("Read  BBP_R49\n"));
						ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49, &BbpData);
						pATEInfo->TssiReadSampleG[2] = BbpData;
						DBGPRINT(RT_DEBUG_TRACE, ("BBP R49=0x%x\n", BbpData));	
					}
					else
					{
				...
				else
				{
					...
					if (pATEInfo->Channel == 100)
					{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("Read  BBP_R49\n"));
						ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49, &BbpData);
						pATEInfo->TssiReadSampleA[1] = BbpData;
						DBGPRINT(RT_DEBUG_TRACE, ("BBP R49=0x%x\n", BbpData));			
					}
					/* step 2: calibrate channel 1 and 13 TSSI delta values */
					else if (pATEInfo->Channel == 36)
					{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("Read  BBP_R49\n"));
						ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49, &BbpData);
						pATEInfo->TssiReadSampleA[0] = BbpData;
						DBGPRINT(RT_DEBUG_TRACE, ("BBP R49=0x%x\n", BbpData));	
					}
					else if (pATEInfo->Channel == 161)
					{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("Read  BBP_R49\n"));
						ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49, &BbpData);
						pATEInfo->TssiReadSampleA[2] = BbpData;
						DBGPRINT(RT_DEBUG_TRACE, ("BBP R49=0x%x\n", BbpData));	
					}
					else
					{
	...
	IN	PSTRING			arg)
{
	...
	if (IS_RT5592(pAd))
	{
		DBGPRINT(RT_DEBUG_OFF, ("ATETSSICBAEX, enable I/Q gain/phase compensation.\n"));
	}
#endif /* RT5592 */
#ifndef RT5592
	...
	IN	PSTRING				arg)
{
	...
	if (pATEInfo->bTSSICalbrEnable == FALSE)
	{
		DBGPRINT_ERR(("bTSSICalbrEnable == FALSE !!!\n"));
	...
	{
		pATEInfo->bTSSICalbrEnable = FALSE;
	}
	...
	IN	PSTRING			arg)
{
	...
	else if (strcmp(arg, "1") == 0)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("temperature calibration enabled!\n"));
	}
	else
	{
		return FALSE;
