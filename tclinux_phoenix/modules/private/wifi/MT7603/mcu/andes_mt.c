...
printk("\r\nCmdRFRegAccessWrite:RFRegAccess.Data = %x\r\n",RFRegAccess.Data);
...
static VOID CmdRFRegAccessReadCb(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	printk("\r\nCmdRFRegAccessReadCb:%x,len:%d\r\n",RFRegAccess->Data,Len);
	NdisMoveMemory(msg->rsp_payload, &RFRegAccess->Data, Len - 8);
	printk("\r\n before %x %x %x %x",msg->rsp_payload[0],msg->rsp_payload[1],msg->rsp_payload[2],msg->rsp_payload[3]);
	*((UINT32 *)(msg->rsp_payload)) = le2cpu32(*((UINT32 *)msg->rsp_payload));
	printk("\r\n after %x %x %x %x",msg->rsp_payload[0],msg->rsp_payload[1],msg->rsp_payload[2],msg->rsp_payload[3]);
}
