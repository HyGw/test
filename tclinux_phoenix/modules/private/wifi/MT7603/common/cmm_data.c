...
BOOLEAN RTMP_FillTxBlkInfo(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	...
	else
	{
		...
		if(IS_ENTRY_CLIENT(pMacEntry))
			{
			pMacEntry->McsTotalTxCount++;
			if(pTxBlk->pTransmit->field.MCS <= 3)
				{
				pMacEntry->McsLowRateTxCount++; 	
				}
			}
...
//YF MCC
		...
		if (pTxBlk->TxFrameType == TX_LEGACY_FRAME)
		{
			if ( ((RTMP_GET_PACKET_LOWRATE(pPacket))
				...
				|| (RTMP_GET_PACKET_ICMP(pTxBlk->pPacket)) //add icmp fix rate by dragon
			)
...
dequeue:
		if ((qEntry = pQueue->Head) != NULL)
		{
			...
			if(RTMP_GET_PACKET_ICMP(pPacket))
