...
#else
	sprintf(msg+strlen(msg),"%-4s%-33s%-20s%-23s%-9s%-9s%-7s%-7s%-3s\n",
	    "Ch", "SSID", "BSSID", "Security", "RSSI","Siganl(%)", "W-Mode", " ExtCH"," NT");	
#endif /* AIRPLAY_SUPPORT */
...
{	
	...
	#else
	DBGPRINT(RT_DEBUG_OFF, ("\n\tmulticlient_mode_check: \t%d, [%d]", 
		pAd->MultiClientOnMode, pAd->MacTab.Size));
	#endif
	printk("\n");
