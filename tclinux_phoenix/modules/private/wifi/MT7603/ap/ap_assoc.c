    ...
    OUT USHORT *pAid)
{
	...
	if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
		...
		if (pAd->LowRateCtrl.RatePrior) {
			if(pAd->CommonCfg.Channel < 14 && ie_list->ht_cap_len == 0)
				return MLME_UNSPECIFY_FAIL;
#ifdef DOT11_VHT_AC		
			if(pAd->CommonCfg.Channel > 14 && ie_list->vht_cap_len == 0)
				return MLME_UNSPECIFY_FAIL;
#endif /* DOT11_VHT_AC */
			...
			DlListForEach(pBlackSta, &pAd->LowRateCtrl.BlackList, BLACK_STA, List) {
				if (NdisCmpMemory(pBlackSta->Addr,pEntry->Addr, MAC_ADDR_LEN) == 0){
					DBGPRINT(RT_DEBUG_OFF,("Reject black sta: %02x:%02x:%02x:%02x:%02x:%02x\n",
