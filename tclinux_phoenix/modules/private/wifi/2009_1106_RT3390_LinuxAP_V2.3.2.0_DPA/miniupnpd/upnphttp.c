/* $Id: //BBN_Linux/Branch/Branch_for_Rel_CMCC_7526_20161014/tclinux_phoenix/modules/private/wifi/2009_1106_RT3390_LinuxAP_V2.3.2.0_DPA/miniupnpd/upnphttp.c#1 $ */
/* Project :  miniupnp
...
#ifdef ENABLE_WSC_SERVICE
		else if(strcasecmp(WFA_ROOTDESC_PATH, HttpUrl) == 0)
		{
			sendXMLdesc(h, genWSCRootDesc);
		}
		else if(strcasecmp(WSC_PATH, HttpUrl) == 0)
		{
			sendXMLdesc(h, genWSC);
		}
#endif /* ENABLE_WSC_SERVICE */
...
#ifdef RT_DEBUG
				wsc_chardump("h->req_buf before realloc", h->req_buf, h->req_buflen);
				wsc_chardump("receive_buf before realloc", buf, n);
#endif
				h->req_buf = (char *)realloc(h->req_buf, n + h->req_buflen);
				memcpy(h->req_buf + h->req_buflen, buf, n);
#ifdef RT_DEBUG
				wsc_chardump("h->req_buf after realloc", h->req_buf, h->req_buflen);
				wsc_chardump("receive_buf after realloc", buf, n);
				wsc_chardump("feed into req_buf after realloc", h->req_buf + h->req_buflen, n);
#endif
				h->req_buflen += n;
				if((h->req_buflen - h->req_contentoff) >= h->req_contentlen)
				{
