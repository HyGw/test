/* $Id: //BBN_Linux/Branch/Branch_for_Rel_CMCC_7526_20161014/tclinux_phoenix/modules/private/wifi/2009_1106_RT3390_LinuxAP_V2.3.2.0_DPA/miniupnpd/minissdp.c#1 $ */
/* MiniUPnP project
                  ...
                  unsigned int lifetime)
{
	...
	for(i=0; i<n_lan_addr; i++)
	{
		WSC_SendSSDPNotifies(sockets[i], lan_addr[i].str, port, lifetime);
	}
}
#endif /* ENABLE_WSC_SERVICE */
                   ...
                   unsigned short port)*/
{
	...
	else if(memcmp(bufr, "M-SEARCH", 8) == 0)
	{
		...
		if(st)
		{
			...
			/* Responds to request with a device as ST (Service Type) header */
			...
			if (GETFLAG(ENABLEWPSMASK))
			{
				for(i = 0; wfa_known_service_types[i]; i++)
				{
					...
					if(l<=st_len && (0 == memcmp(st, wfa_known_service_types[i], l)))
					{
						SendSSDPAnnounce2(s, sendername,
						                  st, st_len, "",
						                  lan_addr[lan_addr_index].str, WPS_PORT);
						break;
					}
				}
				/* Responds to request with ST: ssdp:all */
