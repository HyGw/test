...
INT Set_WscBlinkRate_Proc(
    IN  PRTMP_ADAPTER   pAd, 
    IN  PSTRING          arg)
{
	int cnt=0;

	cnt=simple_strtol(arg, 0, 10);
	if(cnt<=0){
		printk("Please input the value>=0\n");
		return FALSE;
	}else{
		led_count1=cnt;
	}

	printk("The led_count1 is %d\n",led_count1);
	return TRUE;
}	
...
#define	MAC_LINE_LEN	(1+14+4+4+4+4+6+6+9+6+6+1)
	...
	IN struct iwreq *wrq)
{
	...
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
		if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i]) && (pAd->MacTab.Content[i].Sst == SST_ASSOC))
		{
			...
			MacTab.Entry[MacTab.Num].Psm = pAd->MacTab.Content[i].apidx;
#ifdef DOT11_N_SUPPORT
...
#else
	sprintf(msg+strlen(msg),"%-14s%-4s%-4s%-4s%-4s%-6s%-6s%-9s%-6s%-6s\n",
		"MAC", "AP",  "AID", "PSM", "AUTH", "CTxR", "LTxR","LDT", "RxB", "TxB");//If change, be remember to modify the macro "MAC_LINE_LEN"!!!
#endif
...
#else
			sprintf(msg+strlen(msg),"%-9d",0/*pAd->MacTab.Content[i].HSCounter.LastDataPacketTime*/); // ToDo
			sprintf(msg+strlen(msg),"%-6d",0/*pAd->MacTab.Content[i].HSCounter.TotalRxByteCount*/); // ToDo
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	#ifdef DOT11N_DRAFT3
	//update 20_40Mcoexistance 20100519
