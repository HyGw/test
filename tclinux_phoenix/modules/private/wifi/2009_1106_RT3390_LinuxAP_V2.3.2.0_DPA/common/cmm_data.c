...
//added by xyzhu_nj_20091208,to check if wifi mac table is changed.
...
//	PNDIS_PACKET DuplicatePkt;
	...
	*/
		...
		(FlgIsDeltsFrame == 0))
	{
		...
		RTMP_SET_PACKET_UAPSD(pPacket, TRUE, QID_AC_VO);
...
#ifdef UAPSD_AP_SUPPORT
	                UAPSD_MR_ENTRY_RESET(pAd, &pAd->MacTab.Content[i]);
#endif // UAPSD_AP_SUPPORT //
	            }
...
#ifdef WAPI_SUPPORT
			RTMPCancelWapiRekeyTimerAction(pAd, &pAd->MacTab.Content[i]);
#endif // WAPI_SUPPORT //
