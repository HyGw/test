	...
	*/
	...
	if(pAd->Mlme.PeriodicRound % MLME_TASK_EXEC_MULTIPLE == 0 && pAd->LowRateCtrl.RatePrior == 1)
	{
		for (i=1; i<MAX_LEN_OF_MAC_TABLE; i++)
		{
			...
			if(IS_ENTRY_CLIENT(pEntry) && pEntry->RssiCount!=0)
			{
				pEntry->AvgRssi = pEntry->PerEntryTotalRssi/pEntry->RssiCount;
				pEntry->PerEntryTotalRssi = 0;
				pEntry->RssiCount = 0;
			}
		}
		...
		DlListForEach(pBlackSta, &pAd->LowRateCtrl.BlackList, BLACK_STA, List) {
			if (time_after(jiffies,pBlackSta->Jiff+(pAd->LowRateCtrl.BlackListTimeout)*HZ)) {
				DBGPRINT(RT_DEBUG_OFF, ("Remove from blacklist, %02x:%02x:%02x:%02x:%02x:%02x\n",
							pBlackSta->Addr[0],pBlackSta->Addr[1],pBlackSta->Addr[2],
							pBlackSta->Addr[3],pBlackSta->Addr[4],pBlackSta->Addr[5]));
				
				tmp = pBlackSta;
				pBlackSta = DlListEntry(pBlackSta->List.Prev, BLACK_STA, List);				
				DlListDel(&(tmp->List));				
				os_free_mem(NULL, tmp);
			}
		}
		RTMP_SEM_UNLOCK(&pAd->LowRateCtrl.BlackListLock);
	}
	if(pAd->Mlme.PeriodicRound % (MLME_TASK_EXEC_MULTIPLE * pAd->LowRateCtrl.LowRateCountPeriod) == 0 && pAd->LowRateCtrl.RatePrior == 1)
	{
		for (i=1; i<MAX_LEN_OF_MAC_TABLE; i++)
		{
			pEntry = &(pAd->MacTab.Content[i]);
			...
			{
				if(pEntry->McsTotalTxCount + pEntry->McsTotalRxCount > pAd->LowRateCtrl.TotalCntThreshold)
				{
					if((pAd->LowRateCtrl.LowRateRatioThreshold)*(pEntry->McsTotalTxCount + pEntry->McsTotalRxCount) < 10*(pEntry->McsLowRateTxCount + pEntry->McsLowRateRxCount))
					{
						DBGPRINT(RT_DEBUG_OFF, ("LowRateRatio exceed Threshold!Send DEAUTH frame and Add to blacklist, %02x:%02x:%02x:%02x:%02x:%02x\n",
				...
				else if(pEntry->AvgRssi < -86)
				{
					DBGPRINT(RT_DEBUG_OFF, ("RSSI below Threshold!Send DEAUTH frame and Add to blacklist, %02x:%02x:%02x:%02x:%02x:%02x\n",
							pEntry->Addr[0],pEntry->Addr[1],pEntry->Addr[2],
							pEntry->Addr[3],pEntry->Addr[4],pEntry->Addr[5]));
					os_alloc_mem(NULL, (UCHAR **)&pBlackSta, sizeof(BLACK_STA));
					NdisZeroMemory(pBlackSta, sizeof(BLACK_STA));
				...
				pEntry->McsTotalTxCount = 0;
				pEntry->McsLowRateTxCount = 0;
