...
#if defined(TCSUPPORT_LED_SWITCH_BUTTON)
...
#if defined(TCSUPPORT_LED_SWITCH_BUTTON)
int BBUWlanLedAct_5G(int action ,int gpio){
	PRTMP_ADAPTER	pAd = pAd_save;
...
int BBUAfterInsMod_5G(IN RTMP_ADAPTER *pAd){
	pAd_save = pAd;
	rcu_assign_pointer(hook_wlan_led_action_5g, BBUWlanLedAct_5G);
	return 0;
}
int BBUBeforeRmMod_5G(IN RTMP_ADAPTER *pAd){
	pAd_save = NULL;
	rcu_assign_pointer(hook_wlan_led_action_5g, NULL);	
	return 0;
}
#endif
