	...
	IN UCHAR FromWhichBSSID)
{
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pRxBlk->pData, pRxBlk->DataSize, FromWhichBSSID))
	{
		...
		if ((IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_WDS(pEntry))
			&& wifi_bandwidth_hook_rx!= NULL)
		{

			PUCHAR pSA = pRxBlk->pHeader->Addr2;			
			INT data_len = pRxBlk->DataSize;
			if(!wifi_bandwidth_hook_rx(pSA,data_len)){
				RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
				return;
			}
		}
	...
	)
	{
   	...
   	Update_Rssi_Sample(pAd, &pEntry->RssiSample, pRxWI);
		...
		if(IS_ENTRY_CLIENT(pEntry)){
			pEntry->PerEntryAvgRssi = RTMPAvgRssi(pAd, &pEntry->RssiSample);		
			pEntry->PerEntryTotalRssi += pEntry->PerEntryAvgRssi;		
			pEntry->RssiCount++;
		}
#endif /*RATE_PRIOR_SUPPORT*/
