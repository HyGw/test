		...
		switch (MsgType)
		{
			...
			case WSC_MSG_M8:
				...
				if (MsgType == WSC_MSG_M8)
				{
					...
					WscBuildProbeRespIE(pAdapter, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, (pWscControl->EntryIfIdx & 0x0F), NULL, 0, AP_MODE);
					APMakeAllBssBeacon(pAdapter);
					APUpdateAllBeaconFrame(pAdapter);
				}
				break;
	...
	if (pWscControl->WscState == WSC_STATE_CONFIGURED)
	{
		...
		else
		{
			if (CurOpMode == AP_MODE)
			{
				...
				WscBuildProbeRespIE(pAdapter, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, pEntry->apidx, NULL, 0, CurOpMode);
				APMakeAllBssBeacon(pAdapter);
				APUpdateAllBeaconFrame(pAdapter);
	...
	if (CurOpMode == AP_MODE)
	{
			...
			((nWscState == WSC_STATE_WAIT_DONE) || (nWscState == WSC_STATE_WAIT_ACK)))
		{
			...
			pWscControl->WscConfStatus = WSC_SCSTATE_CONFIGURED;
				...
				WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, pWscControl->WscConfStatus, FALSE, WscMode, pWscControl->WscConfigMethods, (pWscControl->EntryIfIdx & 0x0F), NULL, 0, CurOpMode);
		...
		if (pWscControl->Wsc2MinsTimerRunning)
		{
			...
			if (CurOpMode == AP_MODE)
			{
				...
				if ((pWscControl->EntryIfIdx & 0x0F) < pAd->ApCfg.BssidNum)
				{
					...
					WscBuildProbeRespIE(pWscControl->pAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, FALSE, 0, 0, (pWscControl->EntryIfIdx & 0x0F), NULL, 0, CurOpMode);
	...
	IN  UCHAR	apidx)
{
	...
	if (wsc_sel_reg == 0x01)
	{
		...
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, TRUE, wsc_dev_pass_id, wsc_sel_reg_conf_mthd, apidx, pAuthorizedMACs, AuthorizedMACsLen, AP_MODE);		
#ifdef WSC_V2_SUPPORT
			...
			(pWscCtrl->WscState <= WSC_STATE_WAIT_M3))
		{
			...
			os_alloc_mem(pAd, (UCHAR **)&pWscPeer, WSC_PEER_ENTRY_SIZE);
			if (pWscPeer)
			{
	...
	else
	{
		...
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, apidx, NULL, 0, AP_MODE);
	}
	...
	IN  UCHAR       	BssIndex)
{
...
#endif /* CONFIG_AP_SUPPORT */
	{
		...
		if (CurOpMode == AP_MODE)
		{
			...
			{
				IsAPConfigured = pWscControl->WscConfStatus;
				WscBuildBeaconIE(pAd, IsAPConfigured, FALSE, 0, 0, BssIndex, NULL, 0, AP_MODE);
	...
	IN  BOOLEAN         bFromUPnP)
{
	...
	WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, TRUE, WscMode, pWscControl->WscConfigMethods, (pWscControl->EntryIfIdx & 0x0F), NULL, 0, AP_MODE);
	...
	IN	PUCHAR			pMacAddr)
{
	...
	{
		os_alloc_mem(NULL, (UCHAR **)&pWscPeer, WSC_PEER_ENTRY_SIZE);
		if (pWscPeer)
		{
			NdisZeroMemory(pWscPeer, WSC_PEER_ENTRY_SIZE);
			pWscPeer->pNext = NULL;
