...
UCHAR RateSwitchTable11N3S_Normal[] = {
/* Item No.	Mode	Curr-MCS	TrainUp	TrainDown	 Mode- Bit0: STBC, Bit1: Short GI, Bit4,5: Mode(0:CCK, 1:OFDM, 2:HT Mix, 3:HT GF)*/
...
Ralink_ATE_On:
    	if (ATE_ON(pAd))
    	{
			...
			if ((pAd->ate.bAutoTxAlc == TRUE) && (pAd->ate.Mode == ATE_TXFRAME))
			{
	...
	do
	{
		...
		{/* 11BGN 2S AP*/
...
#endif /* AGS_SUPPORT */
			{
				...
				if (RTMP_DRS_GROUP_IS_USED(pAd, pEntry))
				{
					RTMP_DRS_GROUP_TABLE_2S_SELECT(ppTable, pTableSize, pInitTxRateIdx);
				}
				else
			...
			(pAd->CommonCfg.TxStream == 3))
		{
			...
			if (RTMP_DRS_GROUP_IS_USED(pAd, pEntry))
			{
				RTMP_DRS_GROUP_TABLE_3S_SELECT(ppTable, pTableSize, pInitTxRateIdx);
			}
			else
#endif /* NEW_RATE_ADAPT_SUPPORT */
			{
				*ppTable = RateSwitchTable11N3S_Normal;
				*pTableSize = RateSwitchTable11N3S_Normal[0];
				*pInitTxRateIdx = RateSwitchTable11N3S_Normal[1];
			}
		...
		{/* 11N 1S AP*/
...
#endif /* AGS_SUPPORT */
			{
				...
				if (RTMP_DRS_GROUP_IS_USED(pAd, pEntry))
				{
					RTMP_DRS_GROUP_TABLE_1S_SELECT(ppTable, pTableSize, pInitTxRateIdx);
				}
				else
#endif /* NEW_RATE_ADAPT_SUPPORT */
				if (pAd->LatchRfRegs.Channel <= 14)
...
#ifdef NEW_RATE_ADAPT_SUPPORT
	if (RTMP_DRS_GROUP_ALG_IS_SELECTED(*ppTable))
	{
		UCHAR tempUchar[] = {0x0f, 0x20, 15,  8,   25,  14, 21, 16, 15, 130,/*mcs15*/
						0x10, 0x22, 15,  8,   25,  15, 21, 16, 16, 144};/*mcs15+short gi*/
		UCHAR tempUchar1[] = {0x07, 0x21,  7,  8, 14,  6,  19,  12,  8, 65,/*mcs7*/
	...
	IN	PSTRING					arg)
{
	...
	/* parse Alg */
	Alg = simple_strtol(arg, 0, 10);

	/* check Alg */
	if (Alg <= RTMP_CHIP_DRS_GROUP)
	{
		/* reset DRS parameter */
		if (Alg != pAd->chipCap.DrsMethod)
		{
