...
VOID 	WscOnOff(
	...
	IN  BOOLEAN			bOff)
{
	PWSC_V2_INFO	pWpsV2Info = &pAd->ApCfg.MBSSID[ApIdx].WscControl.WscV2Info;
	if (bOff)
	{
		/*
	...
	IN PVOID SystemSpecific3)
{
	...
	if ((pWscControl->PinAttackCount < 2) &&
		(pWscControl->WscSetupLockTimerRunning == FALSE))
	{
		pWscControl->PinAttackCount = 0;
	}
	pWscControl->WscPinAttackCountCheckTimerRunning = FALSE;
	DBGPRINT(RT_DEBUG_TRACE, ("WscPinAttackCountCheckTimeout!\n"));
	...
	IN  PWSC_CTRL		pWscControl)
{
	...
	if (pWscControl->PinAttackCount == 0)
	{
		if (pWscControl->WscPinAttackCountCheckTimerRunning)
		{
			RTMPCancelTimer(&pWscControl->WscPinAttackCountCheckTimer, &bCancelled);
		}
		else
			pWscControl->WscPinAttackCountCheckTimerRunning = TRUE;
		RTMPSetTimer(&pWscControl->WscPinAttackCountCheckTimer, 60000);
	}
	
	/*
		PIN fail third time, block WSC setup 60 seconds.
	*/
	...
	if (pWscControl->PinAttackCount >= 3)
	{
		...
		RTMPSetTimer(&pWscControl->WscSetupLockTimer, 60000);
	...
	OUT	INT				*pOutBufLen)
{
	...
	if (pWscV2Msg)
	{
		...
		if (bSelRegistrar)
		{
			...
			else if (pWpsCtrl->WscMode == WSC_PIN_MODE)
			{
				WscAppendV2SubItem(	WFA_EXT_ID_AUTHORIZEDMACS, 
									BROADCAST_ADDR, 
									MAC_ADDR_LEN, 
