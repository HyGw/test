	...
	IN  UCHAR			FrameGap)
{
	...
	{
		UINT32 longptr =  *ptr + (*(ptr+1)<<8) + (*(ptr+2)<<16) + (*(ptr+3)<<24);
		RTMP_IO_WRITE32(pAd, CTSBaseAddr + TXWISize + i, longptr);
		ptr += 4;
	}
...
VOID MlmeUSBCarrierDetectionStateCtrl(
	IN PRTMP_ADAPTER pAd)
{
	...
	switch(*pCD_State)
	{
		...
		case CD_SILENCE:
			/*DBGPRINT(RT_DEBUG_TRACE, ("RadarToneCount = %d\n", CarrDetectParam.RadarToneCount));*/
...
#ifdef USB_CARRIER_DETECTION_SUPPORT
	{
		USHORT sVal = (USHORT) Value;
		RTUSBMultiWrite(pAd, CD_CRITERIA, (PUCHAR) &sVal, 2);
		/* send enable cmd to mcu to take effect */
		AsicSendCommandToMcu(pAd, CD_ONOFF_MCU_CMD, 0xff, 0x01, 0x00);
		pAd->CommonCfg.CarrierDetect.criteria = ( Value << 6); /* convert unit from ms to 16us:(2^10 /2^4)  */
	}
#endif /* USB_CARRIER_DETECTION_SUPPORT */
	return TRUE;
}
...
#ifdef USB_CARRIER_DETECTION_SUPPORT
	RTUSBSingleWrite(pAd, CD_CHECK_COUNT, pAd->CommonCfg.CarrierDetect.recheck1);
	/* send enable cmd to mcu to take effect */
	AsicSendCommandToMcu(pAd, CD_ONOFF_MCU_CMD, 0xff, 0x01, 0x00);
#endif /* USB_CARRIER_DETECTION_SUPPORT */
...
static void ToneRadarProgram(PRTMP_ADAPTER pAd)
{
	...
	if(pAd->CommonCfg.carrier_func==TONE_RADAR_V2)
	{
		...
		bbp |= 0x10<<3; /* Full 40MHz Detection Mode */
		bbp |= (pAd->CommonCfg.CarrierDetect.SymRund & 0x3) <<4;
...
{
	BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R184, 0x05);
	DBGPRINT(RT_DEBUG_TRACE, ("ToneRadarEnable v1\n"));
	...
	if(pAd->CommonCfg.carrier_func==TONE_RADAR_V2)
	{
		DBGPRINT(RT_DEBUG_TRACE, ("ToneRadarEnable v2\n"));
...
{	
	...
	if (pAd->CommonCfg.CarrierDetect.Enable)
	{
		...
		RTUSBMultiWrite(pAd, CD_CRITERIA, (PUCHAR) &criteria, 2);
		RTUSBSingleWrite(pAd, CD_CHECK_COUNT, pAd->CommonCfg.CarrierDetect.recheck1);
		AsicSendCommandToMcu(pAd, CD_ONOFF_MCU_CMD, 0xff, 0x01, 0x00);
		}
#endif /* USB_CARRIER_DETECTION_SUPPORT */	
