...
/*		if (IS_RT2860(pAd))  RT2860 supports 3 Rx and the 2.4 GHz RSSI #2 offset is in the EEPROM 0x48*/
	...
	/*
		1. ODMS must program EEPROM_DEFINE_MAX_TXPWR else use this default value suggested by Ralink.
		2. Some dongle has old EEPROM value, use ModuleTxpower for saving correct value fo DefineMaxTxPwr.
	*/
	if (pAd->CommonCfg.DefineMaxTxPwr == 65535 || pAd->CommonCfg.DefineMaxTxPwr <= 255) 
	{
		if (pAd->CommonCfg.ModuleTxpower > 0) 
		{
			...
			pAd->CommonCfg.DefineMaxTxPwr = 0x1313;
	}
