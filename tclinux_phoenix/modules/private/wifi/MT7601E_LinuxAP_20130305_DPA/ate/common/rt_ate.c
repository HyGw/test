	...
	IN RXWI_STRUC *pRxWI)
{
	...
	if(IS_MT7601(pAd))
	{
		pATEInfo->LastRssi0	= ConvertToRssi(pAd, (CHAR)pRxWI->RxWISNR2, RSSI_0, pRxWI->RxWISNR1, pRxWI->RxWIBW);
	}
	else
	#endif
...
//	TXD_STRUC       TxD;
	...
	{
#ifdef RLT_MAC
		RTMP_IO_READ32(pAd, TX_RING_DIDX + QID_AC_BE * 0x10,  &pTxRing->TxDmaIdx);
#else
...
#ifdef RLT_MAC
		RTMP_IO_WRITE32(pAd, TX_RING_CIDX + QID_AC_BE * 0x10, pTxRing->TxCpuIdx);
#else
...
#else
		printk("%s (bCalFreeIC=TRUE)\n\n", __FUNCTION__);
#endif /* DBG */
	else
#ifdef DBG
		DBGPRINT(RT_DEBUG_OFF, ("%s (bCalFreeIC=FALSE)\n\n", __FUNCTION__));
#else
		printk("%s (bCalFreeIC=FALSE)\n\n", __FUNCTION__);
#endif /* DBG */

	RtmpOsMsDelay(100);	/* for litepoit ATE tool, because the tool reads the message too fast */
...
#ifdef DBG
	DBGPRINT(RT_DEBUG_TRACE,("Temperature = %d (0x%02X)\n", Temperature, Temperature));
#else
	printk("Temperature = %d (0x%02X)\n", Temperature, Temperature);
#endif /* DBG */
...
#ifdef DBG
	DBGPRINT(RT_DEBUG_TRACE,("TSSI DC = %d (0x%02X)\n", BbpReg, BbpReg));
#else
	printk("TSSI DC = %d (0x%02X)\n", BbpReg, BbpReg);
#endif /* DBG */
	RtmpOsMsDelay(1);
