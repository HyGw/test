	...
	IN UINT32 TxIdx)
{
	...
	//printk("\x1b[32m%s: TxIdx %d\x1b[m\n", __FUNCTION__, TxIdx);
	/* fill TxWI */
...
//	pTxD = &TxD;
	...
	if(0){
		...
		RTMP_IO_READ32(pAd, 0x208, &MacVal);
		printk("\x1b[31m%s: 1 0x208 = %x, hdr len %d\x1b[m\n", __FUNCTION__, MacVal, pATEInfo->HLen);
	}
	...
	if(0){
		...
		RTMP_IO_READ32(pAd, 0x208, &MacVal);
		printk("\x1b[31m%s: 2 0x208 = %x\x1b[m\n", __FUNCTION__, MacVal);
		//RTMP_IO_WRITE32(pAd, 0x208, 0x75);
	}
#endif /* RT_BIG_ENDIAN */
