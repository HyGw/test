	...
	IN	PSTRING			arg)
{
	...
	PRTMP_RX_RING pRxRing1 = &pAd->RxRing1;	
	PRTMP_CTRL_RING pCtrlRing = &pAd->CtrlRing;
	
	DBGPRINT(RT_DEBUG_OFF, ("TX-BE---------------------------------------------------\n"));
	...
	DBGPRINT(RT_DEBUG_OFF, ("CTRL---------------------------------------------------\n"));
	for(i=0;i<CTRL_RING_SIZE;i++)
	{	
#ifdef RT_BIG_ENDIAN
		...
		RTMPDescriptorEndianChange((PUCHAR)pTxD, TYPE_TXD);
#else
	    pTxD = (PTXD_STRUC) pCtrlRing->Cell[i].AllocVa;
#endif /* RT_BIG_ENDIAN */
	    DBGPRINT(RT_DEBUG_OFF, ("Desc #%d\n",i));
	    hex_dump("Tx Descriptor", (PUCHAR)pTxD, 16);
	    DBGPRINT(RT_DEBUG_OFF, ("pTxD->DMADONE = %x\n", pTxD->DMADONE));
	}    
	DBGPRINT(RT_DEBUG_OFF, ("Mgmt---------------------------------------------------\n"));
	...
	DBGPRINT(RT_DEBUG_OFF, ("RX1---------------------------------------------------\n"));
	for(i=0;i<RX1_RING_SIZE;i++)
	{	
#ifdef RT_BIG_ENDIAN
		pDestRxD = (RXD_STRUC *)pRxRing1->Cell[i].AllocVa;
		//RxD = *pDestRxD;
		//pRxD = &RxD;
		NdisMoveMemory(&rx_hdr_info[0], pDestRxD, RXD_SIZE);
		pRxD = (RXD_STRUC *)&rx_hdr_info[0];
		RTMPDescriptorEndianChange((PUCHAR)pRxD, TYPE_RXD);
#else
	    pRxD = (RXD_STRUC *)pRxRing1->Cell[i].AllocVa;
#endif /* RT_BIG_ENDIAN */
	    DBGPRINT(RT_DEBUG_OFF, ("Desc #%d\n",i));
	    hex_dump("Rx Descriptor", (PUCHAR)pRxD, 16);
	    DBGPRINT(RT_DEBUG_OFF, ("pRxD->DDONE = %x\n", pRxD->DDONE));
...
INT show_trinfo_proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	if (IS_RBUS_INF(pAd) || IS_PCI_INF(pAd))
	{
		...
		UINT32 rbase[2], rcnt[2];
		UINT32 rcidx[2], rdidx[2];
		...
		//Rx ring 0
		{
			RTMP_IO_READ32(pAd, pAd->RxRing.hw_desc_base, &rbase[0]);
			RTMP_IO_READ32(pAd, pAd->RxRing.hw_cnt_addr, &rcnt[0]);
			RTMP_IO_READ32(pAd, pAd->RxRing.hw_cidx_addr, &rcidx[0]);
			RTMP_IO_READ32(pAd, pAd->RxRing.hw_didx_addr, &rdidx[0]);
		}

		//Rx ring 1
		{
			RTMP_IO_READ32(pAd, pAd->RxRing1.hw_desc_base, &rbase[1]);
			RTMP_IO_READ32(pAd, pAd->RxRing1.hw_cnt_addr, &rcnt[1]);
			RTMP_IO_READ32(pAd, pAd->RxRing1.hw_cidx_addr, &rcidx[1]);
			RTMP_IO_READ32(pAd, pAd->RxRing1.hw_didx_addr, &rdidx[1]);
		}
...
#ifdef RLT_MAC
		{
			...
			RTMP_IO_READ32(pAd, PBF_CFG, &pbf_val);
		...
		DBGPRINT(RT_DEBUG_OFF, ("\tRingIdx Reg \tBase \t\tCnt \tCIDX \tDIDX \tSWIDX\n"));
		DBGPRINT(RT_DEBUG_OFF, ("\t%d \t0x%04x \t0x%08x \t0x%x \t0x%x \t0x%x \t0x%x\n",
						0, pAd->RxRing.hw_desc_base, rbase[0], rcnt[0], rcidx[0], rdidx[0],pAd->RxRing.RxSwReadIdx));
		DBGPRINT(RT_DEBUG_OFF, ("\t%d \t0x%04x \t0x%08x \t0x%x \t0x%x \t0x%x \t0x%x\n",
						1, pAd->RxRing1.hw_desc_base, rbase[1], rcnt[1], rcidx[1], rdidx[1],pAd->RxRing1.RxSwReadIdx));
...
INT set_rf(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	if (arg)
	{
		...
		if (rv == 3)
		{
			...
			rlt_rf_write(pAd, bank_id, rf_id, rf_val);
#ifdef DBG
			DBGPRINT(RT_DEBUG_TRACE, ("%s():bank%d r%03d = 0x%02X\n", __FUNCTION__, bank_id, rf_id, rf_val));
#else
			printk("%s():bank%d r%03d = 0x%02X\n", __FUNCTION__, bank_id, rf_id, rf_val);
#endif	/* DBG */
		}
		else if (rv == 2)
		{
			rlt_rf_read(pAd, bank_id, rf_id, &rrf_val);
#ifdef DBG
			DBGPRINT(RT_DEBUG_TRACE, ("%s():bank%d r%03d = 0x%02X\n", __FUNCTION__, bank_id, rf_id, rrf_val));
#else
			printk("%s():bank%d r%03d = 0x%02X\n", __FUNCTION__, bank_id, rf_id, rrf_val);
#endif /* DBG */
		}
