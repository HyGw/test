...
VOID RTMPResetTxRxRingMemory(RTMP_ADAPTER *pAd)
{
...
#ifdef RLT_MAC
	for (index = RX1_RING_SIZE - 1 ; index >= 0; index--)
	{
	...
	NdisZeroMemory(pAd->RxRing1.Cell, RX1_RING_SIZE * sizeof(RTMP_DMACB));
#endif /* RLT_MAC */
...
NDIS_STATUS RTMPInitTxRxRingMemory(RTMP_ADAPTER *pAd)
{
	...
	RingBaseVa = pDescRing->AllocVa;
	NdisZeroMemory(pDescRing->AllocVa, pDescRing->AllocSize);
	for (index = 0; index < CTRL_RING_SIZE; index++)
	{
	...
	pAd->RxRing1.RxCpuIdx = RX1_RING_SIZE - 1;
#endif /* RLT_MAC */
...
VOID RTMPFreeTxRxRingMemory(RTMP_ADAPTER *pAd)
{
...
#ifdef CONFIG_ANDES_SUPPORT
	{
		...
		while (pCtrlRing->TxSwFreeIdx!= pCtrlRing->TxDmaIdx)
		{
			...
			if (pPacket == NULL)
			{
				INC_RING_INDEX(pCtrlRing->TxSwFreeIdx, CTRL_RING_SIZE);
				continue;
			}
	...
	if (pAd->RxDescRing1.AllocVa)
		RtmpFreeDescBuf(((POS_COOKIE)(pAd->OS_Cookie))->pci_dev, pAd->RxDescRing1.AllocSize, pAd->RxDescRing1.AllocVa, pAd->RxDescRing1.AllocPa);
	
	NdisZeroMemory(&pAd->RxDescRing1, sizeof(RTMP_DMABUF));
#endif /* RLT_MAC */
...
VOID RTMPRingCleanUp(RTMP_ADAPTER *pAd, UCHAR RingType)
{
	...
	switch (RingType)
	{
		...
		case QID_RX:
...
#endif /* RT_BIG_ENDIAN */
#ifdef RX_DMA_SCATTER
				pDmaBuf = &pAd->RxRing.Cell[i].DmaBuf;
				pRxD->SDL0 = pDmaBuf->AllocSize;
#endif /* RX_DMA_SCATTER */
                pRxD->DDONE = 0 ;
...
#ifdef RX_DMA_SCATTER
				pDmaBuf = &pAd->RxRing1.Cell[i].DmaBuf;
				pRxD->SDL0 = pDmaBuf->AllocSize;
			...
			pAd->RxRing1.RxCpuIdx = ((pAd->RxRing1.RxDmaIdx == 0) ? (RX1_RING_SIZE-1) : (pAd->RxRing1.RxDmaIdx-1));
	...
	IN ULONG			UpdatePos)
{
	...
	//printk("\x1b[31m%s: FrameLen %d, UpdatePos %d\x1b[m\n", __FUNCTION__, FrameLen, UpdatePos);
	...
	if(0){
		...
		RTMP_IO_READ32(pAd, 0xc050+0x14, &Value);
		printk("\x1b[31m%s: 0xC050 %x\x1b[m\n", __FUNCTION__, Value);
		RTMP_IO_READ32(pAd, 0xc054+0x14, &Value);
		printk("\x1b[31m%s: 0xC054 %x\x1b[m\n", __FUNCTION__, Value);
		RTMP_IO_READ32(pAd, 0xc058+0x14, &Value);
		printk("\x1b[31m%s: 0xC058 %x\x1b[m\n", __FUNCTION__, Value);
	}
...
{    
    ...
    {
#if 1 
    	RT28XXDMADisable(pAd); // WCN brian's advice
	AsicWaitPDMAIdle(pAd, 200, 1000); // WCN brian's advice
#endif 
...
#if 1
	RT28XXDMAEnable(pAd);// WCN brian's advice
#endif 
...
VOID RT28xxPciMlmeRadioOFF(RTMP_ADAPTER *pAd)
{
	...
	/* Zero's patch to fix RadioOff fail under chariot traffic */
