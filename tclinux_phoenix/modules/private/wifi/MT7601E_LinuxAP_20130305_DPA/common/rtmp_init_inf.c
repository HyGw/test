		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			{	
				if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_START_UP) == 0)
				{
					...
					/* IRQ init first to fix enable auto channel, sta can't connect AP any more. WCN Zero's patch */
					RTMP_IRQ_INIT(pAd);
					RTMP_IRQ_ENABLE(pAd);
					/* Now Enable RxTx*/
					RTMPEnableRxTx(pAd);
	...
	IN VOID			*pAdSrc)
{
	...
	if (rx_cpu_idx == rx_dma_idx)
	{
//		dbg_plinel_1("\r\n RTMPDrvOpen: rx_dma_idx = ", rx_dma_idx);
		RTMP_IO_READ32(pAd, WPDMA_GLO_CFG, &Value);
		RTMP_IO_WRITE32(pAd, WPDMA_GLO_CFG, (Value & ~0x5));
	...
	IN VOID				*net_dev)
{
	...
	Cancelled = FALSE;
	 ...
	 if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RADIO_OFF)) //zero debug
	 {
	 	printk("Radio_ON first....\n");
    		MlmeRadioOn(pAd);
	 }
