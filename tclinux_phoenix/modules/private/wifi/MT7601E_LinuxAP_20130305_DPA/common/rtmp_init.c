...
/*		pBeaconBuf = kmalloc(MAX_BEACON_SIZE, MEM_ALLOC_FLAG);*/
		...
		printk("\x1b[31m\n\n=== pAd = %p, size = %d ===\n\n\x1b[m", pAd, (UINT32)sizeof(RTMP_ADAPTER));
...
VOID AsicInitTxRxRing(RTMP_ADAPTER *pAd)
{
	...
	*/
	...
	pAd->MgmtRing.hw_desc_base = TX_MGMT_BASE;
	...
	RTMP_IO_WRITE32(pAd, RX_RING1_CNT, RX1_RING_SIZE);
	...
	pAd->RxRing1.hw_desc_base = RX_RING1_BASE;
	pAd->RxRing1.hw_cidx_addr = RX_RING1_CIDX;
	pAd->RxRing1.hw_didx_addr = RX_RING1_DIDX;
	pAd->RxRing1.hw_cnt_addr = RX_RING1_CNT;
...
#ifdef MT7601
	if (IS_MT7601(pAd) && (pAd->MacTab.Size <= 8))
		return;
#endif /* MT7601 */
...
#ifdef CONFIG_AP_SUPPORT
{
	...
	pAd->dvga_cca_h_th = 800;
	pAd->dvga_cca_l_th = 10;
	pAd->dvga_rssi_bw20_th = -79;
	pAd->dvga_rssi_bw40_th = -76;
...
#if 1 
	if (RxDmaBusy(pAd) || TxDmaBusy(pAd))
	{
		RTMP_IO_READ32(pAd, WPDMA_GLO_CFG, &MacReg);
//		dbg_plinel_1("\r\n==>RTMPEnableRxTx(1): MacReg(0x208) = ", MacReg);
	}
#endif 
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("==> RTMPEnableRx\n"));
	...
	DBGPRINT(RT_DEBUG_TRACE, ("<== RTMPEnableRx (%x)\n", Val));
}
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("==> RTMPEnableTx\n"));
	...
	DBGPRINT(RT_DEBUG_TRACE, ("<== RTMPEnableTx (%x)\n", Val));
}
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("==> RTMPDisableRx\n"));

	RTMP_IO_READ32(pAd, MAC_SYS_CTRL, &Val);
	Val &= ~(0x00000008);
	RTMP_IO_WRITE32(pAd, MAC_SYS_CTRL, Val);

	DBGPRINT(RT_DEBUG_TRACE, ("<== RTMPDisableRx (%x)\n", Val));
}
#endif /* BB_SOC */
