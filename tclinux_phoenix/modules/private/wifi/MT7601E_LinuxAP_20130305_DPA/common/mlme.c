	...
	IN PVOID SystemSpecific3) 
{
	...
	if (IS_MT7601(pAd) && pAd->dvga_enable)
		ap_dvga_collect_rssi(pAd);
...
#ifdef AP_DYNAMIC_VGA_SUPPORT
		if (IS_MT7601(pAd) && pAd->dvga_enable)
			mt7601_ap_dynamic_vga_adjust(pAd);
#endif /* AP_DYNAMIC_VGA_SUPPORT */
#ifdef AP_DYNAMIC_PD_LPF_SUPPORT
		if (IS_MT7601(pAd) && pAd->dpd_enable)
			mt7601_ap_dynamic_pd_lpf_adjust(pAd);
	...
	IN UCHAR	 Channel) 
{
	...
	for (i = 0; i < Tab->BssNr; i++) 
	{
		//printk("%s: ssid %s ch %d/%d\n", __FUNCTION__, Tab->BssEntry[i].Ssid, Tab->BssEntry[i].Channel, Channel);
	...
	//printk("\x1b[32m%s: total ap %d, num %d\x1b[m\n", __FUNCTION__, ap_count, Tab->BssNr);
...
#ifdef MT7601
VOID bbp_hang_check(RTMP_ADAPTER *pAd)
{
	UINT32 rx_cnt = 0;
	UINT8 bbp_val = 0;
	RX_STA_CNT1_STRUC RxStaCnt1;

	RTMP_IO_READ32(pAd, RX_SEC_CNT0, &rx_cnt);
	RTMP_IO_READ32(pAd, RX_STA_CNT1, &RxStaCnt1.word);
	pAd->false_cca_cnt_per_100ms += RxStaCnt1.field.FalseCca;
	pAd->RalinkCounters.OneSecFalseCCACnt += RxStaCnt1.field.FalseCca;

	/* get bbp status */
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R142, 0x21);
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R143, &bbp_val);
	//printk("%s: bbp %x, false cca %d, rx %d\n", __FUNCTION__, bbp_val, pAd->false_cca_cnt_per_100ms, rx_cnt);

	if ((bbp_val == 0x14) && (pAd->false_cca_cnt_per_100ms == 0) && (rx_cnt == 0))
		pAd->bbp_halt_cnt++;
	else
		pAd->bbp_halt_cnt = 0;

	if (pAd->bbp_halt_cnt >= 3)
	{
		UINT32 mac_val;
		...
		/* bbp soft reset */
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R21, 0x3);
		RTMPusecDelay(1);
		...
		//printk("%s: bbp %x, false cca %d, rx %d\n", __FUNCTION__, bbp_val, pAd->false_cca_cnt_per_100ms, rx_cnt);
		//printk("%s: reset bbp %d\n", __FUNCTION__, pAd->bbp_halt_cnt);
		pAd->bbp_halt_cnt = 0;
	}

	/* reset */
	pAd->false_cca_cnt_per_100ms = 0;
}

#ifdef CONFIG_AP_SUPPORT
#ifdef AP_DYNAMIC_VGA_SUPPORT
VOID ap_dvga_collect_rssi(RTMP_ADAPTER *pAd)
{
	MAC_TABLE_ENTRY *pEntry;
	CHAR rssi, i, cnt = 0;
	INT32 acc_rssi = pAd->dvga_acs_rssi_sample;
	...
	if (cnt > 0)
	{
	    ...
	    */
		pAd->dvga_acs_rssi256 = ((pAd->dvga_acs_rssi256*15) >> 4) + (acc_rssi << 4);
		pAd->dvga_acs_rssi = pAd->dvga_acs_rssi256 >> 8;
		//pAd->dvga_acs_rssi = RTMPAvgRssi(pAd, &pAd->ApCfg.RssiSample);
		
	//	printk("\x1b[32m%s: sta cnt %d, avg rssi %d\x1b[m\n", __FUNCTION__, cnt, pAd->dvga_acs_rssi);
	}
	else
	{
		pAd->dvga_acs_rssi256 = (-128 << 8);
		pAd->dvga_acs_rssi = -128;
	}
}
#endif /* AP_DYNAMIC_VGA_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
