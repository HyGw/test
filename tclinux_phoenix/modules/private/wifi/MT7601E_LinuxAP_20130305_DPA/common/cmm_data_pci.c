...
//---Add by shiang for debug
	...
	//hex_dump("TxD2", pTxD, TXD_SIZE);
	WriteBackToDescriptor((PUCHAR)pDestTxD, (PUCHAR)pTxD, FALSE, TYPE_TXD);
	//hex_dump("DestTxD", pDestTxD, TXD_SIZE);
#endif /* RT_BIG_ENDIAN */
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	//add for avoiding RX coherent again
	//only for transit coherent 
	AsicWaitPDMAIdle(pAd, 200, 1000);

	RTMP_IO_READ32(pAd, WPDMA_GLO_CFG , &GloCfg.word);
	if ((GloCfg.field.TxDMABusy == 0)  && (GloCfg.field.RxDMABusy == 0)) {
		DBGPRINT(RT_DEBUG_TRACE, ("==>  DMAIdle, GloCfg=0x%lx\n", GloCfg.word));  
		return;
	}
	...
	UCHAR RxRingNo)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
	RTMP_INT_LOCK(pRxRingLock, IrqFlags);
#else
	...
	if (*pRxPending == 0)
	{
		...
		if (pRxRing->RxDmaIdx > pRxRing->RxSwReadIdx)
			...
			*pRxPending	= pRxRing->RxDmaIdx + ring_size - pRxRing->RxSwReadIdx;
	}
...
#endif
	//hex_dump("hw_rx_info", &rx_hw_info[0], sizeof(rx_hw_info));
	...
	if (pNewPacket)
	{
		...
		pRxWI = (RXWI_STRUC *)(GET_OS_PKT_DATAPTR(pRxPacket) + RXINFO_SIZE);
		//APFindDataPattern(__FUNCTION__, __LINE__, GET_OS_PKT_DATAPTR(pRxPacket), pRxWI->RxWIMPDUByteCnt);
		//SET_OS_PKT_LEN(pRxPacket, );
		//printk("1pkt len %d\n", GET_OS_PKT_LEN(pRxPacket));
#ifdef RTMP_MAC
		...
		pRxInfo = &RxInfo;
		//printk("\x1b[31m%s: RxInfo %p/%p, pRxPacket %p/%p, RxInfo %d\x1b[m\n", __FUNCTION__, 
		//															&RxInfo, RxInfo, &pRxPacket, pRxPacket, sizeof(RxInfo));
		//RTMPDescriptorEndianChange((PUCHAR)pRxInfo, TYPE_RXINFO);
		//(*(UINT32*)pRxInfo) = SWAP32(*(UINT32*)pRxInfo);
		(*(UINT32*)pRxInfo) = le2cpu32(*(UINT32*)pRxInfo);
		//printk("\x1b[31m%s:  RxInfo %p/%p, pRxPacket %p/%p\x1b[m\n", __FUNCTION__,
		//															&RxInfo, RxInfo, &pRxPacket, pRxPacket);
	...
	*pRxPending = *pRxPending - 1;	
	...
	INC_RING_INDEX(pRxRing->RxSwReadIdx, ring_size);

	pRxRing->RxCpuIdx = (pRxRing->RxSwReadIdx == 0) ? (ring_size-1) : (pRxRing->RxSwReadIdx-1);
#ifdef RLT_MAC
	if (RxRingNo == 1)
	{
	...
	*/
	...
	else
	{
		...
		if (pRxRing->RxSwReadIdx >= (ring_size-1))
		{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
	RTMP_INT_UNLOCK(pRxRingLock, IrqFlags);
#else
...
BOOLEAN RxRing1DoneInterruptHandle(RTMP_ADAPTER *pAd) 
{
	...
	while (1)
	{
		...
		*/
		...
		if ((pRxPacket == NULL) && bCmdRspPacket)
		{
			INC_RING_INDEX(pAd->RxRing1.RxSwReadIdx, RX1_RING_SIZE);
			continue;
		}
		...
		{
			DBGPRINT(RT_DEBUG_TRACE,("\x1b[32m%s: Receive command packet.\x1b[m\n", __FUNCTION__));
...
/*	int 		 i;*/
	...
	//printk("\x1b[31m%s: ... DMA index %d, free index %d\x1b[m\n", __FUNCTION__, pCtrlRing->TxDmaIdx, pCtrlRing->TxSwFreeIdx);
