...
VOID AP_AMPDU_Frame_Tx(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	...
	*/
	...
	if(0){
		...
		RTMP_IO_READ32(pAd, 0x434, &Value);
		printk("\x1b[32m%s: 0x434 = %x\x1b[m\n", __FUNCTION__, Value);
		RTMP_IO_READ32(pAd, 0x438, &Value);
		printk("\x1b[32m%s: 0x438 = %x\x1b[m\n", __FUNCTION__, Value);
	}
	...
	IN UCHAR FromWhichBSSID)
{
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pRxBlk->pData, pRxBlk->DataSize, FromWhichBSSID))
	{
		...
		if (pEntry->PrivacyFilter == Ndis802_11PrivFilter8021xWEP)
		{
			...
			{		
				printk("\x1b[31m%s: de-auth line %d\x1b[m\n", __FUNCTION__, __LINE__);
...
//---Add by shiangf for debug
	...
	//APFindDataPattern( __FUNCTION__, __LINE__, pRxBlk->pData, pRxBlk->DataSize );
	...
	*/
	...
	if (IS_MT7601(pAd))
	{
			pAd->dvga_acs_rssi_sample = ConvertToRssi(pAd, (UCHAR)pRxWI->RxWISNR2, RSSI_0, pRxWI->RxWISNR1, pRxWI->RxWIBW);
			//printk("%s: wcid .......... %d\n", __FUNCTION__, pRxWI->RxWIWirelessCliID);
	}
...
BOOLEAN APRxDoneInterruptHandle(RTMP_ADAPTER *pAd) 
{
	...
	while (1)
	{
		...
		*/
		...
		//APFindDataPattern_b( __FUNCTION__, __LINE__, pData, pRxWI->RxWIMPDUByteCnt);
		//APFindDataPattern( __FUNCTION__, __LINE__, rxblk.pData, rxblk.DataSize);
//+++Add by shiang for debug
if (0) { 
