...
INT set_ap_dynamic_vga_proc(
	IN PRTMP_ADAPTER pAd, 
	IN PSTRING arg);

INT set_ap_dvga_ccal_proc(
	PRTMP_ADAPTER pAd, 
	PSTRING arg);

INT set_ap_dvga_ccah_proc(
	PRTMP_ADAPTER pAd, 
	PSTRING arg);

INT set_ap_dvga_rssi_proc(
	PRTMP_ADAPTER pAd, 
	PSTRING arg);
#endif /* AP_DYNAMIC_VGA_SUPPORT */

#ifdef AP_DYNAMIC_PD_LPF_SUPPORT
INT set_ap_dynamic_pd_proc(
	IN PRTMP_ADAPTER pAd, 
	IN PSTRING arg);

INT set_ap_dpd_ccah_proc(
	PRTMP_ADAPTER pAd, 
	PSTRING arg);

INT set_ap_dpd_ccal_proc(
	PRTMP_ADAPTER pAd, 
	PSTRING arg);
#endif /* AP_DYNAMIC_PD_LPF_SUPPORT */
...
static struct {
	...
	{"HwTxRateEnable", Set_HwTxRateLookUp_Proc},
#endif /* MCS_LUT_SUPPORT */
#ifdef AP_DYNAMIC_VGA_SUPPORT
	{"ap_dvga", set_ap_dynamic_vga_proc},
	{"ap_dvga_rssi", set_ap_dvga_rssi_proc},
	{"ap_dvga_ccah", set_ap_dvga_ccah_proc},
	{"ap_dvga_ccal", set_ap_dvga_ccal_proc},
#endif /* AP_DYNAMIC_VGA_SUPPORT */
#ifdef AP_DYNAMIC_PD_LPF_SUPPORT
	{"ap_dpd", set_ap_dynamic_pd_proc},
	{"ap_dpd_ccah", set_ap_dpd_ccah_proc},
	{"ap_dpd_ccal", set_ap_dpd_ccal_proc},
#endif /* AP_DYNAMIC_PD_LPF_SUPPORT */
	...
	IN	INT                 cmd)
{
	...
	UCHAR	apidx = pObj->ioctl_if;
#endif /* defined(DBG) || defined(WSC_AP_SUPPORT) || defined(BB_SOC) */
#ifdef WSC_AP_SUPPORT
	UINT	WscPinCode = 0;
   ...
   INT IEEE8021X = 0;
	...
	case OID_802_11_AUTHENTICATION_MODE:
		...
		case RT_OID_WSC_QUERY_STATUS:
		{
			...
			printk("\x1b[31m%s: WscStatus %d\x1b[m\n", __FUNCTION__, WscStatus);
			wrq->u.data.length = sizeof(INT);
	...
	IN PSTRING arg)
{
	...
	UCHAR tx_hdr_info[TXD_SIZE];
	UCHAR rx_hdr_info[RXD_SIZE];
#endif /* RT_BIG_ENDIAN */
	...
	//TxD = *pDestTxD;
	//pTxD = &TxD;
	NdisMoveMemory(&tx_hdr_info[0], pDestTxD, TXD_SIZE);
	pTxD = (TXD_STRUC *)&tx_hdr_info[0];
	...
	//RxD = *pDestRxD;
	//pRxD = &RxD;
	NdisMoveMemory(&rx_hdr_info[0], pDestRxD, RXD_SIZE);
	pRxD = (RXD_STRUC *)&rx_hdr_info[0];
	...
	IN	PSTRING			arg)
{
    ...
    if (!bFromApCli)
    {
		...
		{
		#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
			RTMP_INT_LOCK(&pWscControl->WscPeerListSemLock, IrqFlags);
		#else
			RTMP_SEM_LOCK(&pWscControl->WscPeerListSemLock);
		#endif
			WscClearPeerList(&pWscControl->WscPeerList);
		#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
			RTMP_INT_UNLOCK(&pWscControl->WscPeerListSemLock, IrqFlags);
		#else		
...
{
	INT LedCmd;
	LedCmd = simple_strtol(arg, 0, 10);
	printk("@%s LedCmd=%d \n", __FUNCTION__, LedCmd);
	AndesLedOP(pAd, 3, LedCmd);
...
INT ed_status_read(RTMP_ADAPTER *pAd)
{
...
#ifdef MT7601
	/* for MT7601 bbp hang */
	pAd->false_cca_cnt_per_100ms = pAd->false_cca_stat[pAd->ed_stat_lidx];
#endif /* MT7601 */
	...
	//printk("\x1b[32m%s: ed busy time %d, false cca %d\x1b[m\n", __FUNCTION__, pAd->ed_stat[pAd->ed_stat_lidx], pAd->false_cca_stat[pAd->ed_stat_lidx]);
...
#endif /* ED_MONITOR */
#ifdef AP_DYNAMIC_VGA_SUPPORT
INT set_ap_dynamic_vga_proc(PRTMP_ADAPTER pAd, PSTRING arg)
{
	UINT dvga_en;
	
	dvga_en = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_OFF, ("%s(): dvga_en=%d\n",
				__FUNCTION__, dvga_en));
	if (dvga_en)
		pAd->dvga_enable = TRUE;
	else
	{
		pAd->dvga_enable = FALSE;
		if (IS_MT7601(pAd))
...
INT set_ap_dvga_rssi_proc(PRTMP_ADAPTER pAd, PSTRING arg)
{
	LONG dvga_rssi_th;
	
	dvga_rssi_th = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_OFF, ("%s(): dvga_rssi_th=%d\n",
				__FUNCTION__, dvga_rssi_th));

	pAd->dvga_rssi_bw20_th = dvga_rssi_th;
	pAd->dvga_rssi_bw40_th = dvga_rssi_th + 3;

	return TRUE;
}

INT set_ap_dvga_ccah_proc(PRTMP_ADAPTER pAd, PSTRING arg)
{
	UINT dvga_cca_th;
	
	dvga_cca_th = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_OFF, ("%s(): dvga_cca_high=%d\n",
				__FUNCTION__, dvga_cca_th));

	pAd->dvga_cca_h_th = dvga_cca_th;

	return TRUE;
}

INT set_ap_dvga_ccal_proc(PRTMP_ADAPTER pAd, PSTRING arg)
{
	UINT dvga_cca_th;
	
	dvga_cca_th = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_OFF, ("%s(): dvga_cca_th=%d\n",
				__FUNCTION__, dvga_cca_th));

	pAd->dvga_cca_l_th = dvga_cca_th;
...
#ifdef AP_DYNAMIC_PD_LPF_SUPPORT
INT set_ap_dynamic_pd_proc(PRTMP_ADAPTER pAd, PSTRING arg)
{
	UINT dpd_en;
	
	dpd_en = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_OFF, ("%s(): dpd_en=%d\n",
				__FUNCTION__, dpd_en));
	...
	{
		pAd->dpd_enable = FALSE;
		if (IS_MT7601(pAd))
			mt7601_ap_dynamic_pd_lpf_restore(pAd, pAd->CommonCfg.BBPCurrentBW);
	}

	return TRUE;
}

INT set_ap_dpd_ccah_proc(PRTMP_ADAPTER pAd, PSTRING arg)
{
	UINT dpd_cca_th;
	
	dpd_cca_th = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_OFF, ("%s(): dpd_cca_high=%d\n",
				__FUNCTION__, dpd_cca_th));

	pAd->dpd_cca_h_th = dpd_cca_th;

	return TRUE;
}

INT set_ap_dpd_ccal_proc(PRTMP_ADAPTER pAd, PSTRING arg)
{
	UINT dpd_cca_th;
	
	dpd_cca_th = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_OFF, ("%s(): dpd_cca_th=%d\n",
				__FUNCTION__, dpd_cca_th));

	pAd->dpd_cca_l_th = dpd_cca_th;
