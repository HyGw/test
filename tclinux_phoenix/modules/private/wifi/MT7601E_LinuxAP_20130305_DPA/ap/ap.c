...
/*			pMbss->CapabilityInfo |= 0x0200; */
	...
	RTMP_IO_READ32(pAd, RX_RING_CIDX, &rx_cpu_idx);
	RTMP_IO_READ32(pAd, RX_RING_DIDX, &rx_dma_idx);

//	printk("%s: rx_cpu_idx %d, rx_dma idx %d\n", __FUNCTION__, rx_cpu_idx, rx_dma_idx);
	if (rx_cpu_idx == rx_dma_idx)
	{
		RTMP_IO_READ32(pAd, WPDMA_GLO_CFG, &Value);
		RTMP_IO_WRITE32(pAd, WPDMA_GLO_CFG, (Value & ~0x5));
		/* before clean ring, need to stop DMA first */
...
//	RTMP_CLEAR_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS);
	...
	*/
	...
	if ( IS_MT7601(pAd) )
	{
		...
		RTMP_IO_READ32(pAd, WPDMA_GLO_CFG, &Val);
		if ((Val & 0x5) == 0)
		{	
			DBGPRINT(RT_DEBUG_TRACE, ("Can't send in-band cmd, DMA is not ready.\n"));
			DBGPRINT(RT_DEBUG_TRACE, ("<=== APStartUp\n"));
			return;
		}
	...
	// Neil: suggest to use it as the augmentation
	{
		BOOLEAN bEdcca = FALSE;
	
		bEdcca = GetEDCCASupport(pAd);
	
		printk("\x1b[33m%s: bEdcca %d\x1b[m\n", __FUNCTION__, bEdcca);
		if (IS_MT7601(pAd) && bEdcca)
		{
			ed_monitor_init(pAd);
		}
	}
...
#ifdef ED_MONITOR
	// TODO: for certification only!
	// Neil: suggest to use it as an augmentation
	if (IS_MT7601(pAd) && pAd->ed_chk)
	{
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) 
	{
		...
		if (pEntry->PsQueue.Head)
		{
			...
			{
			#ifdef BB_SOC			
				//modified for hang without CPU binding issue
	...
	//printk("\x1b[32m%s: total sta %d\x1b[m\n", __FUNCTION__, total_sta);
#endif
...

{
...
#ifdef BB_SOC
		/* for small RX ring setting, clear all RX ring after set flag */
		RTMP_IO_READ32(pAd, RX_RING_CIDX, &rx_cpu_idx);
		RTMP_IO_READ32(pAd, RX_RING_DIDX, &rx_dma_idx);
	...
	//	dbg_plinel_1("\r\n==>APOverlappingBSSScan(4): MacReg(0x208) = ", MacReg);
	}
#endif 
...
//		dbg_plinel_1("\r\n==>RxCpuIdx = ", RxCpuIdx);
		RTMP_IO_READ32(pAd, WPDMA_GLO_CFG, &MacReg);
//		dbg_plinel_1("\r\n==>APOverlappingBSSScan(5): MacReg(0x208) = ", MacReg);
	}
#endif 
