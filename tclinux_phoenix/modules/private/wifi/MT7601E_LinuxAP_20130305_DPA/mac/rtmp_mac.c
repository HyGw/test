...
VOID RTMPWriteTxWI_Data(RTMP_ADAPTER *pAd, TXWI_STRUC *pTxWI, TX_BLK *pTxBlk)
{
	...
	*/
	...
	//pTxWI->TxWITXLUT = pAd->bUseHwTxLURate;
#ifdef PEER_DELBA_TX_ADAPT
	if (RTMP_GET_PACKET_LOWRATE(pTxBlk->pPacket) || (pTxBlk->TxFrameType == TX_MCAST_FRAME))
		pTxWI->TxWITXLUT = FALSE;
	else
	pTxWI->TxWITXLUT = pAd->bUseHwTxLURate;
