...
static RTMP_REG_PAIR	MT7601_MACRegTable[] = {
	...
	{PBF_CFG,				0x7F723c1f},
	{FCE_CTRL,				0x1},
	{0xA38,					0x8},			// For bi-direction RX fifo overflow issue.
	...
	IN BOOLEAN bResetWLAN)
{
...
#ifdef RTMP_MAC_PCI
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
	RTMP_INT_LOCK(&pAd->WlanEnLock, IrqFlags);
#else
...

	{
			...
			printk("\x1b[31m%s: ... 0x9ac %d, bResetWLAN %d\x1b[m\n", __FUNCTION__, MacVal, bResetWLAN);
	...
	if (bOn == TRUE)
	{
		...
		do
		{
			do 
			{
				...
				*/
				/*  DE: suggest that it is unnecessary to check XTAL_RDY bit, beacause if XTAL_RDY bit
                        is not ready. The driver can't read any register.
                */
				if (CmbCtrl.field.PLL_LD == 1)
					break;
...
#ifdef RTMP_MAC_PCI
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
	RTMP_INT_UNLOCK(&pAd->WlanEnLock, IrqFlags);
#else
...
static VOID NICInitMT7601RFRegisters(RTMP_ADAPTER *pAd)
{
	...
	/* check Xtal freq */
	RTMP_IO_READ32(pAd, MISC_CTRL, &MacVal);
	if (MacVal & XTAL_SEL_20)
		rlt_rf_write(pAd, RF_BANK0, RF_R13, 0x13);
	
	/* set default antenna as main */
	if (IS_MT7601(pAd)) /* IT1R Only */
...
#ifdef BB_SOC
		RTMP_IO_READ32(pAd, CMB_CTRL, &RegValue);	
		/* Enable MT7601E GPIO SWITCH */
		RegValue |= (0x1<<17);
		RegValue |= (0x1<<18);
		RegValue |= (0x1<<11);
		RegValue |= (0x1<<15);
		/* OSBN use GPIO0 as WLAN LED */
		RegValue &= ~(0x1 << 22);
		RTMP_IO_WRITE32(pAd, CMB_CTRL, RegValue);
#endif /* BB_SOC */
	/*
...
VOID MT7601_TxPowerPercentage(
	IN PRTMP_ADAPTER		pAd,
	INOUT PINT32				TxPower)
{
	UINT32	Value;
//	printk("\r\n==>MT7601_TxPowerPercentage: TxPower = %d\r\n", *TxPower);
...
//	printk("\r\n==>MT7601_TxPowerPercentage: before Value = %x\r\n", Value);
	Value = Value & (~0x3F3F);
	Value |= (*TxPower & 0x3F);
	RTMP_IO_WRITE32(pAd, TX_ALC_CFG_0, Value);

//	printk("\r\n==>MT7601_TxPowerPercentage: after Value = %x\r\n", Value);
}
	...
	IN UCHAR				BandWidth)
{
	...
	{	
		...
		R66 += 2 * (lan_gain - 8 ); /* RX gain is too low */
		R196 += 2 * (lan_gain - 8 ); /* RX gain is too low */
		...
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196, R196);

#ifdef CONFIG_AP_SUPPORT
#ifdef AP_DYNAMIC_VGA_SUPPORT
		pAd->dvga_main_def_gain = R66;	
		pAd->dvga_aux_def_gain = R196;
//		printk("\x1b[32m%s: default VGA gain main %x, aux %x\x1b[m\n", __FUNCTION__, pAd->dvga_main_def_gain, pAd->dvga_aux_def_gain); 
#endif /* AP_DYNAMIC_VGA_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
	}

	//printk("\x1b[31m%s: BBP R66 %x\x1b[m\n", __FUNCTION__, R66);
}
...
INT MT7601_set_ed_cca(RTMP_ADAPTER *pAd, BOOLEAN enable)
{
	...
	if (enable) {
		...
		// BBP: Bumpy Energy Control 
		bbp_val = 0x03;
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R123, bbp_val);

		// BBP: enable ED_CCA and high/low threshold
		bbp_val = 0x05; /* 0x2e */ // bit 0~7 for high threshold
	...
	BOOLEAN	 bScan)
{
	...
	if (pAd->TxPowerCtrl.bInternalTxALC)
	{
		...
		pAd->TxPower[Channel - 1].Power = SkuBasePwr;
	}
	else
#endif /* RTMP_INTERNAL_TX_ALC */
#endif /* SINGLE_SKU_V2 */
	TxPwer = pAd->TxPower[Channel - 1].Power;
//	printk("\r\n==>MT7601_ChipSwitchChannel: befor percentage TxPwer = %d Channel = %d\r\n",TxPwer, Channel);
	MT7601_TxPowerPercentage(pAd, &TxPwer);
//	printk("\r\n==>MT7601_ChipSwitchChannel: after percentage TxPwer = %d Channel = %d\r\n",TxPwer,Channel);
	DBGPRINT(RT_DEBUG_TRACE, ("TxPwer = %d, TxPower[%d]=%d\n", TxPwer, (Channel - 1), pAd->TxPower[Channel - 1].Power));
	...
	*/
	...
	{	
		if (!pAd->chipCap.IsComboChip)
			NICEraseFirmware(pAd); /* will reset FCE */
...
VOID MT7601PCIAsicRadioOff(RTMP_ADAPTER *pAd, UCHAR Stage)
{
	...
	/* mark to workaround to solve WL_GPIO3 be configured by mistake*/
	//	PWR_SAVING_OP(pAd, RADIO_OFF, 1, 0, 0, 0, 0);
	/* RadioOff don't let hw to set beacon. */
	AsicDisableSync(pAd);
...
VOID MT7601_TssiDcGainCalibration(RTMP_ADAPTER *pAd)
{
...
#ifdef RT_BIG_ENDIAN
		tssi_linear = le2cpu16(tssi_linear);
#endif /* RT_BIG_ENDIAN */
	...
	DBGPRINT(RT_DEBUG_TRACE, ("TssiDC0_HVGA = %d (0x%x)\n", pTxALCData->TssiDC0_HVGA, pTxALCData->TssiDC0_HVGA));
...
#ifdef RT_BIG_ENDIAN
	printk("\x1b[31m%s: 0x6E Value %x\x1b[m\n", __FUNCTION__, Value);
	pTxALCData->TssiSlope = (Value & 0xFF0000) >> 16;

	/* TSSI Offset Channel 1 ~ 4 in EEPROF 0x6F s.3.4 */
	pTxALCData->MT7601_TSSI_OFFSET[0] = (Value & 0xFF000000) >> 24;
	RT28xx_EEPROM_READ16(pAd, 0x70, Value);
	pTxALCData->MT7601_TSSI_OFFSET[1] = (Value & 0xFF0000) >> 16;
	pTxALCData->MT7601_TSSI_OFFSET[2] = (Value & 0xFF000000) >> 24;
#else
...
//	RT28xx_EEPROM_READ16(pAd, EEPROM_TX0_TSSI_OFFSET, Value);
	RT28xx_EEPROM_READ16(pAd, EEPROM_TX0_TSSI_OFFSET, Value2);
	pTxALCData->MT7601_TSSI_T0_Delta_Offset = ((CHAR)(Value2 & 0xFF)) * 1024;
...
#endif /* DOT11_N_SUPPORT */	
//	printk("\r\n==>MT7601_GetTssiCompensationParam: before TxPowerPercentage Power = %d\r\n",Power);
	MT7601_TxPowerPercentage(pAd, &Power);	
//	printk("\r\n==>MT7601_GetTssiCompensationParam: after TxPowerPercentage Power = %d\r\n",Power);
	DBGPRINT(RT_DEBUG_TRACE, ("Channel Desire Power = %d, Before TxPowerPercentage =%d , chan %d\n", Power, pAd->TxPower[pAd->CommonCfg.CentralChannel-1].Power,  pAd->CommonCfg.CentralChannel-1));
    ...
    IN UCHAR            Ant)
{
   ...
   RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R150, &Val);
   Val &= ~0x80;
   RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R150, Val); /* Disable ANTSW_OFDM */
...
#ifdef RTMP_PCI_SUPPORT
	pChipOps->AsicRadioOn = MT7601PCIAsicRadioOn;
	pChipOps->AsicRadioOff = MT7601PCIAsicRadioOff;
...
#ifdef CONFIG_AP_SUPPORT
#ifdef AP_DYNAMIC_VGA_SUPPORT
VOID mt7601_ap_dynamic_vga_adjust(
	IN PRTMP_ADAPTER pAd)
{
	CHAR rssi_th = pAd->dvga_rssi_bw40_th;

	if (pAd->CommonCfg.BBPCurrentBW == BW_20)
		rssi_th = pAd->dvga_rssi_bw20_th;
	else if (pAd->CommonCfg.BBPCurrentBW == BW_40)
		rssi_th = pAd->dvga_rssi_bw40_th;
	
	if (pAd->dvga_acs_rssi > rssi_th)
	{
		CHAR r66, r196;

		RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R66, &r66);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x87);
		RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R196, &r196);

		/* reduce VGA gain */
		if (pAd->RalinkCounters.OneSecFalseCCACnt > pAd->dvga_cca_h_th)
		{

			/* mimimum bound to -8dBm(0x10) */
			if (r66 < (pAd->dvga_main_def_gain + 0x10))
			{
				/* main antenna */
				r66 += 4; /* 2 dBm */
		...
		/* increase VGA gain */
		else if (pAd->RalinkCounters.OneSecFalseCCACnt < pAd->dvga_cca_l_th)
		{

			/* maximum bound to default VGA */
			if (r66 > pAd->dvga_main_def_gain)
			{
				/* main antenna */
				r66 -= 4; /* 2 dBm */
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R66, r66);
	...
	//	printk("\x1b[32m%s: r66 %x, r196 %x, cca %d, rssi %d\x1b[m\n", __FUNCTION__, r66, r196, pAd->RalinkCounters.OneSecFalseCCACnt, pAd->dvga_acs_rssi);
	}
	else	 /* restore to default VGA gain */
		MT7601_ChipAGCInit(pAd, pAd->CommonCfg.BBPCurrentBW);

}
#endif /* AP_DYNAMIC_VGA_SUPPORT */

#ifdef AP_DYNAMIC_PD_LPF_SUPPORT
static VOID mt7601_ap_dynamic_pd_lpf_reduce(
	IN PRTMP_ADAPTER pAd,
	IN CHAR bw)
{
	/* reduce LPF corner */
	if (bw == BW_20)
	{
		rlt_rf_write(pAd, RF_BANK5, RF_R06, 0x3E);
		rlt_rf_write(pAd, RF_BANK5, RF_R07, 0x3E);
	}
	else if (bw == BW_40)
	{
		rlt_rf_write(pAd, RF_BANK5, RF_R06, 0x1E);
		rlt_rf_write(pAd, RF_BANK5, RF_R07, 0x1E);
	}
	...
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R68, 0xDD);
	/* aux antenna */
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x81);	//AGC_R129
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196, 0xD);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x82);	//AGC_R130
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196, 0x5);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x80);	//AGC_R128
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196, 0xDD);
}

VOID mt7601_ap_dynamic_pd_lpf_restore(
	IN PRTMP_ADAPTER pAd,
	IN CHAR bw)
{
	if (bw == BW_20)
	{
		...
		/* increase PD threshold */
		/* main antenna */
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R69, 0x12);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R70, 0x9);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R68, 0x8B);
		/* aux antenna */
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x81);	//AGC_R129
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196, 0x12);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x82);	//AGC_R130
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196, 0x9);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x80);	//AGC_R128
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R196, 0x8B);
	}
	else if (bw == BW_40)
	{
		/* increase LPF corner */
		...
		/* increase PD threshold */
		/* main antenna */
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R69, 0x15);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R70, 0x9);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R68, 0x8B);
		/* aux antenna */
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R195, 0x81);	//AGC_R129
...
VOID mt7601_ap_dynamic_pd_lpf_adjust(
	IN PRTMP_ADAPTER pAd)
{
	CHAR bw = pAd->CommonCfg.BBPCurrentBW;

	/* reduce PD threshold, LPF */
	if (pAd->RalinkCounters.OneSecFalseCCACnt > pAd->dpd_cca_h_th)
		mt7601_ap_dynamic_pd_lpf_reduce(pAd, bw);
	/* restore PD threshold, LPF */
	else if (pAd->RalinkCounters.OneSecFalseCCACnt < pAd->dpd_cca_l_th)
		mt7601_ap_dynamic_pd_lpf_restore(pAd, bw);

//	printk("\x1b[32m%s: False CCA %d\x1b[m\n", __FUNCTION__, pAd->RalinkCounters.OneSecFalseCCACnt);
