...
//ULONG mem_alloc_size;
//ULONG pkt_alloc_size;
/*
	...
	IN PVOID mem)
{
	...
	//printk("%s: __line__ %d --> free ptr %p\x1b[m\n", func, line, mem);
#ifdef VENDOR_FEATURE4_SUPPORT
	OS_NumOfMemFree++;
#endif /* VENDOR_FEATURE4_SUPPORT */
...
PNDIS_PACKET RtmpOSNetPktAlloc(VOID *dummy, int size)
{
	...
	//pkt_alloc_size += size;
	//printk("pkt_alloc_size \x1b[31m%d/%d\x1b[m\n", pkt_alloc_size, size);
	if (skb != NULL)
...
VOID RtmpOsPktRcvHandle(PNDIS_PACKET pNetPkt)
{
	...
	pRxPkt->ip_summed = CHECKSUM_NONE;
	//printk("\x1b[31m%s: ip_sumed %d, csum %x, feature %x\x1b[m\n", __FUNCTION__, pRxPkt->ip_summed, pRxPkt->csum, pNetDev->features);
#endif
