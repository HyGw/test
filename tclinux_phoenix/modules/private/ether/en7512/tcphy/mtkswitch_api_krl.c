...
int macMT7530GetBRTable(mt7530_switch_brtable *brtable, u8 port)
{
	...
	while (1)
	{
		...
		if ( ( (value & REG_AT_SRCH_END_MASK) && ( ((value>> REG_AT_ADDR_OFFT) & REG_AT_ADDR_RELMASK)==0x7ff ) )
			|| (brtable->count >= 2048) )
		{
			ret_val = 0;
...
int macMT7530SetMACForceFC(u8 port,u8 mode)
{
	...
	value = (macMT7530ReadReg(reg) & (~(0x3<<4)) ) | ( mode ? 0x3<<4 : 0 );
	macMT7530WriteReg(reg, value);
...
void macMT7530PortReset(u8 port)
{
	...
	if(linkstate)
	{
		macMT7530SetPortAdmin(1, port);
		mdelay(100);  /* changed by baiyong & jason.gong*/
		macMT7530SetPortAdmin(0, port);
	}
	return;
}
...
int macMT7530GetPortRxByte(u8 port, unsigned int* RxByte_L, unsigned int* RxByte_H)
{
	...
	*RxByte_L = macMT7530ReadReg(reg);

	reg = REG_PORT_RX_BYTE_HIGH_CNT + (0x100 * port);
	*RxByte_H = macMT7530ReadReg(reg);
...
int macMT7530GetPortTxByte(u8 port, unsigned int* TxByte_L, unsigned int* TxByte_H)
{
	...
	*TxByte_L = macMT7530ReadReg(reg);

	reg = REG_PORT_TX_BYTE_HIGH_CNT + (0x100 * port);
	*TxByte_H = macMT7530ReadReg(reg);
...
inline u32 macMT7530LanPortMap2Switch(u32 lanPortId){
	int switch_port ;
	if (lanPortId < RT63365_SWIC_PORTNUM-1){
...
inline u32 macMT7530SwitchMap2LanPort(u32 switchPortId){
	...
	if (switchPortId < RT63365_SWIC_PORTNUM-1){
		...
		if (0 <= LanPort &&  LanPort < RT63365_SWIC_PORTNUM-1)
			return LanPort;
	}
	return switchPortId;
}
...
u8 getPortRevertFWC(u8 in_port)
{
	...
	for ( idx = 0; idx < RT63365_SWIC_PORTNUM; idx ++ )
	{
...
int macMT7530SetPortFollowControlGlobal(u8 enable)
{
	u32 reg, val;
...
int macMT7530GetPortFollowControlGlobal(void)
{
	u32 reg, val;
	u8 ret;
...
int macMT7530SetPortFollowControl(u8 port, u8 enable)
{
...
int macMT7530GetPortFollowControl(u8 port)
{
	u32 reg, val;
	u8 ret;
...
void set_port_matrix(u8 port, u8 matrix)
{
    ...
    reg_value = macMT7530ReadReg(reg_add);
    reg_value = (reg_value & (~0xFF0000)) | (matrix<<16);
    macMT7530WriteReg(reg_add, reg_value);
}
...
int macMT7530SetPortMatrix(u8 group[4])
{
    u8 lan_port, group_id, matrix, switch_port;
    u32 reg_value, reg_add;

    for(lan_port=0;lan_port<4;lan_port++)
    {
        if((0 == group[lan_port]) || (group[lan_port] > 4))
        {
    ...
    {
        matrix = 0x40;   /* port 6 in matrix by default*/
        for(lan_port=0; lan_port<4; lan_port++)
        {
            if(group[lan_port] == group_id){
                matrix |= 1<<macMT7530LanPortMap2Switch(lan_port);
            }
        }
        for(switch_port=0; switch_port<6; switch_port++)
        {
            if(matrix & (1<<switch_port))
            {
...
//				goto err_exit;
		...
		{
			ret_value = macMT7530SetPortLoopBackRemote(portId, (u8)(u32)(api_param.paramext_ptr));
		}
		break;
		...
		{
			ret_value = macMT7530SetPortMode(portId, (u8)(u32)(api_param.paramext_ptr));
		}
		break;
		...
		case CMD_GET_PORT_LINKSTATE:
		{
			...
			if (copy_to_user((u32 *)(paramext.p1), &linkstate, sizeof(speed)))
			{
				ret_value = -1;
			}
			if (copy_to_user((u32 *)(paramext.p2), &speed, sizeof(speed)))
			{
				ret_value = -1;
			}
		}
		...
		case CMD_SET_PORT_FOLLOWCONTROL_GLOBAL:
		{
			ret_value = macMT7530SetPortFollowControlGlobal((u8)(u32)(api_param.paramext_ptr));
		}
		break;

		case CMD_GET_PORT_FOLLOWCONTROL_GLOBAL:
		{
			u8 enable;
			enable = macMT7530GetPortFollowControlGlobal();
			if (copy_to_user(api_param.paramext_ptr, &enable, sizeof(enable)))
			{
				ret_value = -1;
			}
		...
		case CMD_SET_PORT_FOLLOWCONTROL:
		{
			if((5==portId)||(6==portId))
				ret_value = macMT7530SetPortFollowControl(portId, (u8)(u32)(api_param.paramext_ptr));
			else{
				ret_value = macMT7530SetPortPause(portId, (u8)(u32)(api_param.paramext_ptr));
		...
		case CMD_GET_PORT_FOLLOWCONTROL :
		{
			u8 enable; 
			if((5==portId)||(6==portId))
				enable = macMT7530GetPortFollowControl(portId);
			else
				enable = macMT7530GetPortPause(portId);
			if(-1 == enable)
				ret_value = -1;
			else if (copy_to_user(api_param.paramext_ptr, &enable, sizeof(enable)))
			{
				ret_value = -1;
			}
		}
        ...
        {
            ret_value = macMT7530SetPortIsolate((u8)(u32)(api_param.paramext_ptr));
        }
        break;

        case CMD_SET_PORT_MATRIX:
        {
            ret_value = macMT7530SetPortMatrix((u8 (*)[4])(api_param.paramext_ptr));
        }
		...
		{
			ret_value = macMT7530SetPortMaxBitRate(portId, (char *)(api_param.paramext_ptr));
		}
			break;
		...
		{
			ret_value = macMT7530SetPortDuplexMode(portId, (char *)(api_param.paramext_ptr));
		}
			break;
		...
		case CMD_GET_PORT_RX_BYTE :
		{
			...
			if (ret_value == -1) goto err_exit;
			if (copy_to_user(paramext.p1, &RxByteCnt_H, sizeof(unsigned long)))
			{
				ret_value = -1;
			}
			if (copy_to_user(paramext.p2, &RxByteCnt_L, sizeof(unsigned long)))
			{
				ret_value = -1;
			}
		}
		...
		case CMD_GET_PORT_TX_BYTE :
		{
			...
			if (ret_value == -1) goto err_exit;
			if (copy_to_user(paramext.p1, &TxByteCnt_H, sizeof(unsigned long)))
			{
				ret_value = -1;
			}
			if (copy_to_user(paramext.p2, &TxByteCnt_L, sizeof(unsigned long)))
			{
				ret_value = -1;
			}
		}
...
//				goto err_exit;
		...
		case CMD_GET_PORT_RATE_LIMIT :
		{
			...
			ret_value = macMT7530GetPortRateLimit(portId, paramext.p2, &rate);
		...
		{
			ret_value = macMT7530SetDsPortQos(portId, (u8)(u32)(api_param.paramext_ptr));
		}
				break;
...
int macMT7530SetPortRateLimit(u8 port, u32 rate, u8 dir)
{
	...
	if (dir==1){			// ingress
		reg = 0x1FF0;
		val = macMT7530ReadReg(reg)& 0xFFFFFF00;
		val |= 0x18;
		macMT7530WriteReg(reg, val);
	...
	val = ((rate/32)<<16) + (enable<<15) + (5<<8) + (1<<7) + 0xf; /* modify rate measurement period from 9:4ms to 0:1/128ms */
