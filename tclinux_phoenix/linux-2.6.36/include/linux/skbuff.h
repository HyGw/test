...
#define ROUTE_INTERFACE		2
	...
	__u8 qosEnque;	/*0:enqueu flag, 1: from hw_nat flag for PON, 4-7: qos queue index for PON*/
#endif
#if defined(CONFIG_NF_CONNTRACK) || defined(CONFIG_NF_CONNTRACK_MODULE)
...
/*defined(TCSUPPORT_PON_VLAN) || defined(TCSUPPORT_PON_MAC_FILTER) || (defined(TCSUPPORT_GPON_MAPPING) && defined(TCSUPPORT_GPON_DOWNSTREAM_MAPPING))*/
	__u8 ppe_info_flag;
	__u16 ppe_magic;
	__u8 ppe_ai;
	__u16 ppe_foe_entry;
...
static inline void __nf_copy(struct sk_buff *dst, const struct sk_buff *src)
{
	...
	dst->qosEnque = src->qosEnque;	/*0:enqueu flag, 1: from hw_nat flag for PON, 4-7: qos queue index for PON*/
#endif
...
#if defined(TCSUPPORT_LAN_VLAN) || defined(TCSUPPORT_LAN_VLAN_RANGE) || defined(TCSUPPORT_CMCC)
...
#endif
#if defined(TCSUPPORT_CT)
static inline struct sk_buff *skbmgr_dev_alloc_skb2k_tc_critical(void)
{
	struct sk_buff *skb = alloc_skb(SKBMGR_RX_BUF_LEN, GFP_ATOMIC|__GFP_NOWARN | __GFP_TC_CRITICAL);
	if (likely(skb))
		skb_reserve(skb, NET_SKB_PAD);
	return skb;
}
