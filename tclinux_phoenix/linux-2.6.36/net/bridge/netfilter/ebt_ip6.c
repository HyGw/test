...
ebt_ip6_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	int i;
#if defined(TCSUPPORT_CT_PORTSLIMIT)
	const struct icmp6hdr *mptr = NULL;
#endif
...
#if defined(TCSUPPORT_CT_PORTSLIMIT)
			&& !(info->bitmask & EBT_IP6_ICMPV6TYPE)
#endif
		    ){		    
			return true;
		}
...
#if defined(TCSUPPORT_CT_PORTSLIMIT)
		if (info->bitmask & EBT_IP6_ICMPV6TYPE) {
			mptr = (struct icmp6hdr *)pptr;
			if (FWINV((info->icmpv6type != mptr->icmp6_type), EBT_IP6_ICMPV6TYPE))
			return false;
		}
#endif
		return true;
