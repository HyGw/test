...
#if defined(TCSUPPORT_XPON_IGMP) && defined(TCSUPPORT_MULTICAST_SPEED)
static int g_last_snoop_state = 0;
static unsigned int hw_igmp_flood_enable = 1;
static unsigned int g_snooping_enable = 1;
#endif
...
#if !defined(TCSUPPORT_XPON_IGMP_CHT)	
				 ...
				 struct sk_buff *skb)
{
	...
	{
		iph = (struct iphdr *)(skb_mac_header(skb) + VLAN_ETH_HLEN);
		len = ntohs(iph->tot_len) + VLAN_HLEN;
	}
	else
		return -EINVAL;
#else
	...
	if ( skb2->protocol == htons(ETH_P_IP) )
		offset = skb_network_offset(skb2) + hdr_len;
	else if( (skb2->protocol == htons(ETH_P_8021Q)) && (vlan_proto(skb2) == htons(ETH_P_IP)) )
		offset = skb_network_offset(skb2) + hdr_len + VLAN_HLEN;
	else
		return -EINVAL;
#else
	len -= ip_hdrlen(skb2);
...
#if defined(TCSUPPORT_CT)
	if ( skb2->protocol == htons(ETH_P_IPV6) )
		ip6h = ipv6_hdr(skb2);
	else if( (skb2->protocol == htons(ETH_P_8021Q)) && (vlan_proto(skb2) == htons(ETH_P_IPV6)) )
		ip6h = (struct iphdr *)(skb_mac_header(skb2) + VLAN_ETH_HLEN);
	else
		return -EINVAL;
#else
	ip6h = ipv6_hdr(skb2);
...
extern int (*xpon_igmp_exist)(void);
extern int (*xpon_igmp_clear_flows)(void);
#endif
...
#ifdef TCSUPPORT_DS_HWNAT_OFFLOAD
extern int (*xpon_igmp_exist)(void) = NULL;
extern int (*xpon_igmp_clear_flows)(void) = NULL;
#endif
...
#ifdef TCSUPPORT_DS_HWNAT_OFFLOAD
int igmp_hwnat_exist(void)
{
	IGMP_HWNATEntry_t* entry = NULL,*tmp = NULL;
	struct list_head* hwnatflow = NULL;

	hwnatflow = igmp_hwnat_get_list();

#ifdef TCSUPPORT_MULTICAST_SPEED
	list_for_each_entry_rcu(entry,hwnatflow,list)
#else
	list_for_each_entry_safe(entry,tmp,hwnatflow,list)
#endif
	{
		return 1;
	}
...
#if defined(TCSUPPORT_XPON_IGMP) && defined(TCSUPPORT_MULTICAST_SPEED)
	list_for_each_entry_safe(entry, tmp, &multicast_flood_hw_list, list)
	{
	    return 1;
	}
#endif
...
EXPORT_SYMBOL(igmp_hwnat_get_port);
...
EXPORT_SYMBOL(igmp_hwnat_flow_index);
...
#if defined(TCSUPPORT_XPON_IGMP) && defined(TCSUPPORT_MULTICAST_SPEED)
int get_multicast_flood_hw_flag(void)
{
    return hw_igmp_flood_enable;
}
EXPORT_SYMBOL(get_multicast_flood_hw_flag);

/***return true:    igmp snooping ***********
/****** * false:   multicast flood ***********/
int get_multicast_snooping_state(void)
{
    return g_snooping_enable;
}
EXPORT_SYMBOL(get_multicast_snooping_state);
...
void add_multicast_flood_hwentry(struct sk_buff* skb)
{
    ...
    flood_flag = get_multicast_snooping_state();
    if(flood_flag)
    {
        return ;
    }
...
EXPORT_SYMBOL(add_multicast_flood_hwentry);
...
EXPORT_SYMBOL(update_multicast_flood_hwentry);
...
EXPORT_SYMBOL(find_multicast_flood_hwentry);
...
EXPORT_SYMBOL(update_multicast_flood_mask);
...
EXPORT_SYMBOL(clear_multicast_flood_hwentry);

#endif

#ifdef TCSUPPORT_DS_HWNAT_OFFLOAD
int igmp_hwnat_clear_all_flows(void)
{
	igmp_hwnat_clear_flows();
...
#ifdef TCSUPPORT_DS_HWNAT_OFFLOAD
	xpon_igmp_exist = igmp_hwnat_exist;
	xpon_igmp_clear_flows = igmp_hwnat_clear_all_flows;
#endif
