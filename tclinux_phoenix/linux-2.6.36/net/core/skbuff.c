...
#elif defined(MT7615E)
int g_max_skb_num = 4096*2;
#else
...
__IMEM static void skb_release_data(struct sk_buff *skb)
{
 ...
 */
__IMEM static void kfree_skbmem(struct sk_buff *skb)
{
...
#endif
#if defined(TCSUPPORT_RA_HWNAT)
	new->bridge_flag = old->bridge_flag;
#endif
...
struct sk_buff *skb_clone(struct sk_buff *skb, gfp_t gfp_mask)
{
	...
	if (skip_pktflow)
		...
		pktflow_xfer(n, skb);	
#endif
#if 0//defined(TCSUPPORT_RA_HWNAT)
	if (ra_sw_nat_hook_xfer)
		ra_sw_nat_hook_xfer(n, skb);
#endif
...
#if defined (MT7615E)
__DMEM int skbmgr_limit = 8192/*1280*/;
#else
...
__DMEM int skbmgr_alloc_normal = 0;
...
__IMEM struct sk_buff *skbmgr_alloc_skb2k(void)
{
	...
	if (skb_queue_len(list)) {
		...
		alloc_no = atomic_inc_return(&skbmgr_alloc_no);
...
__IMEM struct sk_buff *skbmgr_alloc_skb4k(void)
{
	...
	if (skb_queue_len(list)) {
		...
		alloc_no = atomic_inc_return(&skbmgr_4k_alloc_no);
