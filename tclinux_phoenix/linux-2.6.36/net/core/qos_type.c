...
int qostype_chk(int chk_type, int rule_no, char *wan_if, int rtp_match)
{
	...
	if ( 0 == pqa->qm[0].type_name[0] )
		return 0;
	if ( 0 == strcmp(pqa->qm[0].type_name, "wan_if") )
	{
		if ( 0 == strcmp(pqa->qm[0].type_value, wan_if) )
			return 0;
		else
			return -1;
	}
...
int qostype_wan_check(int rule_no, struct net_device *dev)
{
	...
	{
		return qostype_chk(DEV_XMIT_CHK_TYPE, rule_no, dev->name, 0);
	}
