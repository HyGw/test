		...
		struct nf_conn *ct, enum ip_conntrack_info ctinfo) 
{
	...
	case IP_CT_DIR_ORIGINAL:	
		pr_debug("IP_CT_DIR_ORIGINAL:help_out\n");
...
static int rtsp_alg_port_read_proc(char *buf, char **start, off_t off, int count,int *eof, void *data)
{
	...
	len += sprintf(buf,"%s", "rtsp alg port:");
	for ( i = 0;  i < MAX_PORTS; i++ )
	{
		if ( 0 != ports[i] )
		{
			len += sprintf(port_buf,"%d ", ports[i]);
			strcat(buf, port_buf);
		}
	}
	buf[len - 1] = '\n';
...
static int add_rtsp_alg_port(__be16 port)
{
	...
	if ( RTSP_PORT == port ) // no need to add default port.
		return 1;
...
static int del_rtsp_alg_port(__be16 port)
{
	...
	if ( RTSP_PORT == port ) // can't del default port.
		return 1;
...
#if defined(TCSUPPORT_CT)
	rtsp_buffer = kmalloc(2048, GFP_KERNEL);
#else
