...
#if defined(TCSUPPORT_HWNAT)
   	set_bit(IPS_PKTFLOW_BIT, &ct->status);  
#endif
	return ct;
...
void nf_conntrack_free(struct nf_conn *ct)
{
	...
	ECNT_NF_TRACK_CORE_FREE_HOOK(IPCT_DESTROY, ct, NULL);
	       ...
	       unsigned int dataoff)
{
	...
	ct->ct_general.lVlanId = 4096;
	if (skb->vlan_tag_flag & (VLAN_PACKET | ROUTING_MODE_PACKET) == (VLAN_PACKET | ROUTING_MODE_PACKET)) {
                if (skb->vlan_tags[0] < 4096){
			ct->ct_general.lVlanId = skb->vlan_tags[0];
	}
        }
#endif
	spin_unlock_bh(&nf_conntrack_lock);
	...
	ECNT_NF_TRACK_CORE_INIT_HOOK(master_ct(ct) ? IPCT_RELATED : IPCT_NEW
	, ct, skb);
		  ...
		  enum ip_conntrack_info *ctinfo)
{
	...
	#ifdef TCSUPPORT_RA_HWNAT
	{
		...
		if (help && help->helper) {
			if (strcmp(help->helper->name, "FULLCONE-NAT") != 0) {
				if(!(strcmp(help->helper->name, "tftp") == 0)
				#ifdef TCSUPPORT_DS_HWNAT_OFFLOAD
				&& !(rtsp_hwnat_offload&&(strcmp(help->helper->name, "rtsp") == 0))
				#endif
				) {
