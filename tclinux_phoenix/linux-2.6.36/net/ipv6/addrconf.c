...
static struct ipv6_devconf ipv6_devconf __read_mostly = {
	...
	.slaac_addr		="",
#endif
#if defined(TCSUPPORT_CT_WAN_CHILD_PREFIX) && !defined(TCSUPPORT_FWC_ENV)
	.child_prefix_orign = 0,
	.child_prefix = "",  //used to calulate the new prefix in ra
	.parent_pd_prefix = "",  //used to calulate the new prefix in ra
	.slaac_prefix = "",  //get current ra prefix
#endif
};
...
static inline int send_rs(struct inet6_dev *in6_dev)
{
    if(!strncmp(in6_dev->dev->name, "wan", 3)
        || !strncmp(in6_dev->dev->name, "ppp", 3))
    {
        return 1;
    }
...
static void addrconf_rs_timer(unsigned long data)
{
...
#else
#if defined(TCSUPPORT_FWC_ENV)
    if (idev->cnf.forwarding && !send_rs(idev))
#else
    if (idev->cnf.forwarding)
#endif
#endif
	...
	{
		if ( IF_RS_SENT != (ifp->idev->if_flags & IF_RS_SENT) )
...
#else
#if defined(TCSUPPORT_FWC_ENV)
	if (((ifp->idev->cnf.forwarding == 0)||(send_rs(ifp->idev))) &&
#else
...
#if defined(TCSUPPORT_CT_WAN_CHILD_PREFIX) && !defined(TCSUPPORT_FWC_ENV)
		{
			.procname	= "child_prefix_orign",
			.data		= &ipv6_devconf.child_prefix_orign,
			.maxlen 	= sizeof(int),
			.mode		= 0666,
			.proc_handler	= proc_dointvec,
		},
		{
			.procname	=	"child_prefix",
			.data		=	&ipv6_devconf.child_prefix,
			.maxlen 	=	64,
			.mode		=	0666,
			.proc_handler	=	&proc_dostring,
		},
		{
			.procname	=	"parent_pd_prefix",
			.data		=	&ipv6_devconf.parent_pd_prefix,
			.maxlen 	=	64,
			.mode		=	0666,
			.proc_handler	=	&proc_dostring,
		},
		{
			.procname	=	"slaac_prefix",
			.data		=	&ipv6_devconf.slaac_prefix,
			.maxlen 	=	64,
			.mode		=	0666,
			.proc_handler	=	&proc_dostring,
		},
