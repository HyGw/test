		    ...
		    struct icmp6hdr *icmp6h)
{
	...
	skb->mark &= (~0xF0);
	skb->mark |= 0x10;
	//printk("v6 arp packet to first queue.skb->mark is 0x%x\n", skb->mark);
#endif
#endif
...
skip_linkparms:
	...
	if (neigh) {
		...
		if ( in6_dev )
		{
			neigh->parms->ra_flags &= (~(RA_FLAGS_M | RA_FLAGS_OTHER));
			if ( in6_dev->if_flags & IF_RA_MANAGED )
				neigh->parms->ra_flags |= RA_FLAGS_M;
			if ( in6_dev->if_flags & IF_RA_OTHERCONF )
				neigh->parms->ra_flags |= RA_FLAGS_OTHER;
		}
#endif
#endif
	}
