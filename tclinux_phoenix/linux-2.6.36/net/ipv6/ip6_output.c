...
#if defined(TCSUPPORT_CT_ACCESSLIMIT)
	typeof(al_main_check_hook_ip6) al_main_check_info;

	al_main_check_info = rcu_dereference(al_main_check_hook_ip6);
	if(al_main_check_info)
	{
		if(al_main_check_info(skb, 2) == NULL)
		{
			//printk("\r\nip6_forward_finish fail!");
			return -1;
		}
	}
#endif
