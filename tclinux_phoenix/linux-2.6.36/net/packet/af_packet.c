...
#endif
#if defined(TCSUPPORT_VLAN_TAG) || defined(TCSUPPORT_PON_VLAN) 
//#include <linux/if_vlan.h>
...
#if defined(TCSUPPORT_CT)
	if ( ('b' == orig_dev->name[0]) && ('r' == orig_dev->name[1]) )
	{
		p_proto = (u16*)(skb->data + 12);
		if ( *p_proto == htons(ETH_P_PPP_DISC)
			|| *p_proto == htons(ETH_P_PPP_SES) )
			lan_mark = (skb->mark & 0xf0000000) >> 28;
	}
#endif
			  ...
			  struct msghdr *msg, size_t len)
{
	...
	skb->mark = sk->sk_mark;
#ifdef TCSUPPORT_MULTICAST_SPEED	
		foe_info_op(sk,skb,RECEIVE_OR_SEND,FOEINFORECOVER);
#endif
...
#ifdef TCSUPPORT_MULTICAST_SPEED	
		foe_info_op(sk,skb,RECEIVE_OR_SEND,FOEINFOSTORE);
#endif
		...
		return put_user(amount, (int __user *)arg);
			...
			case SIOCGSKBMARK:
			{
				...
				if (skb)
					mark = skb->mark;
				spin_unlock_bh(&sk->sk_receive_queue.lock);
				return put_user(mark, (int __user *)arg);
			}
#endif
	case SIOCGSTAMP:
