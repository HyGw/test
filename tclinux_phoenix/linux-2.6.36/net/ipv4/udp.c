...
send:
	...
	if( (inet->inet_sport == 53) && (sk->lVlanId < 4096) ) {
...
#if defined(TCSUPPORT_CT_VLAN_BIND)
		if ((inet->inet_sport == 53)/* || (dport == 53)*/)
			sk->sk_mark &= (~0x7f0000);
#endif

#if defined(TCSUPPORT_CUC_QOS)													
	if ((inet->inet_sport == 53) || (inet->inet_sport == 0x43) || (inet->inet_sport  == 0x44)){ /* udp port is 67 or 68 */
		/* dhcp packet, set it to first queue */
		//printk("inet->inet_sport = 0x%x,dport=0x%x,sk->sk_mark=0x%x\n",inet->inet_sport,dport,sk->sk_mark);
		sk->sk_mark &= ~(QOS_FILTER_MARK);		
		sk->sk_mark |= (0x10);	
	}
#endif
...
**function name: rtpRxShortCut
**
**description:
*    If the packet is rtp, remove the vlan, ppp and ip header, pass to the udp_rcv() function
**parameters:
*    struct sk_buff *skb, unsigned char vlanLayer, int flag
**global:
*    none
**return:
*    0
...
int rtpRxShortCut(struct sk_buff *skb, unsigned char vlanLayer, int flag)
{
	int protocol=0;
