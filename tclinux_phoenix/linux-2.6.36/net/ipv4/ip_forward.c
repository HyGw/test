...
#if defined(TCSUPPORT_CT_ACCESSLIMIT)
	typeof(al_main_check_hook) al_main_check_info;
#endif
...
#if defined(TCSUPPORT_CT_ACCESSLIMIT)
	al_main_check_info = rcu_dereference(al_main_check_hook);
		if(al_main_check_info)
		{
			if(al_main_check_info(skb,1) == NULL)
			{
				//printk("\r\nip_forward_finish fail!");
				return -1;
			}
		}
#endif
...
#if defined(TCSUPPORT_CT_URL_FILTER)
	if(url_filter_check_hook){
		if(url_filter_check_hook(skb) == 0){
			//printk("\nURL Filter: Drop the packet!");
			kfree_skb(skb);
			return -1;
		}
	}
