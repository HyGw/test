...
#if defined(TCSUPPORT_CT_VLAN_BIND)
static void ip_cmsg_recv_lvlanid(struct msghdr *msg, struct sk_buff *skb)
{
	__u16 lVlanId = 4096;

	if (((skb->vlan_tag_flag & (VLAN_PACKET | ROUTING_MODE_PACKET)) == (VLAN_PACKET | ROUTING_MODE_PACKET)) && (skb->vlan_tags[0] < 4096)) {
			    ...
			    int optname, char __user *optval, unsigned int optlen)
{
	...
	case IP_TOS:	/* This sets both TOS and Precedence */
		...
		/*It's for marking rtp packets*/
		if(val == RTP_TOS_VALUE || val == QOS_RTP_MARK){
			sk->sk_mark &= (~0xf0);
...
//		printk("xyz_dbg:%s, skb_mark is %x\n", __FUNCTION__, skb_mark);
		...
		case IP_SKB_VLAN_ID:
		{
			...
			if (optlen < sizeof(__u16))
				goto e_inval;
			err = -EFAULT;
			if (copy_from_user(&lVlanId, optval, sizeof(__u16)))
				break;
			sk->lVlanId = lVlanId;
			err = 0;
		
			/* dbg_info */
//	printk("ztz_dbg:%s, lVlanId=0x%x\n", __FUNCTION__, lVlanId);
		...
		case IP_SKB_PBIT:
		{
			...
			if (optlen < sizeof(__u16))
				goto e_inval;
			err = -EFAULT;
			if (copy_from_user(&lPbit, optval, sizeof(__u16)))
				break;
			sk->lPbit = lPbit;
			err = 0;

			//printk("IP_SKB_PBIT:%s, lPbit=0x%x\n", __FUNCTION__, sk->lPbit);
			break;
		}
#endif
#endif
...
//		printk("ztz_dbg:%s, lVlanId=0x%x\n", __FUNCTION__, lVlanId);
			...
			if (put_user(len, optlen))
				return -EFAULT;
			if (copy_to_user(optval, &lVlanId, len))
				return -EFAULT;
			return 0;
		}	
#endif
